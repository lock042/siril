...
/* The HSL, HSV, CIE Lab, XYZ and YUV coordinate transforms (including the
 * _float_sat and double HSL/HSV variants and rgb_to_hsvf) are now branchless
 * static inline functions in algos/colors.h: vectorisable, and with the
 * tie-for-max hue bug fixed. Only the WORD wrappers below remain out-of-line. */
...
/* The XYZ, CIE Lab and YUV transforms (rgb_to_xyzf, xyz_to_rgbf, xyz_to_LABf,
 * LAB_to_xyzf, linrgb_to_xyzf, xyz_to_linrgbf, rgb_to_yuvf, yuv_to_rgbf) are now
 * branch-free static inline functions in algos/colors.h (vectorisable). Their
 * double-precision counterparts were dropped as they had no remaining callers. */
