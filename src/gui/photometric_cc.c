...
void fill_combo_from_glist(GtkWidget *widget, GList *list, int channel, const gchar *favourite) {
	// Input validation
	if (!widget) {
		siril_log_message("fill_combo_from_glist: widget is NULL\n");
		return;
	}

	if (!GTK_IS_COMBO_BOX_TEXT(widget)) {
		siril_log_message("fill_combo_from_glist: widget is not a combo box text widget\n");
		return;
	}

	// Critical: Check if list is NULL
	if (!list) {
		siril_log_message("fill_combo_from_glist: list is NULL, cannot populate combo\n");
		return;
	}
	...
	if (!GTK_IS_COMBO_BOX_TEXT(combo)) {
		siril_log_message("fill_combo_from_glist: combo cast failed\n");
		return;
	}
	...
	if (signal_id == 0) {
		siril_log_message("fill_combo_from_glist: failed to lookup 'changed' signal\n");
		return;
	}
	...
	if (list_length >= max_items) {
		siril_log_message("fill_combo_from_glist: list appears to be corrupted (too long or circular)\n");
		goto cleanup;
	}

	if (list == com.spcc_data.osc_sensors) {
		siril_log_message("fill_combo_from_glist: populating OSC sensors (%d items)\n", list_length);

		while (iterator && item_count < max_items) {
			// Validate iterator and its data
			if (!iterator->data) {
				siril_log_message("fill_combo_from_glist: NULL data in osc_sensors list at item %d\n", item_count);
			...
			if (!object) {
				siril_log_message("fill_combo_from_glist: NULL osc_sensor object at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
			...
			if (!object->channel[0].model) {
				siril_log_message("fill_combo_from_glist: NULL model string in osc_sensor at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
			...
			if (!is_valid_string(object->channel[0].model)) {
				siril_log_message("fill_combo_from_glist: invalid string in osc_sensor at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
		...
		siril_log_message("fill_combo_from_glist: populating SPCC objects (%d items)\n", list_length);

		while (iterator && item_count < max_items) {
			// Validate iterator and its data
			if (!iterator->data) {
				siril_log_message("fill_combo_from_glist: NULL data in spcc_object list at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
			...
			if (!object) {
				siril_log_message("fill_combo_from_glist: NULL spcc_object at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
			...
			if (!object->name) {
				siril_log_message("fill_combo_from_glist: NULL name in spcc_object at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
			...
			if (!is_valid_string(object->name)) {
				siril_log_message("fill_combo_from_glist: invalid string in spcc_object at item %d\n", item_count);
				iterator = iterator->next;
				item_count++;
				continue;
			}
	...
	siril_log_message("fill_combo_from_glist: successfully added %d items to combo\n", item_count);
...
cleanup:
	...
	if (active_index >= 0 && active_index < item_count) {
		...
		siril_log_message("fill_combo_from_glist: set active index to %d\n", active_index);
	} else if (active_index >= 0) {
		siril_log_message("fill_combo_from_glist: favourite index %d is out of range (max: %d)\n", active_index, item_count - 1);
	}
}

gboolean populate_spcc_combos(gpointer user_data) {
	siril_log_message("populate_spcc_combos: starting\n");

	// Validate we're on the main thread
	if (!g_main_context_is_owner(g_main_context_default())) {
		siril_log_message("populate_spcc_combos: ERROR - not called from main thread!\n");
		return FALSE;
	}

	// Comprehensive validation of critical data structures before use
	if (!com.spcc_data.osc_sensors) {
		siril_log_message("populate_spcc_combos: ERROR - osc_sensors is NULL\n");
		return FALSE;
	}

	if (!com.spcc_data.mono_sensors) {
		siril_log_message("populate_spcc_combos: ERROR - mono_sensors is NULL\n");
		return FALSE;
	}

	if (!com.spcc_data.osc_filters) {
		siril_log_message("populate_spcc_combos: ERROR - osc_filters is NULL\n");
		return FALSE;
	}

	if (!com.spcc_data.wb_ref) {
		siril_log_message("populate_spcc_combos: ERROR - wb_ref is NULL\n");
		return FALSE;
	}

	if (!com.spcc_data.osc_lpf) {
		siril_log_message("populate_spcc_combos: ERROR - osc_lpf is NULL\n");
		return FALSE;
	}

	// Check individual filter arrays
	if (!com.spcc_data.mono_filters[RLAYER]) {
		siril_log_message("populate_spcc_combos: ERROR - red filters (RLAYER) is NULL\n");
		return FALSE;
	}

	if (!com.spcc_data.mono_filters[GLAYER]) {
		siril_log_message("populate_spcc_combos: ERROR - green filters (GLAYER) is NULL\n");
		return FALSE;
	}

	if (!com.spcc_data.mono_filters[BLAYER]) {
		siril_log_message("populate_spcc_combos: ERROR - blue filters (BLAYER) is NULL\n");
		return FALSE;
	}
	...
	if (!oscfilters) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_filters_osc widget\n");
		return FALSE;
	}

	GtkWidget *rfilters = lookup_widget("combo_spcc_filters_r");
	if (!rfilters) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_filters_r widget\n");
		return FALSE;
	}

	GtkWidget *gfilters = lookup_widget("combo_spcc_filters_g");
	if (!gfilters) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_filters_g widget\n");
		return FALSE;
	}

	GtkWidget *bfilters = lookup_widget("combo_spcc_filters_b");
	if (!bfilters) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_filters_b widget\n");
		return FALSE;
	}

	GtkWidget *lpfilters = lookup_widget("combo_spcc_filters_lpf");
	if (!lpfilters) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_filters_lpf widget\n");
		return FALSE;
	}

	GtkWidget *monosensors = lookup_widget("combo_spcc_sensors_mono");
	if (!monosensors) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_sensors_mono widget\n");
		return FALSE;
	}

	GtkWidget *oscsensors = lookup_widget("combo_spcc_sensors_osc");
	if (!oscsensors) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_sensors_osc widget\n");
		return FALSE;
	}

	GtkWidget *whitepoint = lookup_widget("combo_spcc_whitepoint");
	if (!whitepoint) {
		siril_log_message("populate_spcc_combos: ERROR - could not find combo_spcc_whitepoint widget\n");
		return FALSE;
	}

	GtkWidget *switch_widget_lookup = lookup_widget("spcc_sensor_switch");
	if (!switch_widget_lookup) {
		siril_log_message("populate_spcc_combos: ERROR - could not find spcc_sensor_switch widget\n");
		return FALSE;
	}

	GtkSwitch *switch_widget = GTK_SWITCH(switch_widget_lookup);
	if (!GTK_IS_SWITCH(switch_widget)) {
		siril_log_message("populate_spcc_combos: ERROR - spcc_sensor_switch is not a GtkSwitch\n");
		return FALSE;
	}

	// Validate preferences exist (basic check)
	if (!com.pref.spcc.oscfilterpref) {
		siril_log_message("populate_spcc_combos: WARNING - oscfilterpref is NULL\n");
	}
	if (!com.pref.spcc.redpref) {
		siril_log_message("populate_spcc_combos: WARNING - redpref is NULL\n");
	}
	if (!com.pref.spcc.greenpref) {
		siril_log_message("populate_spcc_combos: WARNING - greenpref is NULL\n");
	}
	if (!com.pref.spcc.bluepref) {
		siril_log_message("populate_spcc_combos: WARNING - bluepref is NULL\n");
	}
	if (!com.pref.spcc.lpfpref) {
		siril_log_message("populate_spcc_combos: WARNING - lpfpref is NULL\n");
	}
	if (!com.pref.spcc.monosensorpref) {
		siril_log_message("populate_spcc_combos: WARNING - monosensorpref is NULL\n");
	}
	if (!com.pref.spcc.oscsensorpref) {
		siril_log_message("populate_spcc_combos: WARNING - oscsensorpref is NULL\n");
	}

	// Initialize filters with error handling - each call is now safe due to validation in fill_combo_from_glist
	siril_log_message("populate_spcc_combos: populating OSC filters");
	fill_combo_from_glist(oscfilters, com.spcc_data.osc_filters, -1, com.pref.spcc.oscfilterpref);

	siril_log_message("populate_spcc_combos: populating red filters\n");
	fill_combo_from_glist(rfilters, com.spcc_data.mono_filters[RLAYER], RLAYER, com.pref.spcc.redpref);

	siril_log_message("populate_spcc_combos: populating green filters\n");
	fill_combo_from_glist(gfilters, com.spcc_data.mono_filters[GLAYER], GLAYER, com.pref.spcc.greenpref);

	siril_log_message("populate_spcc_combos: populating blue filters\n");
	fill_combo_from_glist(bfilters, com.spcc_data.mono_filters[BLAYER], BLAYER, com.pref.spcc.bluepref);

	siril_log_message("populate_spcc_combos: populating LP filters\n");
	fill_combo_from_glist(lpfilters, com.spcc_data.osc_lpf, -1, com.pref.spcc.lpfpref);

	siril_log_message("populate_spcc_combos: populating mono sensors\n");
	fill_combo_from_glist(monosensors, com.spcc_data.mono_sensors, -1, com.pref.spcc.monosensorpref);

	siril_log_message("populate_spcc_combos: populating OSC sensors\n");
	fill_combo_from_glist(oscsensors, com.spcc_data.osc_sensors, -1, com.pref.spcc.oscsensorpref);

	siril_log_message("populate_spcc_combos: populating white point\n");
	fill_combo_from_glist(whitepoint, com.spcc_data.wb_ref, -1, "Average Spiral Galaxy");

	siril_log_message("populate_spcc_combos: setting switch state\n");
	...
	siril_log_message("populate_spcc_combos: completed successfully\n");
	return FALSE;
}
