...
#pragma omp parallel for num_threads(com.max_thread) schedule(guided) shared(progress, ngood)
	...
	for (int i = 0; i < nb_stars; i++) {
		...
		for (int chan = 0; chan < 3 && !no_phot; chan ++) {
			psf_star *photometry = psf_get_minimisation(fit, chan, &area, FALSE, TRUE, ps, FALSE, GAUSSIAN, &error);
...
#endif
		psf_star **stars = peaker(&im, chan, &com.pref.starfinder_conf, &nb_stars, NULL, FALSE, TRUE, 200, GAUSSIAN, FALSE, nb_subthreads);
		if (stars) {
