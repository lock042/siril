...
/* Register-time per-AP frame cap: how many top-quality frames per AP get a
 * shift computed (Stage B does proportionally less correlation work for a
 * smaller value). Independent of the Stack tab's stack percent/frames; it
 * is the upper bound the stack selection can use. */
...
static void mpp_update_action_button_highlight(void) {
	...
	GtkStyleContext *actx = gtk_widget_get_style_context(analyze_btn);
	GtkStyleContext *rctx = gtk_widget_get_style_context(register_btn);
	if (analyze_suggested) {
		gtk_style_context_add_class(actx, "suggested-action");
		gtk_style_context_remove_class(rctx, "suggested-action");
	} else {
		gtk_style_context_remove_class(actx, "suggested-action");
		gtk_style_context_add_class(rctx, "suggested-action");
	}
}
