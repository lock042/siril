...
static siril_cat_index get_cat_index_from_combo() {
	...
	const gchar *cat_char = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(lookup_widget("conesearch_combo")));
...
void on_annotate_save_as_button_clicked(GtkButton *button, gpointer user_data) {
	...
	GtkWindow *annotate_dialog = GTK_WINDOW(lookup_widget("annotate_dialog"));
	gint res;
	gchar *filename;
...
static conesearch_params* parse_conesearch_ui() {
	...
	GtkEntry *save_entry = GTK_ENTRY(lookup_widget("conesearch_save_entry"));
	GtkEntry *obscode_entry = GTK_ENTRY(lookup_widget("annotate_obscode_entry"));
	...
	params->display_log = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget("conesearch_log"))) ? BOOL_TRUE : BOOL_FALSE;
	params->display_tag = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget("conesearch_tag"))) ? BOOL_TRUE : BOOL_FALSE;
	params->photometric = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget("conesearch_photometric")));
	params->limit_mag = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("conesearch_maglimit")));
    const gchar *output = gtk_entry_get_text(save_entry);

    if (output != NULL && strlen(output) != 0) {
        params->outfilename = g_strdup(output);
    }
    const gchar *obscode = gtk_entry_get_text(obscode_entry);
...
void on_annotate_apply_clicked(GtkButton *button, gpointer user_data) {
	GtkNotebook *notebook_annotate = GTK_NOTEBOOK(lookup_widget("notebook_annotate"));
	GtkEntry *entry = NULL;
	...
	case SEARCH_OBJECT_PAGE:
		entry = GTK_ENTRY(GTK_SEARCH_ENTRY(lookup_widget("search_objects_entry")));
		search_object(entry);
		break;
	}
