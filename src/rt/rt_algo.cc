...
// This is a modified version of findMinMaxPercentile which computes an array 
// of percentiles instead of just a low and high values
extern "C" int findPercentiles(const float* data, size_t size, float *Prct, float *minOut, int n, int threads);

int findPercentiles(const float* data, size_t size, float *Prct, float *Out, int n, int threads)
{
    // Copyright (c) 2017 Ingo Weyrich <heckflosse67@gmx.de>
    // We need to find the (minPrct*size) smallest value and the (maxPrct*size) smallest value in data.
    // We use a histogram based search for speed and to reduce memory usage.
    ...
    for (int i = 0; i < n; i++) {
        ...
        const float thresh = Prct[i] * size;
        while (count < thresh) {
            count += histo[k++];
        }
        ...
        *(Out + i) /= scale;
        *(Out + i) += minVal;
        *(Out + i) = rtengine::LIM(*(Out + i), minVal, maxVal);
    }
    return 0;
}
