...
#pragma omp for nowait
        for (int k = 0; k < opts.Ntries; k++) {
            ...
            if (scores[0] < currentScore) {
                ...
                /* The next line generates a warning, the compiler wants
                 * bestKernel to be declared using the copy constructor,
                 * however we can't as it must be defined outside the
                 * OMP for. The warning should be ignored. */
...
#pragma omp critical
        {
            if (currentScore < globalCurrentScore) {
                ...
                /* Similar to above, the copy constructor cannot be used
                 * here as outkernel is passed to the function as a
                 * reference, it can't be declared again. The warning
                 * should be ignored. */
