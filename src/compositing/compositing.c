...
static void update_metadata(gboolean do_cumul) {
	...
	merge_fits_headers_to_result2(&gfit, f, do_cumul);
	free(f);
}

// Called after alignment
static void update_comp_metadata(fits *fit, gboolean do_cumul) {
	int nb = number_of_images_loaded();
	fits **f = malloc((nb + 1) * sizeof(fits *));
	int j = 0;
	for (int i = 0; i < nb ; i++)
...
void on_button_align_clicked(GtkButton *button, gpointer user_data) {
	...
	gboolean do_cumul = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget("cumulate_rgb_button")));
	...
	// update WCS etc.
	update_comp_metadata(seq->internal_fits[seq->reference_image], do_cumul);
