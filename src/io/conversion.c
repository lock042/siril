		...
		if (imagetype == TYPEFITS && reader->allow_link) {
			...
			*retval = fit ? READ_OK : READ_FAILED;
			if (*retval == READ_OK) {
				...
				/** 48 is the maximum length taking into account total length of card, comment, keyword and spaces */
				copy_filename(g_filename_display_basename(reader->filename), fit->keywords.filename, 48);
			}
		}
	}
