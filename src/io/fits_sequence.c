...
static int _find_hdus(fitsfile *fptr, int **hdus, int *nb_im) {
	...
	if (hdus) {
		*hdus = siril_malloc(nb_hdu * sizeof(int));
		if (!*hdus) {
			PRINT_ALLOC_ERR;
			return 1;
		}
	...
	if (status) {
		...
		*nb_im = nb_images;
		...
		// we could siril_realloc *hdus, but it's not much useful
	}
	return status;
}
...
int fitseq_create_file(const char *filename, fitseq *fitseq, int frame_count) {
	...
	fitseq->writer = siril_malloc(sizeof(struct seqwriter_data));
...
static int fitseq_destroy(fitseq *fitseq, gboolean abort) {
	...
	if (fitseq->filename) {
		siril_free(fitseq->filename);
		fitseq->filename = NULL;
	}
	if (fitseq->hdu_index) {
		siril_free(fitseq->hdu_index);
		fitseq->hdu_index = NULL;
	}
	return retval;
}
...
static int fitseq_prepare_for_multiple_read(fitseq *fitseq, int iomode) {
	...
	fitseq->thread_fptr = siril_malloc(fitseq->num_threads * sizeof(fitsfile *));
