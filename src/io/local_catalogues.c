...
int siril_catalog_get_stars_from_local_catalogues(siril_catalogue *siril_cat) {
	...
	if (xp_cat == CAT_LOCAL_GAIA_XPSAMP) {
		...
		if (kind == LOCAL_GAIA_PHOTO_XPCTS) {
			...
			siril_log_color_message(_("Local Gaia photo dir contains both xpsamp and xpcts chunks; using xpsamp.\n"), "salmon");
		} else if (kind == LOCAL_GAIA_PHOTO_BAD) {
			siril_log_color_message(_("Local Gaia photo dir has unreadable headers.\n"), "red");
			return 0;
