...
int fit_stats(fitsfile *fptr, float *mini, float *maxi) {
	...
	*maxi = 0;
	...
	pix = siril_malloc(npixels * sizeof(float)); /* memory for 1 row */
	if (pix == NULL) {
		PRINT_ALLOC_ERR;
		return (1);
	}
...
GSList *read_header_keyvals_strings(fitsfile *fptr) {
	...
	for (int n = 1; n <= nkeys; n++) {
		...
		if (value[0] == '\0' && comment[0] == '\0')
			continue;
		header_record *r = siril_malloc(sizeof(header_record));
		if (!r) {
			PRINT_ALLOC_ERR;
			break;
		}
...
static int copy_header_from_hdu(fitsfile *fptr, char **header, int *strsize, int *strlength) {
	...
	for (int i = 1; i <= nkeys; i++) {
		...
		if (*strlength + cardlen + 1 >= *strsize) {
			*strsize += 567;
			newstr = siril_realloc(*header, *strsize);
			if (!newstr) {
				PRINT_ALLOC_ERR;
				siril_free(*header);
				*header = NULL;
	...
	if (*strlength + 3 + 1 >= *strsize) {
		*strsize += 4;
		char *newstr = siril_realloc(*header, *strsize);
		if (!newstr) {
			PRINT_ALLOC_ERR;
			siril_free(*header);
			*header = NULL;
...
char *copy_header(fits *fit) {
	...
	strsize = 1620;
	if (!(header = siril_malloc(strsize))) {
		PRINT_ALLOC_ERR;
		return NULL;
	}
	strlength = 0;
...
int read_fits_with_convert(fits* fit, const char* filename, gboolean force_float) {
	...
	switch (fake_bitpix) {
		...
		case USHORT_IMG:
			...
			if ((fit->data = siril_malloc(nbdata * sizeof(WORD))) == NULL) {
				PRINT_ALLOC_ERR;
				return -1;
			}
		...
		case FLOAT_IMG:		// 32-bit floating point pixels
			...
			if ((fit->fdata = siril_malloc(nbdata * sizeof(float))) == NULL) {
				PRINT_ALLOC_ERR;
				return -1;
			}
	...
	switch (fake_bitpix) {
	case BYTE_IMG:
		data8 = siril_malloc(nbdata * sizeof(BYTE));
	...
	case ULONG_IMG:		// 32-bit unsigned integer pixels
	case LONG_IMG:		// 32-bit signed integer pixels
		pixels_long = siril_malloc(nbdata * sizeof(unsigned long));
...
int internal_read_partial_fits(fitsfile *fptr, unsigned int ry,
	...
	switch (bitpix) {
		...
		case ULONG_IMG:		// 32-bit unsigned integer pixels
		case LONG_IMG:		// 32-bit signed integer pixels
			pixels_long = siril_malloc(nbdata * sizeof(long));
			status = 0;
...
int write_icc_profile_to_fptr(fitsfile *fptr, cmsHPROFILE icc_profile) {
	...
	if (profile_length > 0) {
		profile = siril_malloc(profile_length * sizeof(cmsUInt8Number));
...
cmsHPROFILE read_icc_profile_from_fptr(fitsfile *fptr) {
	...
	int zero = 0;
	BYTE *profile = NULL;
	if (!(profile = siril_malloc(profile_length * sizeof(BYTE)))) {
		PRINT_ALLOC_ERR;
		siril_free(header);
		fits_movabs_hdu(fptr, orig_hdu, &hdutype, &status);
		if (status)
...
int readfits_partial(const char *filename, int layer, fits *fit,
	...
	if (fit->type == DATA_USHORT) {
		/* siril_realloc fit->data to the image size */
		...
		/* siril_realloc fit->fdata to the image size */
		float *olddata = fit->fdata;
		if ((fit->fdata = siril_realloc(fit->fdata, nbdata * sizeof(float))) == NULL) {
			PRINT_ALLOC_ERR;
			status = 0;
			fits_close_file(fit->fptr, &status);
			if (olddata)
...
int readfits_partial_all_layers(const char *filename, fits *fit, const rectangle *area) {
	...
	if (fit->type == DATA_USHORT) {
		/* siril_realloc fit->data to the image size */
		WORD *olddata = fit->data;
		if ((fit->data = siril_realloc(fit->data, nblayer * nbdata * sizeof(WORD))) == NULL) {
			PRINT_ALLOC_ERR;
			status = 0;
			fits_close_file(fit->fptr, &status);
			if (olddata)
		...
		/* siril_realloc fit->fdata to the image size */
		float *olddata = fit->fdata;
		if ((fit->fdata = siril_realloc(fit->fdata, nblayer * nbdata * sizeof(float))) == NULL) {
			PRINT_ALLOC_ERR;
			status = 0;
			fits_close_file(fit->fptr, &status);
			if (olddata)
...
void flip_buffer(int bitpix, void *buffer, const rectangle *area) {
	...
	if (get_data_type(bitpix) == DATA_FLOAT) {
		...
		void *swap = siril_malloc(line_size);
...
int save_opened_fits(fits *f) {
	...
	switch (f->bitpix) {
	case BYTE_IMG:
		data8 = siril_malloc(pixel_count * sizeof(BYTE));
	...
	case FLOAT_IMG:
		if (f->type == DATA_USHORT) {
			...
			f->fdata = siril_malloc(pixel_count * sizeof(float));
...
int copyfits(fits *from, fits *to, unsigned char oper, int layer) {
	...
	if ((oper & CP_FORMAT)) {
		// siril_free anything that might need deallocating in to
		clearfits(to);
	...
	if ((oper & CP_ALLOC)) {
		...
		if (from->type == DATA_USHORT) {
			...
			if (!(to->data = siril_realloc(to->data, nbdata * depth * sizeof(WORD)))) {
				PRINT_ALLOC_ERR;
				if (olddata)
					siril_free(olddata);
				return -1;
			...
			if (!(to->fdata = siril_realloc(to->fdata, nbdata * depth * sizeof(float)))) {
				PRINT_ALLOC_ERR;
				if (olddata)
					siril_free(olddata);
				return -1;
...
int fits_change_depth(fits *fit, int layers) {
	...
	if (fit->type == DATA_USHORT) {
		WORD *tmp;
		if (!(tmp = siril_realloc(fit->data, nbdata * layers * sizeof(WORD)))) {
			PRINT_ALLOC_ERR;
			return -1;
		}
		fit->data = tmp;
		...
		float *tmp;
		if (!(tmp = siril_realloc(fit->fdata, nbdata * layers * sizeof(float)))) {
			PRINT_ALLOC_ERR;
			return -1;
		}
		fit->fdata = tmp;
...
int extract_fits(fits *from, fits *to, int channel, gboolean to_float) {
	...
	if (from->type == DATA_USHORT)
		if (to_float) {
			...
			to->fdata = siril_malloc(nbdata * sizeof(float));
			if (!to->fdata) {
				PRINT_ALLOC_ERR;
				return -1;
			}
			...
			to->data = siril_malloc(nbdata * sizeof(WORD));
			if (!to->data) {
				PRINT_ALLOC_ERR;
				return -1;
			}
...
void keep_only_first_channel(fits *fit) {
	...
	size_t nbdata = fit->naxes[0] * fit->naxes[1];
	if (fit->type == DATA_USHORT) {
		fit->data = siril_realloc(fit->data, nbdata * sizeof(WORD));
		fit->pdata[1] = fit->data;
		fit->pdata[2] = fit->data;
	} else if (fit->type == DATA_FLOAT) {
		fit->fdata = siril_realloc(fit->fdata, nbdata * sizeof(float));
...
static void extract_region_from_fits_ushort(fits *from, int layer, fits *to,
	...
	clearfits(to);
	to->data = siril_malloc(area->w * area->h * sizeof(WORD));
...
static void extract_region_from_fits_float(fits *from, int layer, fits *to,
	...
	int x, y, d, ystart, yend;
	clearfits(to);
	to->fdata = siril_malloc(area->w * area->h * sizeof(float));
...
int new_fit_image_with_data(fits **fit, int width, int height, int nblayer, data_type type, void *data) {
	...
	if (!data) {
		data = siril_calloc(npixels * nblayer, data_size);
		if (!data) {
			PRINT_ALLOC_ERR;
			return -1;
		}
...
/* use for type change, it doesn't siril_free old data if it's the same type */
...
#define TRYFITS(f, ...) \
	do{ \
		...
		if(status){ \
			if(ima_data) siril_free(ima_data); \
...
GdkPixbuf* get_thumbnail_from_fits(char *filename, gchar **descr) {
	...
	ima_data = siril_malloc(sz * sizeof(float));
	if (!ima_data) {
		fits_close_file(fp, &status);
		return NULL;
	}
	...
	float *preview_data = siril_malloc(prev_size * n_channels * sizeof(float));
	if (!preview_data) {
		siril_free(ima_data);
		fits_close_file(fp, &status);
		g_free(description);
...
#pragma omp parallel for num_threads(num_threads) if (num_threads > 1)
	...
	guchar *pixbuf_data = siril_malloc(3 * prev_size * sizeof(guchar));
...
void merge_fits_headers_to_result(fits *result, gboolean do_sum, fits *f1, ...) {
	...
	while ((current = va_arg(ap, fits *)))
		nb_fits++;
	va_end(ap);
	fits **array = siril_malloc((nb_fits + 1) * sizeof(fits *));
