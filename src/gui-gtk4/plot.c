...
/* Re-entry guards for the three dropdown handlers in this file.
 * In GTK4 the dropdown's `notify::selected` can be raised as a SIDE
 * EFFECT of validate_combos clearing / repopulating the dropdown's
 * GtkStringList model — the items-changed cascade reaches the
 * GtkSingleSelection inside the GtkDropDown, which updates its selected
 * position and emits `notify::selected` on the dropdown.
 * `g_signal_handlers_block_by_func` was supposed to suppress that
 * re-entrance but does not reliably match the builder-installed
 * handler in this build (the recursion was confirmed with a backtrace).
 * These guards short-circuit the user-facing change handler whenever
 * it's triggered re-entrantly from validate_combos. */
 ...
 * "menu_plot") containing a GtkBox with three GtkButtons.  We keep the
 * pointers as static caches resolved in fill_plot_statics(). */
static GtkPopover *menu_plot_popover = NULL;
static GtkWidget *menu_item1 = NULL, *menu_item2 = NULL, *menu_item3 = NULL;

/* Forward decls so fill_plot_statics() can reference symbols defined
 * further down the file. */
void drawing_the_graph(GtkWidget *widget, cairo_t *cr);
void on_menu_plot_item1_activate(GtkWidget *menuitem, gpointer user_data);
void on_menu_plot_item2_activate(GtkWidget *menuitem, gpointer user_data);
void on_menu_plot_item3_activate(GtkWidget *menuitem, gpointer user_data);
static void on_DrawingPlot_motion(GtkEventControllerMotion *controller,
		double x, double y, gpointer user_data);
static void on_DrawingPlot_enter_notify_event(GtkEventControllerMotion *controller,
		double x, double y, gpointer user_data);
static void on_DrawingPlot_leave_notify_event(GtkEventControllerMotion *controller,
		gpointer user_data);
static void on_DrawingPlot_pressed(GtkGestureClick *gesture, int n_press,
		double x, double y, gpointer user_data);
...
static void plot_draw_cb(GtkDrawingArea *area, cairo_t *cr,
	...
	drawing_the_graph(GTK_WIDGET(area), cr);
}
...
static GtkDropDown *plot_obstype_combo = NULL, *plot_filter_combo = NULL;
static GtkDropDown *plot_cstar_combo = NULL, *plot_kstar_combo = NULL;
...
void on_JulianPhotometry_toggled(GtkCheckButton *button, gpointer user_data);
void on_plotCombo_changed(GObject *obj, GParamSpec *pspec, gpointer user_data);
void on_plotComboX_changed(GObject *obj, GParamSpec *pspec, gpointer user_data);
...
/* GTK3-era set_sensitive() greyed-out invalid items in the X/Y dropdowns
 * by hooking gtk_cell_layout_set_cell_data_func.  After Phase 10 the
 * widgets are GtkDropDown, which is not a GtkCellLayout — the calls are
 * gone; greyed-out filtering will be reintroduced in a later pass via a
 * GtkSignalListItemFactory bound to the use_photometry/is_fwhm state.
 * Until then the user can still pick any entry from the populated list;
 * the existing validate_combos() logic only appends meaningful choices,
 * so no invalid selection paths are reachable. */

static void fill_plot_statics() {
	if (drawingPlot == NULL) {
		drawingPlot = GTK_WIDGET(gtk_builder_get_object(gui.builder, "DrawingPlot"));

		/* GTK4: wire the draw_func + event controllers on DrawingPlot.
		 * Replaces the GTK3 "draw", "motion-notify-event",
		 * "button-press-event", "button-release-event",
		 * "enter-notify-event", "leave-notify-event" signals. */
		gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(drawingPlot),
		    plot_draw_cb, NULL, NULL);

		GtkEventController *plot_motion = gtk_event_controller_motion_new();
		g_signal_connect(plot_motion, "motion", G_CALLBACK(on_DrawingPlot_motion), NULL);
		g_signal_connect(plot_motion, "enter",
		    G_CALLBACK(on_DrawingPlot_enter_notify_event), NULL);
		g_signal_connect(plot_motion, "leave",
		    G_CALLBACK(on_DrawingPlot_leave_notify_event), NULL);
		gtk_widget_add_controller(drawingPlot, plot_motion);

		GtkGesture *plot_click = gtk_gesture_click_new();
		gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(plot_click), 0); /* any */
		g_signal_connect(plot_click, "pressed",  G_CALLBACK(on_DrawingPlot_pressed),  NULL);
		...
		GtkWidget *mi2 = GTK_WIDGET(gtk_builder_get_object(gui.builder, "menu_plot_item2"));
		GtkWidget *mi3 = GTK_WIDGET(gtk_builder_get_object(gui.builder, "menu_plot_item3"));
		if (mi1) g_signal_connect(mi1, "clicked", G_CALLBACK(on_menu_plot_item1_activate), NULL);
		if (mi2) g_signal_connect(mi2, "clicked", G_CALLBACK(on_menu_plot_item2_activate), NULL);
		if (mi3) g_signal_connect(mi3, "clicked", G_CALLBACK(on_menu_plot_item3_activate), NULL);
		...
		plot_obstype_combo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "obstype_combo"));
		...
		plot_filter_combo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "aavso_filter_combo"));
		...
		plot_cstar_combo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "cstar_combo"));
		plot_kstar_combo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "kstar_combo"));
...
static void validate_combos() {
	fill_plot_statics();
	use_photometry = gtk_drop_down_get_selected(GTK_DROP_DOWN(sourceCombo));
	if (!use_photometry) {
		reglayer = gtk_drop_down_get_selected(GTK_DROP_DOWN(layer_selector));
...
void on_plotSourceCombo_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)obj;
	(void)pspec;
	if (in_plotSourceCombo_changed) return;
	in_plotSourceCombo_changed = TRUE;
	requires_seqlist_update = TRUE;
	reset_plot_zoom();
	drawPlot();
	validate_combos();
	in_plotSourceCombo_changed = FALSE;
}
...
void reset_plot() {
	...
	if (sourceCombo) {
		gtk_drop_down_set_selected(GTK_DROP_DOWN(sourceCombo), 0);
		gtk_drop_down_set_selected(GTK_DROP_DOWN(combo), registration_selected_source); //remove?
		gtk_drop_down_set_selected(GTK_DROP_DOWN(comboX), r_FRAME);
		gtk_widget_set_sensitive(comboX, FALSE);
...
void drawPlot() {
	...
	int current_selected_source = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo));
	if (!arcsec_is_ok) {
		siril_toggle_set_active(GTK_WIDGET(arcsec), FALSE);
		is_arcsec = FALSE;
	}
...
void on_button_aavso_apply_clicked(GtkButton *button, gpointer user_data) {
	...
	fill_plot_statics();
	aavso_ptr->obscode = gtk_editable_get_text(GTK_EDITABLE(plot_obscode_entry));
	aavso_ptr->obstype = siril_drop_down_get_active_text(plot_obstype_combo);
	aavso_ptr->starid = gtk_editable_get_text(GTK_EDITABLE(plot_starid_entry));
	aavso_ptr->cname = gtk_editable_get_text(GTK_EDITABLE(plot_cname_entry));
	aavso_ptr->filter = siril_drop_down_get_active_text(plot_filter_combo);
	aavso_ptr->kname = gtk_editable_get_text(GTK_EDITABLE(plot_kname_entry));
	aavso_ptr->c_std = g_ascii_strtod(gtk_editable_get_text(GTK_EDITABLE(plot_cstd_entry)), NULL);
	aavso_ptr->c_idx = gtk_drop_down_get_selected(GTK_DROP_DOWN(plot_cstar_combo));
	aavso_ptr->k_idx = gtk_drop_down_get_selected(GTK_DROP_DOWN(plot_kstar_combo));
	aavso_ptr->chart = gtk_editable_get_text(GTK_EDITABLE(plot_chart_entry));
	aavso_ptr->notes = gtk_editable_get_text(GTK_EDITABLE(plot_notes_entry));
...
static void fill_combo_boxes() {
	fill_plot_statics();
	siril_drop_down_clear_strings(plot_cstar_combo);
	siril_drop_down_clear_strings(plot_kstar_combo);
	int n = get_number_of_stars(&com.seq);

	for (int i = 1; i < n; i++) {
		gchar *txt = g_strdup_printf("%d", i);
		siril_drop_down_append_text(plot_cstar_combo, txt);
		siril_drop_down_append_text(plot_kstar_combo, txt);
...
void on_exportAAVSO_button_clicked(GtkButton *button, gpointer user_data) {
	fill_combo_boxes();
	gtk_widget_set_visible(plot_aavso_dialog, TRUE);
}
...
void on_plotCombo_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)obj;
	(void)pspec;
	if (in_plotCombo_changed) return;
	in_plotCombo_changed = TRUE;
	if (use_photometry) {
		photometry_selected_source = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo));
	} else {
		registration_selected_source = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo));
	}
	requires_seqlist_update = TRUE;
	pdd.selection = (rectangled){0., 0., 0., 0.};
	update_slider(SLIDER_Y, 0., 1.);
	in_plotCombo_changed = FALSE;
}

void on_plotComboX_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)obj;
	(void)pspec;
	if (in_plotComboX_changed) return;
	in_plotComboX_changed = TRUE;
	X_selected_source = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboX));
	pdd.selection = (rectangled){0., 0., 0., 0.};
	update_slider(SLIDER_X, 0., 1.);
	in_plotComboX_changed = FALSE;
}

void on_arcsecPhotometry_toggled(GtkCheckButton *button, gpointer user_data) {
	requires_seqlist_update = TRUE;
	is_arcsec = siril_toggle_get_active(GTK_WIDGET(button));
	drawPlot();
}

void on_JulianPhotometry_toggled(GtkCheckButton *button, gpointer user_data) {
	force_Julian = siril_toggle_get_active(GTK_WIDGET(button));
	drawPlot();
}
...
void notify_new_photometry() {
	...
	gtk_drop_down_set_selected(GTK_DROP_DOWN(sourceCombo), 1);
...
/* GTK4 motion handler: drives selection drag, slider drag, marker
...
static void on_DrawingPlot_motion(GtkEventControllerMotion *controller,
	...
	if (pdd.action == SELACTION_SELECTING) {
		double x1 = max(SIDE_MARGIN, min(pdd.start.x, x));
		double x2 = min(pdd.surf_w - PLOT_SLIDER_THICKNESS, max(pdd.start.x, x));
		double y1 = max(SIDE_MARGIN, min(pdd.start.y, y));
		double y2 = min(pdd.surf_h - PLOT_SLIDER_THICKNESS, max(pdd.start.y, y));
		pdd.selection = (rectangled){x1, y1, x2 - x1, y2 - y1};
		drawPlot();
		return;
	}
	if (pdd.action == SELACTION_RESIZING) {
		double x1 = pdd.selection.x;
		double y1 = pdd.selection.y;
		double x2 = pdd.selection.x + pdd.selection.w;
		double y2 = pdd.selection.y + pdd.selection.h;
		switch (pdd.border_grabbed) {
			case SELBORDER_LEFT:
				if (x <= pdd.selection.x + pdd.selection.w) {
					x1 = x; x2 = pdd.selection.x + pdd.selection.w;
				} else {
					x2 = x; x1 = pdd.selection.x + pdd.selection.w;
					pdd.border_grabbed = SELBORDER_RIGHT;
				}
				break;
			case SELBORDER_RIGHT:
				if (x >= pdd.selection.x) {
					x1 = pdd.selection.x; x2 = x;
				} else {
					x1 = x; x2 = pdd.selection.x;
					pdd.border_grabbed = SELBORDER_LEFT;
				}
				break;
			case SELBORDER_TOP:
				if (y <= pdd.selection.y + pdd.selection.h) {
					y1 = y; y2 = pdd.selection.y + pdd.selection.h;
				} else {
					y2 = y; y1 = pdd.selection.y + pdd.selection.h;
					pdd.border_grabbed = SELBORDER_BOTTOM;
				}
				break;
			case SELBORDER_BOTTOM:
				if (y >= pdd.selection.y) {
					y1 = pdd.selection.y; y2 = y;
				} else {
					y1 = y; y2 = pdd.selection.y;
	...
	if (pdd.action == SELACTION_MOVING) {
		double dx = x - pdd.start.x;
		double dy = y - pdd.start.y;
		...
		pdd.start.x = x;
		pdd.start.y = y;
		pdd.selection = (rectangled){pdd.selection.x + dx, pdd.selection.y + dy,
		                              pdd.selection.w, pdd.selection.h};
		drawPlot();
		return;
	}
		...
		for (int j = 0; j < 2; j++) {
			if (pdd.marker_grabbed == 2 * i + j) {
				...
				if (i == SLIDER_X) {
					...
					dv = pdd.start.y - y;
					v1 = (1. - pdd.yrange[0]) * pdd.range.y + pdd.offset.y;
	...
	if (border > SELBORDER_NONE) {
		set_cursor((border <= SELBORDER_BOTTOM) ? "n-resize" : "w-resize");
		return;
	...
	if (is_inside_grid(x, y, &pdd)) {
		...
		if (get_index_of_frame(x, y, FALSE, &index, &xpos, &ypos)) {
			gchar *tooltip_text;
			if (index > 0)
...
/* GTK4: GtkEventControllerMotion's "enter" / "leave" signals use
 * different signatures from the GTK3 enter-notify/leave-notify-event
 * callbacks they replace.  Bodies are unchanged. */
static void on_DrawingPlot_enter_notify_event(GtkEventControllerMotion *controller,
		double x, double y, gpointer user_data) {
...
static void on_DrawingPlot_leave_notify_event(GtkEventControllerMotion *controller,
	...
	if (processing_is_job_active()) {
		...
		if (pdd.marker_grabbed == MARKER_NONE
		    && pdd.border_grabbed == SELBORDER_NONE
		    && pdd.slider_grabbed == SLIDER_NONE
		    && pdd.action == SELACTION_NONE)
			set_cursor_waiting(FALSE);
...
/* Lazy resolve of the menu_plot popover (defined in siril.ui).  All three
 * buttons exist permanently; we toggle item3's visibility instead of the
 * GTK3 unparent/reparent dance, and read labels via gtk_button_set_label.
 * The "clicked" signals are connected in fill_plot_statics(). */
static void plot_menu_resolve(GtkWidget *anchor) {
	if (menu_plot_popover) return;
	menu_plot_popover = GTK_POPOVER(gtk_builder_get_object(gui.builder, "menu_plot"));
	menu_item1 = GTK_WIDGET(gtk_builder_get_object(gui.builder, "menu_plot_item1"));
	menu_item2 = GTK_WIDGET(gtk_builder_get_object(gui.builder, "menu_plot_item2"));
	menu_item3 = GTK_WIDGET(gtk_builder_get_object(gui.builder, "menu_plot_item3"));
	if (menu_plot_popover && anchor) {
		GtkWidget *cur = gtk_widget_get_parent(GTK_WIDGET(menu_plot_popover));
		if (cur != anchor) {
			if (cur) gtk_widget_unparent(GTK_WIDGET(menu_plot_popover));
			gtk_widget_set_parent(GTK_WIDGET(menu_plot_popover), anchor);
		}
	}
}

/* Right-click on a frame in the plot: pop a menu offering "Exclude Frame
 * N" / "Show Frame N".  Item3 is hidden in this mode. */
static void do_popup_singleframemenu(GtkWidget *my_widget, double ev_x, double ev_y) {
	double index, xpos, ypos;
	if (!get_index_of_frame(ev_x, ev_y, TRUE, &index, &xpos, &ypos)) return;
	if (index < 0) return;

	plot_menu_resolve(my_widget);
	if (!menu_plot_popover) return;

	gchar *s1 = g_strdup_printf(_("Exclude Frame %d"), (int)index);
	gchar *s2 = g_strdup_printf(_("Show Frame %d"),    (int)index);
	gtk_button_set_label(GTK_BUTTON(menu_item1), s1);
	gtk_button_set_label(GTK_BUTTON(menu_item2), s2);
	gtk_widget_set_visible(menu_item3, FALSE);
	g_free(s1);
	g_free(s2);

	GdkRectangle rect = { (int)ev_x, (int)ev_y, 1, 1 };
	gtk_popover_set_pointing_to(menu_plot_popover, &rect);
	gtk_popover_popup(menu_plot_popover);
}

/* Right-click with an active selection: pop the same menu but offering
 * the bulk operations.  Item3 is shown in this mode. */
static void do_popup_selectionmenu(GtkWidget *my_widget, double ev_x, double ev_y) {
	plot_menu_resolve(my_widget);
	if (!menu_plot_popover) return;

	gtk_button_set_label(GTK_BUTTON(menu_item1), _("Zoom to selection"));
	gtk_button_set_label(GTK_BUTTON(menu_item2), _("Only keep points within selection"));
	gtk_button_set_label(GTK_BUTTON(menu_item3), _("Exclude selected points"));
	gtk_widget_set_visible(menu_item3, TRUE);
	gtk_widget_set_sensitive(menu_item3, TRUE);
...
/* GTK4 press: classify the click into one of the interaction modes
 * (selection draw / move / resize / slider drag / marker drag / popup
 * menu).  Replaces the GTK3 "button-press-event"; (x, y) and button
 * come from GtkGestureClick. */
static void on_DrawingPlot_pressed(GtkGestureClick *gesture, int n_press,
		double x, double y, gpointer user_data) {
	(void)user_data;
	if (!plot_data) return;
	...
	/* Right-click: bulk-action menu over a selection, or single-frame
	 * menu when clicking close to a data point. */
	if (button == GDK_BUTTON_SECONDARY) {
		if (selection_is_active()) {
			do_popup_selectionmenu(widget, x, y);
			gtk_gesture_set_state(GTK_GESTURE(gesture), GTK_EVENT_SEQUENCE_CLAIMED);
			return;
		}
		if (is_inside_grid(x, y, &pdd)) {
			do_popup_singleframemenu(widget, x, y);
			gtk_gesture_set_state(GTK_GESTURE(gesture), GTK_EVENT_SEQUENCE_CLAIMED);
			return;
		}
	}

	if (is_inside_selectable_zone(x, y) && button == GDK_BUTTON_PRIMARY) {
		enum border_type border = is_over_selection_border(x, y);
		if (n_press == 2) {
			reset_plot_zoom();
			return;
	...
	for (int i = SLIDER_X; i <= SLIDER_Y; i++) {
		if (is_inside_slider(x, y, i)) {
			if (button == GDK_BUTTON_PRIMARY) {
				...
				/* Otherwise just clicked once → snap nearest marker to
				 * the click position. */
				...
				set_cursor((i == SLIDER_X) ? "w-resize" : "n-resize");
...
/* GTK4 release: clear all in-progress drag state and request a redraw. */
static void on_DrawingPlot_released(GtkGestureClick *gesture, int n_press,
		double x, double y, gpointer user_data) {
	(void)gesture; (void)n_press; (void)x; (void)y; (void)user_data;
	if (plot_data) set_cursor("tcross");
	else           set_cursor_waiting(FALSE);
	pdd.marker_grabbed = MARKER_NONE;
	pdd.action = SELACTION_NONE;
	pdd.border_grabbed = SELBORDER_NONE;
	pdd.slider_grabbed = SLIDER_NONE;
...
/* Phase 15: gtk_menu_item_get_label() removed.  Until plot's context menu
 * is rebuilt with GMenuModel + GtkPopoverMenu, we look at the button
 * label directly (the "menu items" are now GtkButtons). */
static const gchar *menu_widget_label(GtkWidget *w) {
...
void on_menu_plot_item1_activate(GtkWidget *menuitem, gpointer user_data) {
	if (!selection_is_active()) { // Exclude single frame
		const gchar *label = menu_widget_label(menuitem);
		gint index;
