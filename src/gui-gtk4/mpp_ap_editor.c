/*
 * AP editor dialog handlers — GTK4 port.
 *
 * Edits the cached MPP run (com.mpp_run) in place. Dialog is non-modal so
 * the user can interact with the image. Auto-place + Clear mutate run->aps
 * via mpp_ap_replace / mpp_ap_clear_all; mouse interactivity (left=add,
 * right=remove, drag=move) lives in mouse_action_functions.c.
 *
 ...
 * closes, until the user reopens the editor and unchecks. GtkCheckButton
 * in GTK4 no longer derives from GtkToggleButton, so its active state is
 * read through the siril_toggle_* helpers. */
static GtkWidget *check_show_patches = NULL;
 ...
 * Connected via GtkEventControllerKey to both the editor dialog and the
 * control window so it fires whichever has keyboard focus (the user
 * typically mouses the image, i.e. the control window). Inert unless the
 * editor is open, and never steals the keys from a focused entry/spin-button
 * so numeric fields stay usable. */
static gboolean editor_key_pressed(GtkEventControllerKey *ctrl, guint keyval,
	...
	GtkWidget *focus = GTK_IS_WINDOW(root) ? gtk_window_get_focus(GTK_WINDOW(root)) : NULL;
...
static void editor_init_statics(void) {
	...
	check_show_patches = GTK_WIDGET     (gtk_builder_get_object(gui.builder, "check_mpp_ap_editor_show_patches"));
	/* Resize-by-key: connect once via GtkEventControllerKey. The handler
	 * self-gates on mpp_ap_editor_is_open() so it's inert when closed. */
	if (dialog) {
		/* No default widget, so Enter never activates a button — the key
		 * handler uses Enter to deselect a selected AP, and with no
		 * selection Enter should simply do nothing. */
		gtk_window_set_default_widget(GTK_WINDOW(dialog), NULL);
		GtkEventController *kc = gtk_event_controller_key_new();
		g_signal_connect(kc, "key-pressed", G_CALLBACK(editor_key_pressed), NULL);
		gtk_widget_add_controller(dialog, kc);
	}
	GtkWidget *ctrl = GTK_WIDGET(gtk_builder_get_object(gui.builder, "control_window"));
	if (ctrl) {
		GtkEventController *kc = gtk_event_controller_key_new();
		g_signal_connect(kc, "key-pressed", G_CALLBACK(editor_key_pressed), NULL);
		gtk_widget_add_controller(ctrl, kc);
	}
}
...
gboolean mpp_ap_editor_show_patches(void) {
	if (!check_show_patches) {
		...
		check_show_patches = GTK_WIDGET(
		    gtk_builder_get_object(gui.builder, "check_mpp_ap_editor_show_patches"));
		if (!check_show_patches) return FALSE;
	}
	return siril_toggle_get_active(check_show_patches);
}

void on_mpp_ap_editor_show_patches_toggled(GtkCheckButton *toggle, gpointer user_data) {
...
/* GtkWindow "close-request" -> revert + hide. Closing the window with the
 * X is semantically a Cancel — discard edits made since the dialog was
 * opened. Returning TRUE stops the default close so the toplevel isn't
 * destroyed (we only hide it). */
gboolean on_mpp_ap_editor_dialog_close_request(GtkWindow *window, gpointer user_data) {
	(void) user_data;
	mpp_run_t *run = mpp_get_cached_run();
	...
	return TRUE;   /* swallow the close so the toplevel isn't destroyed */
}
