...
static GtkCheckButton *sf_toggle_A = NULL, *sf_toggle_R = NULL, *sf_toggle_checks = NULL;
static GtkDropDown *sf_combo_profile = NULL;
...
static void star_finder_init_statics(void) {
	...
	sf_toggle_A = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "psf_amplitude_range_check_button"));
	sf_toggle_R = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "psf_roundness_range_check_button"));
	sf_toggle_checks = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_relax_checks"));
	sf_combo_profile = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "combostarfinder_profile"));
	sf_beta_label = GTK_WIDGET(gtk_builder_get_object(gui.builder, "beta_control_label"));
}

void on_toggle_relax_checks_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	com.pref.starfinder_conf.relax_checks = siril_toggle_get_active(GTK_WIDGET(togglebutton));
}
...
void on_spin_sf_roundness_changed(GtkSpinButton *spinbutton, gpointer user_data) {
	...
	gboolean range = siril_toggle_get_active(GTK_WIDGET(sf_toggle_R));
...
void on_psf_amplitude_range_check_button_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	...
	gboolean enabled = siril_toggle_get_active(GTK_WIDGET(togglebutton));
	if (enabled) {
...
void on_combostarfinder_profile_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	...
	com.pref.starfinder_conf.profile = gtk_drop_down_get_selected(combo);
...
void update_peaker_GUI() {
	...
	siril_toggle_set_active(GTK_WIDGET(sf_toggle_checks), com.pref.starfinder_conf.relax_checks);
	gtk_drop_down_set_selected(sf_combo_profile, com.pref.starfinder_conf.profile);
	if (com.pref.starfinder_conf.max_r == 1.0) {
		siril_toggle_set_active(GTK_WIDGET(sf_toggle_R), FALSE);
	...
	if (com.pref.starfinder_conf.min_A == 0.0 && com.pref.starfinder_conf.max_A == 0.0) {
		siril_toggle_set_active(GTK_WIDGET(sf_toggle_A), FALSE);
		...
		siril_toggle_set_active(GTK_WIDGET(sf_toggle_A), TRUE);
