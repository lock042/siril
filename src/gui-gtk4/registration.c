...
static GtkDropDown *comboboxregmethod = NULL, *comboboxreglayer = NULL, *comboreg_maxstars = NULL, *comboreg_transfo = NULL, *reg_sel_all_combobox = NULL, *combofilter4 = NULL, *filter_type4 = NULL, *combofilter5 = NULL, *filter_type5 = NULL, *combofilter6 = NULL, *filter_type6 = NULL, *comboreg_framing = NULL, *ComboBoxRegInter = NULL, *combo_driz_kernel = NULL, *comboreg_undistort = NULL;
...
/* Forward decl: defined further down; needed by load_all_ips_statics(). */
static void on_interp_drizzle_stack_visible_child_changed(GObject *obj,
		GParamSpec *pspec, gpointer user_data);
static GtkCheckButton *checkStarSelect = NULL, *reg_2pass = NULL, *followStarCheckButton = NULL, *onlyshift_checkbutton = NULL, *toggle_reg_clamp = NULL, *driz_use_flats = NULL, *checkbutton_displayref = NULL, *reg_reference_checkbutton = NULL;
static GtkToggleButton *toggle_reg_manual1 = NULL, *toggle_reg_manual2 = NULL;
...
static void registration_init_statics() {
	if (register_minpairs == NULL) {
		...
		comboboxreglayer = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "comboboxreglayer"));
		comboreg_maxstars = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "comboreg_maxstars"));
		comboreg_transfo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "comboreg_transfo"));
		reg_sel_all_combobox = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "reg_sel_all_combobox"));
		combofilter4 = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "combofilter4"));
		filter_type4 = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "filter_type4"));
		combofilter5 = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "combofilter5"));
		filter_type5 = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "filter_type5"));
		combofilter6 = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "combofilter6"));
		filter_type6 = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "filter_type6"));
		comboreg_framing = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "comboreg_framing"));
		ComboBoxRegInter = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "ComboBoxRegInter"));
		combo_driz_kernel = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "combo_driz_kernel"));
		comboreg_undistort = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "comboreg_undistort"));
		// GtkEntry
		...
		// GtkToggleButton
		checkStarSelect = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "checkStarSelect"));
		reg_2pass = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "reg_2pass"));
		followStarCheckButton = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "followStarCheckButton"));
		onlyshift_checkbutton = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "onlyshift_checkbutton"));
		toggle_reg_clamp = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_reg_clamp"));
		driz_use_flats = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "driz_use_flats"));
		checkbutton_displayref = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "checkbutton_displayref"));
		reg_reference_checkbutton = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "reg_reference_checkbutton"));
		...
		filter_combo[0] = GTK_DROP_DOWN(combofilter4);
		filter_combo[1] = GTK_DROP_DOWN(combofilter5);
		filter_combo[2] = GTK_DROP_DOWN(combofilter6);
		...
		switcher_buttons = widget_list_children(GTK_WIDGET(interp_drizzle_stack_switcher));

		/* GTK4: watch the GtkStack's visible-child instead of clicks on
		 * the switcher — this fires for every page change (keyboard
		 * navigation, programmatic, click). */
		g_signal_connect(interp_drizzle_stack, "notify::visible-child",
		                 G_CALLBACK(on_interp_drizzle_stack_visible_child_changed),
		                 NULL);
...
void initialize_registration_methods() {
	...
	siril_toggle_set_active(GTK_WIDGET(toggle_reg_clamp), com.pref.gui.reg_clamping);
...
void on_reg_2pass_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	gboolean toggled = siril_toggle_get_active(GTK_WIDGET(togglebutton));
	GtkWidget *outputframe = GTK_WIDGET(output_reg_frame);
	gtk_widget_set_sensitive(outputframe, !toggled);
	update_reg_interface(TRUE);
}

void on_regfollowStar_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	update_reg_interface(TRUE);
}

void on_comboboxregmethod_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *box = GTK_DROP_DOWN(obj);
	(void)pspec;
	int index = 0;
	gchar *text = siril_drop_down_get_active_text(GTK_DROP_DOWN(box));
...
void on_ComboBoxRegInter_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *box = GTK_DROP_DOWN(obj);
	(void)pspec;
	com.pref.gui.reg_interpolation = gtk_drop_down_get_selected(box);
	gtk_widget_set_sensitive(GTK_WIDGET(toggle_reg_clamp),
			com.pref.gui.reg_interpolation == OPENCV_LANCZOS4
					|| com.pref.gui.reg_interpolation == OPENCV_CUBIC);
}

void on_comboreg_transfo_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *box = GTK_DROP_DOWN(obj);
	(void)pspec;
...
void on_comboreg_sel_all_combobox_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)obj;
	(void)pspec;
	update_reg_interface(TRUE);
}

void on_comboreg_framing_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *box = GTK_DROP_DOWN(obj);
	(void)pspec;
	gchar *name;
	int i = gtk_drop_down_get_selected(box);
...
void on_comboreg_undistort_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *box = GTK_DROP_DOWN(obj);
	(void)pspec;
	update_reg_interface(TRUE);
...
void on_reg_reference_checkbutton_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	registration_init_statics();
	gboolean active = gtk_check_button_get_active(togglebutton);
...
void on_reg_reference_button_clicked(GtkButton *button, gpointer user_data) {
	...
	siril_fc_set_current_folder_path(fc, com.wd);
	siril_fc_set_select_multiple(fc, FALSE);
	siril_fc_add_filter_pattern(fc, _("FITS Files (*.fit, *.fits, *.fts)"),
			"*.fit;*.FIT;*.fits;*.FITS;*.fts;*.FTS;*.fit.fz;*.FIT.fz;*.fits.fz;*.FITS.fz;*.fts.fz;*.FTS.fz",
			gui.file_ext_filter == TYPEFITS);
	if (siril_fc_run(fc) == GTK_RESPONSE_ACCEPT) {
		gchar *file = siril_fc_get_filename(fc);
		if (file) {
			gtk_editable_set_text(GTK_EDITABLE(reg_reference_entry), file);
 ...
 * GtkStackSwitcher (the GTK3 approach), watch the GtkStack's
 * "notify::visible-child" property — that way every path that swaps
 * pages (keyboard, programmatic, click) refreshes the registration UI.
 * Wired in load_all_ips_statics() below. */
static void on_interp_drizzle_stack_visible_child_changed(GObject *obj,
		GParamSpec *pspec, gpointer user_data) {
...
static void update_comet_entry(point pt, GtkEntry *entry_x, GtkEntry *entry_y) {
	...
	gtk_editable_set_text(GTK_EDITABLE(entry_x), txt_x);
	gtk_editable_set_text(GTK_EDITABLE(entry_y), txt_y);
	g_free(txt_x);
	g_free(txt_y);
}
...
void on_button_comet_clicked(GtkButton *button, gpointer p) {
	...
	int layer = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboboxreglayer));
...
void on_entry_comet_changed(GtkEditable *editable, gpointer user_data) {
	pos_of_image1.x = g_ascii_strtod(gtk_editable_get_text(GTK_EDITABLE(entry1_x_comet)), NULL);
	pos_of_image1.y = g_ascii_strtod(gtk_editable_get_text(GTK_EDITABLE(entry1_y_comet)), NULL);
	pos_of_image2.x = g_ascii_strtod(gtk_editable_get_text(GTK_EDITABLE(entry2_x_comet)), NULL);
	pos_of_image2.y = g_ascii_strtod(gtk_editable_get_text(GTK_EDITABLE(entry2_y_comet)), NULL);
	update_velocity();
}
...
void on_regsel_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
...
static gboolean check_applyreg(regmethod_index index) {
	if (index != REG_APPLY)
		return TRUE;
	framing_type framingmethod = (framing_type)gtk_drop_down_get_selected(GTK_DROP_DOWN(comboreg_framing));
...
static gboolean check_disto(disto_source index) {
	gchar *label = NULL;
	gchar *tooltip = NULL;
	const gchar *text = gtk_editable_get_text(GTK_EDITABLE(reg_wcsfile_entry));
...
static gboolean check_ext_ref(disto_source disto_index) {
	if (!gtk_check_button_get_active(reg_reference_checkbutton))
		return TRUE;
	const gchar *path = gtk_editable_get_text(GTK_EDITABLE(reg_reference_entry));
	if (!path || *path == '\0') {
...
struct registration_method *get_selected_registration_method(int *index) {
	*index = REG_UNDEF;
	int ind = 0;
	gchar *text = siril_drop_down_get_active_text(comboboxregmethod);
	...
	if (ind == REG_GLOBAL && siril_toggle_get_active(GTK_WIDGET(reg_2pass))) // 2 pass method is global + 2pass button checked
		ind = REG_2PASS;
	*index = ind;
...
void update_reg_interface(gboolean dont_change_reg_radio) {
	...
	if (!dont_change_reg_radio) {
		if (com.seq.selnum < com.seq.number) {
			gtk_drop_down_set_selected(GTK_DROP_DOWN(reg_sel_all_combobox), 1);
			gtk_drop_down_set_selected(GTK_DROP_DOWN(combofilter4), 1);
		} else
			gtk_drop_down_set_selected(GTK_DROP_DOWN(reg_sel_all_combobox), 0);
	}

	selection_is_done = (com.selection.w > 0 && com.selection.h > 0);

	/* number of registered image */
	nb_images_reg = gtk_drop_down_get_selected(GTK_DROP_DOWN(reg_sel_all_combobox)) == 0 ? com.seq.number : com.seq.selnum;
	...
	has_reg = layer_has_registration(&com.seq, gtk_drop_down_get_selected(GTK_DROP_DOWN(comboboxreglayer)));
	...
	if (isapplyreg) {
		if (!dont_change_reg_radio && com.seq.selnum < com.seq.number) {
			gtk_drop_down_set_selected(GTK_DROP_DOWN(combofilter4), SELECTED_IMAGES);
		}
		update_filters_registration(-1);
	}
	...
	if (is_star_align) {
		...
		if (!com.seq.is_variable) {
			disto_source_index = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboreg_undistort));
			gtk_widget_set_visible(GTK_WIDGET(reg_wcsfilechooser_box), disto_source_index == DISTO_FILE);
		} else {
			gtk_check_button_set_active(checkStarSelect, FALSE);
		}
	...
	if (!dont_change_reg_radio && com.seq.ext_ref && com.seq.ext_ref_path) {
		gtk_check_button_set_active(reg_reference_checkbutton, TRUE);
		gtk_editable_set_text(GTK_EDITABLE(reg_reference_entry), com.seq.ext_ref_path);
		gtk_editable_set_position(GTK_EDITABLE(reg_reference_entry), -1);
	} else if (!dont_change_reg_radio && !com.seq.ext_ref) {
		gtk_check_button_set_active(reg_reference_checkbutton, FALSE);
		gtk_editable_set_text(GTK_EDITABLE(reg_reference_entry), "");
	}
	gboolean use_external_ref = is_star_align && gtk_check_button_get_active(reg_reference_checkbutton);
	...
	if (isapplyreg && com.seq.ext_ref) {
		gtk_drop_down_set_selected(GTK_DROP_DOWN(comboreg_framing), FRAMING_CURRENT);
	...
	if (has_output && !has_drizzle) {
		gint interpolation_item = gtk_drop_down_get_selected(GTK_DROP_DOWN(ComboBoxRegInter));
...
static int populate_drizzle_data(struct driz_args_t *driz, sequence *seq) {
	driz->use_flats = siril_toggle_get_active(GTK_WIDGET(driz_use_flats));
	driz->weight_scale = 1.f; // Not used for now
	driz->kernel = (enum e_kernel_t) gtk_drop_down_get_selected(GTK_DROP_DOWN(combo_driz_kernel));
	...
	if (driz->use_flats) {
		...
		const gchar *flat_filename = gtk_editable_get_text(GTK_EDITABLE(flatname_entry));
...
static int fill_registration_structure_from_GUI(struct registration_args *regargs) {
	...
	regargs->layer = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboboxreglayer));
	...
	if (is_star_align && gtk_check_button_get_active(reg_reference_checkbutton)) {
		const gchar *path = gtk_editable_get_text(GTK_EDITABLE(reg_reference_entry));
		regargs->external_ref_path = g_strdup(path);
		regargs->use_external_ref = TRUE;
	}
	if (regindex == REG_3STARS) {
		regargs->follow_star = siril_toggle_get_active(GTK_WIDGET(followStarCheckButton));
		regargs->type = (siril_toggle_get_active(GTK_WIDGET(onlyshift_checkbutton))) ? SHIFT_TRANSFORMATION : SIMILARITY_TRANSFORMATION;
	}
	if (is_star_align) {
		regargs->min_pairs = gtk_spin_button_get_value_as_int(spinbut_minpairs);
		int starmaxactive = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboreg_maxstars));
	...
	if (regindex == REG_APPLY) {
		regargs->framing = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboreg_framing));
		get_reg_sequence_filtering_from_gui(
				&regargs->filtering_criterion, &regargs->filtering_parameter, -1);
	} else {
		regargs->filters.filter_included = gtk_drop_down_get_selected(GTK_DROP_DOWN(reg_sel_all_combobox));
	}

	if (has_output_images) {
		regargs->prefix = strdup(gtk_editable_get_text(GTK_EDITABLE(regseqname_entry)));
		...
		if (has_drizzle) {
			...
			regargs->interpolation = gtk_drop_down_get_selected(GTK_DROP_DOWN(ComboBoxRegInter));
			regargs->clamp = siril_toggle_get_active(GTK_WIDGET(toggle_reg_clamp)) && (regargs->interpolation == OPENCV_CUBIC || regargs->interpolation == OPENCV_LANCZOS4);
		}
	}

	regargs->undistort = DISTO_UNDEF;
	if (is_star_align) {
		regargs->undistort = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboreg_undistort));
		if (regargs->undistort > DISTO_UNDEF) {
			regargs->distoparam.index = regargs->undistort;
			if (regargs->undistort == DISTO_FILE) {
				regargs->distoparam.filename = g_strdup(gtk_editable_get_text(GTK_EDITABLE(reg_wcsfile_entry)));
			}
		}
	}
...
gboolean registration_get_follow_star(void) {
	registration_init_statics();
	return followStarCheckButton ? siril_toggle_get_active(GTK_WIDGET(followStarCheckButton)) : FALSE;
}

int get_registration_layer_from_GUI(const sequence *seq) {
	int reglayer = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboboxreglayer));
...
gboolean end_register_idle(gpointer p) {
	...
	if (!args->retval) {
		if (!args->load_new_sequence && sequence_is_loaded()) {
			int chan = gtk_drop_down_get_selected(GTK_DROP_DOWN(comboboxreglayer));
			update_seqlist(chan);
...
void reset_3stars(void) {
	...
	siril_toggle_set_active(GTK_WIDGET(onlyshift_checkbutton), FALSE);
...
gboolean _3stars_check_selection(void) {
	if (com.seq.current < 0)
		return FALSE;
	registration_init_statics();
	gboolean dofollow = siril_toggle_get_active(GTK_WIDGET(followStarCheckButton));
	gboolean doall = !gtk_drop_down_get_selected(GTK_DROP_DOWN(reg_sel_all_combobox));
	if (dofollow) {
...
void on_select_star_button_clicked(GtkButton *button, gpointer user_data) {
	...
	if (three_buttons[0] == widget) {
		...
		selected_stars = 0;
		awaiting_star = 1;
		siril_toggle_set_active(GTK_WIDGET(onlyshift_checkbutton), TRUE);
		gtk_widget_set_sensitive(GTK_WIDGET(onlyshift_checkbutton), FALSE);
