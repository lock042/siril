...
static void free_cbbt_layers(void) {
	GtkDropDown *cbbt = GTK_DROP_DOWN(GTK_WIDGET(
		gtk_builder_get_object(gui.builder, "comboboxreglayer")));
	siril_drop_down_clear_strings(cbbt);
}
...
static void fillSeqAviExport(void) {
	...
	gtk_editable_set_text(GTK_EDITABLE(widthEntry), width);
	gtk_editable_set_text(GTK_EDITABLE(heightEntry), height);
	if (com.seq.type == SEQ_SER && com.seq.ser_file) {
		...
		gtk_editable_set_text(GTK_EDITABLE(fpsEntry), fps);
	}
}
...
static gboolean free_image_data_gui(gpointer p) {
	...
	GtkDropDown *binning = GTK_DROP_DOWN(GTK_WIDGET(
...
static gboolean impl_get_star_follow_state(void) {
	GtkCheckButton *follow = GTK_CHECK_BUTTON(GTK_WIDGET(
		gtk_builder_get_object(gui.builder, "followStarCheckButton")));
	return siril_toggle_get_active(GTK_WIDGET(follow));
}
...
/* FLIS composite invalidation — stage 1.2 no-op stub.  Replaced in
 * stage 3.4 with a real implementation that drops per-layer GdkTexture
 * tile caches and queues a snapshot redraw via flis_display_invalidate(). */
/* Stage 3.1 — the FLIS display composite cache lives in image_display.c
 * as static state.  These wrappers expose only the cache-management
 * operations to gui_iface so non-GUI code (image_format_flis primitives,
 * single_image close path) can invalidate / release without including
 * image_display.h. */
...
static void *impl_flis_get_composite(void) {
