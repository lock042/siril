...
void reset_controls() {
	...
	siril_file_chooser_set_current_folder_path(mcfa_chooser0, com.wd);
	siril_file_chooser_set_current_folder_path(mcfa_chooser1, com.wd);
	siril_file_chooser_set_current_folder_path(mcfa_chooser2, com.wd);
	siril_file_chooser_set_current_folder_path(mcfa_chooser3, com.wd);
	/* GTK4: gtk_file_chooser_unselect_all removed */;
	/* GTK4: gtk_file_chooser_unselect_all removed */;
	/* GTK4: gtk_file_chooser_unselect_all removed */;
	/* GTK4: gtk_file_chooser_unselect_all removed */;
	gtk_drop_down_set_selected(mcfa_pattern_combo, 0);
	siril_toggle_set_active(GTK_WIDGET(mcfa_seqapply_btn), FALSE);
	gtk_widget_set_visible(mcfa_seq_controls, FALSE);
	gtk_editable_set_text(GTK_EDITABLE(mcfa_entry_in), "CFA");
	gtk_editable_set_text(GTK_EDITABLE(mcfa_entry_out), "mCFA_");
}
...
void on_merge_cfa_seqapply_toggled(GtkCheckButton *button, gpointer user_data) {
	merge_cfa_init_statics();
	gtk_widget_set_visible(mcfa_seq_controls, siril_toggle_get_active(GTK_WIDGET(button)));
...
void apply_to_seq() {
	...
	const gchar *seqmarker = gtk_editable_get_text(GTK_EDITABLE(entryMergeCFAin));
	...
	const gchar *outText = gtk_editable_get_text(GTK_EDITABLE(entryMergeCFAout));
	args->seqEntryOut = strdup(outText && outText[0] ? outText : "mCFA_");
	if (!args->seqEntryOut) {
		goto cleanup;
	}

	GtkDropDown *combo_pattern = mcfa_pattern_combo;
	args->pattern = (sensor_pattern)gtk_drop_down_get_selected(combo_pattern);
...
void apply_to_img() {
	GtkDropDown *combo_pattern = mcfa_pattern_combo;
	gint p = gtk_drop_down_get_selected(combo_pattern);
...
void on_merge_cfa_apply_clicked(GtkButton *button, gpointer user_data) {
	merge_cfa_init_statics();
	GtkCheckButton *to_seq = mcfa_seqapply_btn;
	if (siril_toggle_get_active(GTK_WIDGET(to_seq)) && sequence_is_loaded())
		apply_to_seq();
	else
		apply_to_img();
}
