...
G_GNUC_UNUSED static gpointer update_preview(gpointer p) {
	uint8_t *buffer = NULL;
	size_t size;
	char *mime_type = NULL;
...
/* update_preview_cb removed: GtkFileChooser dropped its "update-preview"
 * signal in GTK4.  The preview pipeline (update_preview / new_preview_object /
 * end_update_preview_cb) is currently disconnected; siril_file_chooser_add_preview
 * is a no-op until the file dialog migration to GtkFileDialog lands. */
...
void siril_file_chooser_add_preview(GtkFileChooser *dialog, fileChooserPreview *preview) {
	if (com.pref.gui.show_thumbnails) {
		...
		gtk_widget_set_margin_start(GTK_WIDGET(vbox), 12); gtk_widget_set_margin_end(GTK_WIDGET(vbox), 12); gtk_widget_set_margin_top(GTK_WIDGET(vbox), 12); gtk_widget_set_margin_bottom(GTK_WIDGET(vbox), 12);
		...
		gtk_box_append(GTK_BOX(vbox), preview->image);
		gtk_widget_set_margin_start(preview->name_label, 10);
		gtk_widget_set_margin_end(preview->name_label, 10);
		gtk_widget_set_margin_top(preview->name_label, 10);
		gtk_widget_set_margin_bottom(preview->name_label, 10);
		gtk_box_append(GTK_BOX(vbox), preview->name_label);
		gtk_box_append(GTK_BOX(vbox), preview->size_label);
		gtk_box_append(GTK_BOX(vbox), preview->dim_label);

		gtk_widget_set_visible(vbox, TRUE);

		/* GTK4: gtk_file_chooser_set_preview_widget removed */;
		/* GTK4: gtk_file_chooser_set_use_preview_label removed */;
		/* GTK4: gtk_file_chooser_set_preview_widget_active removed */;

		/* GTK4: GtkFileChooser has no "update-preview" signal — preview
		 * support was removed entirely.  The connect (and update_preview_cb
		 * itself) are no-ops in the GTK4 build until the file dialog
		 * migration to GtkFileDialog lands. */
		(void) preview;
	}
}
