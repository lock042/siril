...
static void ensure_widget_and_parents_visible(GtkWidget *widget) {
	...
	for (GSList *l = parents_to_show; l != NULL; l = l->next) {
		...
		gtk_widget_set_visible(widget_to_show, TRUE);
	}
...
static GtkWidget* floating_window_new(GtkWidget *widget, const gchar *text) {
	...
	/* Phase 14: GTK4 gtk_window_new takes no arguments; GTK_WINDOW_TOPLEVEL
	 * and gtk_window_set_type_hint / GDK_WINDOW_TYPE_HINT_DIALOG were
	 * removed.  Modal/transient windows render as floating dialogs
	 * automatically when set_transient_for is set. */
	window = gtk_window_new();
	gtk_window_set_transient_for(GTK_WINDOW(window), parent_window);
	gtk_window_set_decorated(GTK_WINDOW(window), FALSE);

	box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
	gtk_widget_set_margin_start(GTK_WIDGET(box), 12); gtk_widget_set_margin_end(GTK_WIDGET(box), 12); gtk_widget_set_margin_top(GTK_WIDGET(box), 12); gtk_widget_set_margin_bottom(GTK_WIDGET(box), 12);
	 ...
	 * gtk_window_move are all gone in GTK4 — the compositor owns window
	 * placement.  The previous GTK3 code computed an (x, y) and called
	 * gtk_window_move; in GTK4 the window-level intro popovers are
	 * anchored by the compositor and the manual placement is dropped. */
...
static gboolean intro_popover_update(gpointer user_data) {
	if (go_next) {
		...
		gtk_widget_add_css_class(current_ui->widget, "siril-intro-highlight");
