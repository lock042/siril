...
GtkDropDown *conesearch_combo = NULL;
...
GtkCheckButton *conesearch_photometric = NULL, *conesearch_tag = NULL, *conesearch_log = NULL, *show_tag = NULL, *show_log = NULL;
...
void annotate_dialog_init_statics() {
	if (adj_mag_limit == NULL) {
		...
		// GtkDropDown
		conesearch_combo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "conesearch_combo"));
		// GtkAdjustment
		adj_mag_limit = GTK_ADJUSTMENT(gtk_builder_get_object(gui.builder, "adj_mag_limit"));
		// GtkToggleButton
		conesearch_photometric = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "conesearch_photometric"));
		conesearch_tag = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "conesearch_tag"));
		conesearch_log = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "conesearch_log"));
		show_tag = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "show_tag"));
		show_log = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "show_log"));
		// GtkBox
...
void on_conesearch_combo_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)obj;
	(void)pspec;
	...
	siril_toggle_set_active(GTK_WIDGET(conesearch_tag), logtag);
	siril_toggle_set_active(GTK_WIDGET(conesearch_log), logtag);
...
void on_annotate_dialog_show(GtkWidget *widget, gpointer user_data) {
	...
	if (com.pref.astrometry.default_obscode != NULL) {
		gtk_editable_set_text(GTK_EDITABLE(annotate_obscode_entry), com.pref.astrometry.default_obscode);
	}
	on_conesearch_combo_changed(NULL, NULL, NULL);
}

void on_annotate_save_as_button_clicked(GtkButton *button, gpointer user_data) {
	SirilFileChooser *fc = siril_fc_save(annotate_dialog, GTK_FILE_CHOOSER_ACTION_SAVE);
	siril_fc_set_current_folder_path(fc, com.wd);
	siril_fc_set_select_multiple(fc, FALSE);
	siril_fc_set_current_name(fc, ".csv");
	set_filter(fc);

	if (siril_fc_run(fc) == GTK_RESPONSE_ACCEPT) {
		gchar *file = siril_fc_get_filename(fc);
		if (file) {
			gtk_editable_set_text(GTK_EDITABLE(conesearch_save_entry), file);
...
void on_show_button_clicked(GtkButton *button, gpointer user_data) {
	SirilFileChooser *fc = siril_fc_open(annotate_dialog, GTK_FILE_CHOOSER_ACTION_OPEN);
	siril_fc_set_current_folder_path(fc, com.wd);
	siril_fc_set_select_multiple(fc, FALSE);
	...
	if (siril_fc_run(fc) == GTK_RESPONSE_ACCEPT) {
		gchar *file = siril_fc_get_filename(fc);
		if (file) {
			gtk_editable_set_text(GTK_EDITABLE(show_file_entry), file);
		...
		if (result && error == PSF_NO_ERR) {
			...
			if (world_x >= 0.0 && !isnan(world_x) && !isnan(world_y)) {
				...
				if (world_cs) {
					...
					gtk_editable_set_text(GTK_EDITABLE(gtk_builder_get_object(gui.builder, "show_ra_entry")), ra);
					gtk_editable_set_text(GTK_EDITABLE(gtk_builder_get_object(gui.builder, "show_dec_entry")), dec);
...
static int collect_single_coords_and_name(double *ra, double *dec, gchar **name) {
	const gchar *ra_str = gtk_editable_get_text(GTK_EDITABLE(show_ra_entry));
	const gchar *dec_str = gtk_editable_get_text(GTK_EDITABLE(show_dec_entry));
	...
	*dec = siril_world_cs_get_delta(coords);
	...
	const gchar *name_entry = gtk_editable_get_text(GTK_EDITABLE(show_name_entry));
...
static conesearch_params* parse_conesearch_ui() {
	...
	params->display_log = siril_toggle_get_active(GTK_WIDGET(conesearch_log)) ? BOOL_TRUE : BOOL_FALSE;
	params->display_tag = siril_toggle_get_active(GTK_WIDGET(conesearch_tag)) ? BOOL_TRUE : BOOL_FALSE;
	params->photometric = siril_toggle_get_active(GTK_WIDGET(conesearch_photometric));
	params->limit_mag = gtk_spin_button_get_value(conesearch_maglimit);
	const gchar *output = gtk_editable_get_text(GTK_EDITABLE(conesearch_save_entry));

	if (output != NULL && strlen(output) != 0) {
		params->outfilename = g_strdup(output);
	}
	const gchar *obscode = gtk_editable_get_text(GTK_EDITABLE(annotate_obscode_entry));
...
static show_params* parse_show_ui() {
	...
	if (!g_strcmp0("page0", visible_child_name)) {
		const gchar *input = gtk_editable_get_text(GTK_EDITABLE(show_file_entry));
		...
		params->coords = NULL;
		params->name = NULL;
		params->display_log = siril_toggle_get_active(GTK_WIDGET(show_log));
		params->display_tag = siril_toggle_get_active(GTK_WIDGET(show_tag));
	} else {
		params->file = NULL;
		double ra = 0., dec = 0.;
...
void on_annotate_apply_clicked(GtkButton *button, gpointer user_data) {
	...
	case SEARCH_OBJECT_PAGE:
		/* search_objects_entry is GtkSearchEntry — both GtkSearchEntry and
		 * GtkEntry implement GtkEditable in GTK4. */
		entry = GTK_EDITABLE(search_objects_entry);
		search_object(entry);
		break;
	}
	set_cursor_waiting(FALSE);
