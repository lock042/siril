...
static void siril_image_view_snapshot(GtkWidget *widget, GtkSnapshot *snapshot) {
	...
	if (tiles_snapshot && img_w > 0 && img_h > 0) {
		...
		if (!used_gpu_compose) {
			/* Proxy backdrop: a single scaled-texture node covering the
			 * whole image, drawn *under* the real tiles.  Any visible tile
			 * that hasn't been materialised yet (fast zoom-out) shows the
			 * low-res proxy here instead of a hole; the refine idle then
			 * paints sharp tiles over it frame by frame.  Same widget-space
			 * rect convention as the tiles. */
