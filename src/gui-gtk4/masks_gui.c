 ...
 * in callbacks.c::initialize_mask_tab_label), so casting to GtkToggleButton
 * via GTK_TOGGLE_BUTTON would emit a runtime "invalid cast" warning in GTK4
 * where GtkCheckButton no longer derives from GtkToggleButton.  Hold a
 * GtkWidget* and route through the polymorphic siril_toggle_* shim. */
gboolean set_mask_active_idle(gpointer p) {
	gboolean state = GPOINTER_TO_INT(p);
	GtkWidget *button = GTK_WIDGET(gtk_builder_get_object(gui.builder, "mask_enable_check"));
	if (!button) return FALSE;
	g_signal_handlers_block_by_func(button, on_mask_enable_toggled, NULL);
	siril_toggle_set_active(button, state);
	g_signal_handlers_unblock_by_func(button, on_mask_enable_toggled, NULL);
	return FALSE;
}
...
static void masks_gui_init_statics(void) {
	...
	combo_feather_type = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "combo_mask_feather_type"));
...
void on_mask_from_image_dialog_show(GtkWidget *widget, gpointer user_data) {
	...
	if (mask_from_file_mode) {
		...
		if (mask_file_chooser_box) gtk_widget_set_visible(mask_file_chooser_box, TRUE);
		if (mask_method_box) gtk_widget_set_visible(mask_method_box, TRUE);
		...
		if (entry_mask_filename) {
			gtk_editable_set_text(GTK_EDITABLE(entry_mask_filename), "");
		}

		/* Set initial visibility based on type combo */
		gint active = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo_mask_from_image_type));
		if (active == 0) {
			if (mask_luminance_grid) gtk_widget_set_visible(mask_luminance_grid, TRUE);
			if (mask_channel_grid) gtk_widget_set_visible(mask_channel_grid, FALSE);
		} else {
			if (mask_luminance_grid) gtk_widget_set_visible(mask_luminance_grid, FALSE);
			if (mask_channel_grid) gtk_widget_set_visible(mask_channel_grid, TRUE);
		}
	} else {
		/* Hide file chooser, show method box */
		if (mask_file_chooser_box) gtk_widget_set_visible(mask_file_chooser_box, FALSE);
		if (mask_method_box) gtk_widget_set_visible(mask_method_box, TRUE);
...
void on_mask_from_image_apply_clicked(GtkButton *button, gpointer user_data) {
	/* Get option flags */
	gboolean autostretch = siril_toggle_get_active(GTK_WIDGET(toggle_mask_autostretch));
	gboolean invert = siril_toggle_get_active(GTK_WIDGET(toggle_mask_invert));

	/* Get the bitdepth */
	gint bitdepth_index = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo_mask_from_image_bitdepth));
	...
	gint mask_type = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo_mask_from_image_type));
...
void on_combo_mask_from_image_type_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	...
	(void)pspec;
	gint active = gtk_drop_down_get_selected(combo);
...
void on_mask_from_stars_apply_clicked(GtkButton *button, gpointer user_data) {
	...
	/* Get the bitdepth */
	gint bitdepth_index = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo_mask_from_stars_bitdepth));
	uint8_t bitdepth = (uint8_t) bitdepth_index == 0 ? 8 : bitdepth_index == 1 ? 16 : 32;
...
void on_feather_mask_apply_clicked(GtkButton *button, gpointer user_data) {
	...
	GtkDropDown *combo = combo_feather_type;
	float distance = gtk_spin_button_get_value(spin);
	feather_mode mode = (feather_mode) gtk_drop_down_get_selected(combo);
...
void on_mask_color_apply_clicked(GtkButton *button, gpointer user_data) {
	...
	gint bitdepth_index = gtk_drop_down_get_selected(GTK_DROP_DOWN(combo_color_mask_bitdepth));
	uint8_t bitdepth = (uint8_t)(bitdepth_index == 0 ? 8 : bitdepth_index == 1 ? 16 : 32);
	gint feather_radius = gtk_spin_button_get_value_as_int(spin_color_feather);
	gboolean invert = siril_toggle_get_active(GTK_WIDGET(toggle_color_mask_invert));
	gboolean cleanup = siril_toggle_get_active(GTK_WIDGET(toggle_color_mask_cleanup));
