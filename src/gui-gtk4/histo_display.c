...
static void update_cursor(GtkWidget *widget, gint mouse_x, gint mouse_y) {
	...
	if (in_header) {
		...
		cursor = gdk_cursor_new_from_name("nw-resize", NULL);
	} else if (edge == (RESIZE_RIGHT | RESIZE_TOP) || edge == (RESIZE_LEFT | RESIZE_BOTTOM)) {
		cursor = gdk_cursor_new_from_name("ne-resize", NULL);
	} else if (edge & RESIZE_LEFT || edge & RESIZE_RIGHT) {
		cursor = gdk_cursor_new_from_name("ew-resize", NULL);
	} else if (edge & RESIZE_TOP || edge & RESIZE_BOTTOM) {
		cursor = gdk_cursor_new_from_name("ns-resize", NULL);
	}
...
/* Returns TRUE if (x, y) lies inside the overlay rectangle (header
 * included).  Any inside-hit is claimed by the overlay's gesture so the
 * underlying image-interaction controllers don't also process it. */
static gboolean overlay_hit_test(double x, double y) {
	if (!histo_state.show_histo) return FALSE;
	return x >= histo_state.x && x <= histo_state.x + histo_state.width
	    && y >= histo_state.y && y <= histo_state.y + histo_state.height;
}

/* GTK4 overlay event-controller callbacks.  Replace the GTK3
 * button-press / button-release / motion-notify handlers — same logic,
 * but (x, y) come from the GtkGestureClick / GtkEventControllerMotion
 * directly rather than GdkEventButton/Motion.  Attached at CAPTURE phase
 * to each gray viewport drawingarea in init_histogram_overlay so they
 * see clicks before the image-display gestures and can intercept any
...
static void overlay_press_cb(GtkGestureClick *gesture, int n_press,
	...
	if (is_over_log_button(x, y)) {
		histo_state.logarithmic = !histo_state.logarithmic;
		handled = TRUE;
	} else {
		gint rgb_button = is_over_rgb_button(x, y);
		if (rgb_button > 0 && !histo_state.mode_luminance) {
			switch (rgb_button) {
			case 1: histo_state.show_red   = !histo_state.show_red;   break;
			case 2: histo_state.show_green = !histo_state.show_green; break;
			case 3: histo_state.show_blue  = !histo_state.show_blue;  break;
			}
			handled = TRUE;
		} else if (is_over_rgb_k_button(x, y)) {
			histo_state.mode_luminance = !histo_state.mode_luminance;
			handled = TRUE;
		} else {
			gboolean in_header = FALSE;
			ResizeEdge edge = get_resize_edge(x, y, &in_header);
			if (in_header) {
				histo_state.is_dragging = TRUE;
				histo_state.drag_start_x = x;
				histo_state.drag_start_y = y;
				histo_state.drag_offset_x = x - histo_state.x;
				histo_state.drag_offset_y = y - histo_state.y;
				handled = TRUE;
			} else if (edge != RESIZE_NONE) {
				histo_state.is_resizing = TRUE;
				histo_state.resize_edge = edge;
				histo_state.resize_start_x = x;
				histo_state.resize_start_y = y;
				...
				handled = TRUE;
			} else if (overlay_hit_test(x, y)) {
				/* A click anywhere else over the overlay still gets
				 * claimed so it doesn't slip through to the image. */
				handled = TRUE;
			}
		}
	}
...
static void overlay_release_cb(GtkGestureClick *gesture, int n_press,
                               double x, double y, gpointer data) {
...
static void overlay_motion_cb(GtkEventControllerMotion *controller,
	...
	if (histo_state.is_dragging) {
		cursor_state.active = FALSE;
		histo_state.x = x - histo_state.drag_offset_x;
		histo_state.y = y - histo_state.drag_offset_y;
		gint ww = gtk_widget_get_width(widget);
		gint wh = gtk_widget_get_height(widget);
		if (histo_state.x < 0) histo_state.x = 0;
		if (histo_state.y < 0) histo_state.y = 0;
		if (histo_state.x + histo_state.width  > ww) histo_state.x = ww - histo_state.width;
		if (histo_state.y + histo_state.height > wh) histo_state.y = wh - histo_state.height;
		gtk_widget_queue_draw(widget);
		return;
	}
...
void histogram_overlay_paint(GtkWidget *widget, cairo_t *cr) {
	...
	if (!positioned) {
		gint widget_height = gtk_widget_get_height(widget);
...
void init_histogram_overlay(void) {
	...
	gint widget_height = gtk_widget_get_height(ref_widget);
	if (widget_height > 0) {
	...
	for (size_t i = 0; i < G_N_ELEMENTS(areas); i++) {
		...
		gtk_event_controller_set_propagation_phase(
		    motion, GTK_PHASE_CAPTURE);
		g_signal_connect(motion, "motion", G_CALLBACK(overlay_motion_cb), NULL);
