/*
 * This file is part of Siril, an astronomy image processor.
 * Copyright (C) 2005-2011 Francois Meyer (dulle at free.fr)
 * Copyright (C) 2012-2026 team free-astro (see more in AUTHORS file)
 * Reference site is https://siril.org
 *
 * Siril is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Siril is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Siril. If not, see <http://www.gnu.org/licenses/>.
 */

#include "core/siril.h"
#include "core/proto.h"
#include "core/icc_profile.h"
#include "core/initfile.h"
#include "core/siril_app_dirs.h"
#include "core/siril_language.h"
#include "core/settings.h"
#include "core/siril_log.h"
#include "algos/astrometry_solver.h"
#include "io/annotation_catalogues.h"
#include "io/siril_pythonmodule.h"
#include "gui-gtk4/annotations_pref.h"
#include "gui-gtk4/callbacks.h"
#include "gui-gtk4/icc_profile.h"
#include "gui-gtk4/utils.h"
#include "gui-gtk4/message_dialog.h"
#include "gui-gtk4/progress_and_log.h"
#include "gui-gtk4/script_menu.h"
#include "gui-gtk4/dialogs.h"
#include "gui-gtk4/fix_xtrans_af.h"
#include "gui-gtk4/PSF_list.h"
#include "gui-gtk4/photometric_cc.h"
#include "gui-gtk4/python_gui.h"
#include "gui-gtk4/registration.h"
#include "gui-gtk4/siril_intro.h"
#include "io/single_image.h"
#include "io/sequence.h"
#include "stacking/stacking.h"
#include "io/siril_git.h"

#include "preferences.h"

/* ── Language combo (moved from core/siril_language.c) ────────────────────── */

void siril_language_fill_combo(const gchar *language) {
	GtkDropDown *lang_combo = GTK_DROP_DOWN(GTK_WIDGET(gtk_builder_get_object(gui.builder, "combo_language")));
	GList *list = g_hash_table_get_keys(siril_language_get_full_list());
	gboolean lang_changed = FALSE;
	int i = 1;

	siril_drop_down_clear_strings(lang_combo);
	siril_drop_down_append_text(lang_combo, _("System Language"));

	list = g_list_sort(list, (GCompareFunc) locale_compare);

	for (GList *l = list; l; l = l->next) {
		siril_drop_down_append_text(lang_combo, l->data);
		gchar *locale = extract_locale_from_string(l->data);
		if (!g_strcmp0(language, locale)) {
			gtk_drop_down_set_selected(GTK_DROP_DOWN(lang_combo), i);
			lang_changed = TRUE;
		}
		g_free(locale);
		i++;
	}
	if (!lang_changed)
		gtk_drop_down_set_selected(GTK_DROP_DOWN(lang_combo), 0);
	g_list_free(list);
}

gchar *get_interface_language(void) {
	GtkDropDown *lang_combo = GTK_DROP_DOWN(GTK_WIDGET(gtk_builder_get_object(gui.builder, "combo_language")));
	if (gtk_drop_down_get_selected(GTK_DROP_DOWN(lang_combo)) == 0)
		return g_strdup("");
	gchar *str = siril_drop_down_get_active_text(lang_combo);
	return extract_locale_from_string(str);
}
#include "filters/starnet.h"

#ifndef W_OK
#define W_OK 2
#endif

static gchar *sw_dir = NULL;
static gchar *st_weights = NULL;
static starnet_version st_version = NIL;
static gboolean update_custom_gamut = FALSE;
void on_working_gamut_changed(GObject *obj, GParamSpec *pspec, gpointer user_data);

// Re-entry guards for the Python interpreter widgets. When the user
// cancels the confirmation dialog we revert the widget programmatically;
// the guard prevents the resulting "toggled" / "notify::selected" signal
// from re-opening the dialog.
static gboolean python_uv_widget_reverting = FALSE;
// Remembers the version that was either loaded from prefs at dialog-open
// time or last confirmed by the user, so we can revert to it on cancel.
static gchar *python_uv_version_confirmed = NULL;

// uv-managed Python version choices. The order MUST match the GtkStringList
// items of combo_python_uv_version in settings_window.ui, since the gtk4
// GtkDropDown selects by index rather than a string id. Default is "3.13".
static const char * const uv_python_versions[] = { "3.14", "3.13", "3.12", "3.11" };
#define UV_PYTHON_VERSION_DEFAULT_INDEX 1

static const gchar *uv_python_version_from_index(guint idx) {
	if (idx >= G_N_ELEMENTS(uv_python_versions))
		idx = UV_PYTHON_VERSION_DEFAULT_INDEX;
	return uv_python_versions[idx];
}

static guint uv_python_index_from_version(const gchar *version) {
	if (version) {
		for (guint i = 0; i < G_N_ELEMENTS(uv_python_versions); i++) {
			if (g_strcmp0(version, uv_python_versions[i]) == 0)
				return i;
		}
	}
	return UV_PYTHON_VERSION_DEFAULT_INDEX;
}

static gboolean scripts_updated = FALSE;

void notify_script_update() {
	scripts_updated = TRUE;
}

static void reset_swapdir() {
	GtkWidget *swap_dir = lookup_widget("filechooser_swap");
	const gchar *dir;

	dir = g_get_tmp_dir();

	g_rw_lock_writer_lock(&com.pref_rwlock);
	if (g_strcmp0(dir, com.pref.swap_dir)) {
		g_free(com.pref.swap_dir);
		com.pref.swap_dir = g_strdup(dir);
		siril_file_chooser_set_filename(swap_dir, dir);
	}
	g_rw_lock_writer_unlock(&com.pref_rwlock);
}

static void update_debayer_preferences() {
	com.pref.debayer.use_bayer_header = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_use_header"))));
	com.pref.debayer.orientation = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combo_roworder")));
	com.pref.debayer.xbayeroff = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("xbayeroff_spin")));
	com.pref.debayer.ybayeroff = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("ybayeroff_spin")));
	com.pref.debayer.bayer_pattern = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("comboBayer_pattern")));
	com.pref.debayer.bayer_inter = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("comboBayer_inter")));
	com.pref.debayer.xtrans_passes = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("xtranspass_spin")));
}

static void update_astrometry_preferences() {
	get_astrometry_catalogue_values();

	com.pref.gui.position_compass = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("compass_combobox")));
	com.pref.wcs_formalism = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("wcs_formalism_combobox")));
	gchar *newpath = siril_file_chooser_get_filename(lookup_widget("localcatalogue_path1"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.catalogue_paths[0]);
		com.pref.catalogue_paths[0] = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("localcatalogue_path2"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.catalogue_paths[1]);
		com.pref.catalogue_paths[1] = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("localcatalogue_path3"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.catalogue_paths[2]);
		com.pref.catalogue_paths[2] = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("localcatalogue_path4"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.catalogue_paths[3]);
		com.pref.catalogue_paths[3] = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("localcatalogue_path5"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.catalogue_paths[4]);
		com.pref.catalogue_paths[4] = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("localcatalogue_path6"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.catalogue_paths[5]);
		com.pref.catalogue_paths[5] = newpath;
	}

	com.pref.astrometry.gaia_cache_duration = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("gaia_cache_duration")));

	com.pref.astrometry.sip_correction_order = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("spin_asnet_sip_order")));
	com.pref.astrometry.percent_scale_range = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("spin_sampling_tolerance")));
	com.pref.astrometry.radius_degrees = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spin_asnet_radius")));
	com.pref.astrometry.keep_xyls_files = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_asnet_xyls"))));
	com.pref.astrometry.keep_wcs_files = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_asnet_wcs"))));
	com.pref.astrometry.max_seconds_run = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(lookup_widget("spin_asnet_max_sec")));
	com.pref.astrometry.update_default_scale = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("astrometry_update_fields"))));
	com.pref.astrometry.show_asnet_output = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_asnet_show_output"))));
	if (com.pref.astrometry.default_obscode)
		g_free(com.pref.astrometry.default_obscode);
	com.pref.astrometry.default_obscode = g_strdup(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("obscode_entry"))));
	if (strlen(com.pref.astrometry.default_obscode) != 3 && strlen(com.pref.astrometry.default_obscode) != 0) {
		g_free(com.pref.astrometry.default_obscode);
		com.pref.astrometry.default_obscode = NULL;
		siril_log_error(_("Error: invalid IAU observatory code read from preferences file. Code must be a 3-character code.\n"));
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("obscode_entry")), "");
	}
	if (com.pref.astrometry.default_obscode && strlen(com.pref.astrometry.default_obscode) == 0) {
		g_free(com.pref.astrometry.default_obscode);
		com.pref.astrometry.default_obscode = NULL;
	}
	// In the prefs structure, the dir is stored alongside starnet, not in astrometry
	com.pref.asnet_dir = siril_file_chooser_get_filename(lookup_widget("filechooser_asnet"));
	reset_astrometry_checks();
}

static void update_prepro_preferences() {

	if (com.pref.prepro.bias_lib) {
		g_free(com.pref.prepro.bias_lib);
		com.pref.prepro.bias_lib = NULL;
	}
	const gchar *biasentry = gtk_editable_get_text(GTK_EDITABLE(lookup_widget("biaslib_entry")));
	if (biasentry) {
		com.pref.prepro.bias_lib = g_strdup(biasentry);
		com.pref.prepro.use_bias_lib = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_bias"))));
	}

	if (com.pref.prepro.dark_lib) {
		g_free(com.pref.prepro.dark_lib);
		com.pref.prepro.dark_lib = NULL;
	}
	const gchar *darkentry = gtk_editable_get_text(GTK_EDITABLE(lookup_widget("darklib_entry")));
	if (darkentry) {
		com.pref.prepro.dark_lib = g_strdup(darkentry);
		com.pref.prepro.use_dark_lib = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_dark"))));
	}

	if (com.pref.prepro.flat_lib) {
		g_free(com.pref.prepro.flat_lib);
		com.pref.prepro.flat_lib = NULL;
	}
	const gchar *flatentry = gtk_editable_get_text(GTK_EDITABLE(lookup_widget("flatlib_entry")));
	if (flatentry) {
		com.pref.prepro.flat_lib = g_strdup(flatentry);
		com.pref.prepro.use_flat_lib = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_flat"))));
	}

	if (com.pref.prepro.disto_lib) {
		g_free(com.pref.prepro.disto_lib);
		com.pref.prepro.disto_lib = NULL;
	}
	const gchar *distoentry = gtk_editable_get_text(GTK_EDITABLE(lookup_widget("distolib_entry")));
	if (distoentry) {
		com.pref.prepro.disto_lib = g_strdup(distoentry);
		com.pref.prepro.use_disto_lib = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_disto"))));
	}

	if (com.pref.prepro.stack_default) {
		g_free(com.pref.prepro.stack_default);
		com.pref.prepro.stack_default = NULL;
	}
	const gchar *stackentry = gtk_editable_get_text(GTK_EDITABLE(lookup_widget("stack_default_entry")));
	if (stackentry && stackentry[0] != '\0') {
		com.pref.prepro.stack_default = g_strdup(stackentry);
		com.pref.prepro.use_stack_default = (com.pref.prepro.stack_default) && siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_stack"))));
	}

	com.pref.prepro.xtrans_af.x = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_af_x"))), NULL, 10);
	com.pref.prepro.xtrans_af.y = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_af_y"))), NULL, 10);
	com.pref.prepro.xtrans_af.w = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_af_w"))), NULL, 10);
	com.pref.prepro.xtrans_af.h = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_af_h"))), NULL, 10);

	com.pref.prepro.xtrans_sample.x = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_sample_x"))), NULL, 10);
	com.pref.prepro.xtrans_sample.y = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_sample_y"))), NULL, 10);
	com.pref.prepro.xtrans_sample.w = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_sample_w"))), NULL, 10);
	com.pref.prepro.xtrans_sample.h = g_ascii_strtoull(gtk_editable_get_text(GTK_EDITABLE(lookup_widget("xtrans_sample_h"))), NULL, 10);
}

static void update_photometry_preferences() {
	com.pref.phot_set.gain = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinGain")));
	com.pref.phot_set.inner = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinInner")));
	com.pref.phot_set.outer = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinOuter")));
	com.pref.phot_set.aperture = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinAperture")));
	com.pref.phot_set.force_radius = !siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("photometry_force_radius_button"))));
	com.pref.phot_set.auto_aperture_factor = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinRadRatio")));
	com.pref.phot_set.minval = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinMinPhot")));
	com.pref.phot_set.maxval = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinMaxPhot")));
}

static void update_analysis_preferences() {
	com.pref.analysis.mosaic_panel = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinpanel")));
	com.pref.analysis.mosaic_window = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinwindow")));
}

static void update_scripts_preferences() {
	if (com.pref.gui.script_path)
		g_slist_free_full(g_steal_pointer(&com.pref.gui.script_path), g_free);
	com.pref.gui.script_path = get_list_from_preferences_dialog();
	com.pref.gui.warn_scripts_run = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskScript"))));
	com.pref.script_check_requires = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("script_check_version"))));
	com.pref.drizz_weight_match_bitpix = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_match_drizzweight_bitpix"))));
#ifdef HAVE_LIBGIT2
	com.pref.use_scripts_repository = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_use_gitscripts"))));
	com.pref.auto_script_update = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_script_automatic_updates"))));
	com.pref.spcc.use_spcc_repository = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("spcc_repo_enable"))));
	com.pref.spcc.auto_spcc_update = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("spcc_repo_sync_at_startup"))));
#else
	com.pref.use_scripts_repository = FALSE;
	com.pref.spcc.use_spcc_repository = FALSE;
#endif
	// Python interpreter section. Confirmation already happened at widget-
	// change time (see on_checkbutton_python_uv_managed_toggled /
	// on_combo_python_uv_version_changed); here we just snapshot the
	// current widget state into the pref struct.
	com.pref.python.uv_managed = siril_toggle_get_active(
			GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_python_uv_managed"))));
	guint ver_idx = gtk_drop_down_get_selected(
			GTK_DROP_DOWN(lookup_widget("combo_python_uv_version")));
	g_free(com.pref.python.uv_python_version);
	com.pref.python.uv_python_version = g_strdup(uv_python_version_from_index(ver_idx));
}

static void update_user_interface_preferences() {
	com.pref.lang = get_interface_language();
	int theme = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combo_theme")));
	if (theme != com.pref.gui.combo_theme) {
		com.pref.gui.combo_theme = theme;
		siril_set_theme(theme);
	}
	com.pref.gui.font_scale = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("pref_fontsize")));
	com.pref.gui.icon_symbolic = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_iconstyle"))));
	com.pref.gui.remember_windows = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("rememberWindowsCheck"))));
	com.pref.gui.show_thumbnails = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("show_preview_button"))));
	int selected_index = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("thumbnails_box_size")));
	switch (selected_index) {
	    case 0:
	        com.pref.gui.thumbnail_size = 128; // First option (index 0)
	        break;
	    case 1:
	        com.pref.gui.thumbnail_size = 256; // Second option (index 1)
	        break;
	    case 2:
	        com.pref.gui.thumbnail_size = 512; // Third option (index 2)
	        break;
	    default:
	        com.pref.gui.thumbnail_size = 128; // Default value in case of an unexpected index
	        break;
	}
	selected_index = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_ui_mask_bitpix")));
	switch (selected_index) {
	    case 0:
	        com.pref.default_mask_bitpix = 8; // First option (index 0)
	        break;
	    case 1:
	        com.pref.default_mask_bitpix = 16; // Second option (index 1)
	        break;
	    case 2:
	        com.pref.default_mask_bitpix = 32; // Third option (index 2)
	        break;
	    default:
	        com.pref.default_mask_bitpix = -1; // Default value in case of an unexpected index
	        break;
	}
	com.pref.gui.mask_tints_vports = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_ui_mask_tint"))));
	com.pref.gui.default_rendering_mode = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_default_stf")));
	com.pref.gui.display_histogram_mode = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_default_histo_mode")));
	com.pref.gui.roi_mode = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_ui_roimode")));
	com.pref.gui.mmb_action = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_ui_mmb_zoom")));
	com.pref.gui.mouse_speed_limit = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spin_mouse_speed_limit")));
	update_roi_config();
	/* Configure colors */
	/* Read RGBA from each GtkColorDialogButton.  The pointer it hands back
	 * is owned by the widget — copy it before stringifying. */
	const GdkRGBA *rgba;

	rgba = gtk_color_dialog_button_get_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_bkg")));
	g_free(com.pref.gui.config_colors.color_bkg_samples);
	com.pref.gui.config_colors.color_bkg_samples =
		rgba ? gdk_rgba_to_string(rgba) : NULL;

	rgba = gtk_color_dialog_button_get_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_std_annot")));
	g_free(com.pref.gui.config_colors.color_std_annotations);
	com.pref.gui.config_colors.color_std_annotations =
		rgba ? gdk_rgba_to_string(rgba) : NULL;

	rgba = gtk_color_dialog_button_get_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_dso_annot")));
	g_free(com.pref.gui.config_colors.color_dso_annotations);
	com.pref.gui.config_colors.color_dso_annotations =
		rgba ? gdk_rgba_to_string(rgba) : NULL;

	rgba = gtk_color_dialog_button_get_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_sso_annot")));
	g_free(com.pref.gui.config_colors.color_sso_annotations);
	com.pref.gui.config_colors.color_sso_annotations =
		rgba ? gdk_rgba_to_string(rgba) : NULL;

	rgba = gtk_color_dialog_button_get_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_tmp_annot")));
	g_free(com.pref.gui.config_colors.color_tmp_annotations);
	com.pref.gui.config_colors.color_tmp_annotations =
		rgba ? gdk_rgba_to_string(rgba) : NULL;
}

static void update_color_management_preferences() {
	gchar *newpath = siril_file_chooser_get_filename(lookup_widget("pref_custom_monitor_profile"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.icc.icc_path_monitor);
		com.pref.icc.icc_path_monitor = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("pref_soft_proofing_profile"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.icc.icc_path_soft_proof);
		com.pref.icc.icc_path_soft_proof = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("custom_gray_icc_matching_trc"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.icc.custom_icc_gray);
		com.pref.icc.custom_icc_gray = newpath;
	}
	newpath = siril_file_chooser_get_filename(lookup_widget("custom_icc_standard_trc"));
	if (newpath && newpath[0] != '\0') {
		g_free(com.pref.icc.custom_icc_trc);
		com.pref.icc.custom_icc_trc = newpath;
	}
	com.pref.icc.rendering_intent = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combo_rendering_intent")));
	com.pref.icc.export_intent = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combo_export_intent")));
	com.pref.icc.default_to_srgb = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("default_to_srgb"))));
	com.pref.icc.working_gamut = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("working_gamut")));
	com.pref.icc.export_8bit_method = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("export_profile_8bit")));
	com.pref.icc.export_16bit_method = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("export_profile_16bit")));
	com.pref.icc.cmf = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combo_cmf_pref")));
	com.pref.icc.custom_monitor_profile_active = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("custom_monitor_profile_active"))));
	com.pref.icc.soft_proofing_profile_active = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("custom_proofing_profile_active"))));
	com.pref.icc.rendering_bpc = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_rendering_bpc"))));
	com.pref.icc.pedantic_linear = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_pedantic_linear"))));
	com.icc.rendering_flags = ((com.pref.icc.rendering_bpc * cmsFLAGS_BLACKPOINTCOMPENSATION) & !(com.pref.icc.rendering_intent == INTENT_ABSOLUTE_COLORIMETRIC));
	com.gui_icc.proofing_flags = ((com.pref.icc.rendering_bpc * cmsFLAGS_BLACKPOINTCOMPENSATION) & !(com.pref.icc.rendering_intent == INTENT_ABSOLUTE_COLORIMETRIC)) | cmsFLAGS_SOFTPROOFING;
	com.pref.icc.autoassignment = 	((siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_load")))) * ICC_ASSIGN_ON_LOAD) +
									(siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_stack")))) * ICC_ASSIGN_ON_STACK) +
									(siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_stretch")))) * ICC_ASSIGN_ON_STRETCH) +
									(siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_composition")))) * ICC_ASSIGN_ON_COMPOSITION));
	com.pref.icc.autoconversion = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_icc_autoconversion")));
}

static void update_FITS_options_preferences() {
	com.pref.comp.fits_enabled = !(siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("comp_fits_disabled_radio")))));
	com.pref.comp.fits_method = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combobox_comp_fits_method")));
	com.pref.comp.fits_quantization = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_comp_fits_quantization")));
	com.pref.comp.fits_hcompress_scale = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_comp_fits_hcompress_scale")));

	com.pref.rgb_aladin = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_aladin"))));
	com.pref.use_checksum = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_fits_chksum"))));
	com.pref.binning_update = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_binned_update"))));

	/* IDs aligned with settings_window.ui combobox_ext items */
	static const char *const fits_ext_ids[] = {".fit", ".fits", ".fts"};
	guint ext_idx = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combobox_ext")));
	const gchar *ext = (ext_idx < G_N_ELEMENTS(fits_ext_ids)) ? fits_ext_ids[ext_idx] : ".fit";
	com.pref.ext = g_strdup(ext);

	com.pref.force_16bit = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("combobox_type"))) == 0;
	com.pref.fits_save_icc = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_fits_save_icc"))));

	com.pref.allow_heterogeneous_fitseq = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbox_heterogeneous_fitseq"))));
}

static void update_performances_preferences() {
	/* Phase 17.6: GtkRadioButton + gtk_radio_button_get_group are gone
	 * in GTK4.  The two .ui radio buttons are now GtkCheckButtons in
	 * the same group; we just check both directly. */
	GtkWidget *amount = lookup_widget("memfixed_radio");
	GtkWidget *ratio  = lookup_widget("memfreeratio_radio");
	if (amount && siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(amount)))) {
		com.pref.mem_mode = AMOUNT;
	} else if (ratio && siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(ratio)))) {
		com.pref.mem_mode = RATIO;
	}

	com.pref.memory_ratio = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_mem_ratio")));
	com.pref.memory_amount = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_mem_amount")));
	com.pref.fftw_conf.timelimit = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("pref_fftw_plan_timelimit")));
	com.pref.fftw_conf.strategy = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_fftw_plan_strategy")));
	com.pref.fftw_conf.multithreaded = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_fftw_multithreaded"))));
	com.pref.fftw_conf.fft_cutoff = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("pref_conv_min_fft")));
	int max_slice_size = gtk_drop_down_get_selected(GTK_DROP_DOWN(lookup_widget("pref_max_slice_size")));
	com.pref.max_slice_size = max_slice_size == 0 ? 32769 : 1 << (max_slice_size + 8);
	int bitdepth = (int) gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spin_hd_bitdepth")));
	com.pref.hd_bitdepth = bitdepth;
	com.pref.lazy_tile_cache_mb = (int) gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spin_lazy_tile_cache")));
}

static void update_misc_preferences() {
	GtkWidget *swap_dir = lookup_widget("filechooser_swap");
	GtkWidget *starnet_exe = lookup_widget("filechooser_starnet");
	GtkWidget *starnet_weights = lookup_widget("filechooser_starnet_weights");
	GtkWidget *graxpert_exe = lookup_widget("filechooser_graxpert");

	com.pref.swap_dir = siril_file_chooser_get_filename(swap_dir);

	com.pref.starnet_exe = siril_file_chooser_get_filename(starnet_exe);
	com.pref.starnet_weights = siril_file_chooser_get_filename(starnet_weights);
	com.pref.graxpert_path = siril_file_chooser_get_filename(graxpert_exe);
#ifdef OS_OSX
	if (g_str_has_suffix(com.pref.graxpert_path, ".app"))
		str_append(&com.pref.graxpert_path, "/Contents/MacOS/GraXpert");
#endif

	com.pref.gui.silent_quit = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskQuit"))));
	com.pref.gui.silent_linear = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskSave"))));

	const gchar *copy = gtk_editable_get_text(GTK_EDITABLE(lookup_widget("miscCopyright")));
	com.pref.copyright = g_strdup(copy);

	com.pref.check_update = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskUpdateStartup"))));
	com.pref.gui.enable_roi_warning = siril_toggle_get_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscHideInfoROI")))) ? FALSE : TRUE;
}

void on_photometry_force_radius_button_toggled(GtkCheckButton *button, gpointer user_data) {
//	GtkWidget *spin = (GtkWidget *)user_data;
	GtkWidget *spin1 = lookup_widget("spinAperture");
	GtkWidget *spin2 = lookup_widget("spinRadRatio");
	gtk_widget_set_sensitive(spin1, !siril_toggle_get_active(GTK_WIDGET(button)));
	gtk_widget_set_sensitive(spin2, siril_toggle_get_active(GTK_WIDGET(button)));
}

void initialize_path_directory(const gchar *path) {
	GtkWidget *swap_dir = lookup_widget("filechooser_swap");
	if (path && path[0] != '\0') {
		siril_file_chooser_set_filename (swap_dir, path);
	} else {
		siril_file_chooser_set_filename (swap_dir, g_get_tmp_dir());
	}
}

void initialize_graxpert_executable(gchar *path) {
	GtkWidget *graxpert_exe = lookup_widget("filechooser_graxpert");
#ifdef OS_OSX
	const gchar *suffix = "/Contents/MacOS/GraXpert";
	if (path && g_str_has_suffix(path, suffix)) {
		gsize len = strlen(path) - strlen(suffix);
		gchar *new_path = g_strndup(path, len);
		siril_file_chooser_set_filename(graxpert_exe, new_path);
		g_free(new_path);
		return;
	}
#endif

	if (path && path[0] != '\0') {
		siril_file_chooser_set_filename (graxpert_exe, path);
	}
}

void initialize_starnet_executable(gchar *path) {
#ifdef HAVE_LIBTIFF
	GtkWidget *starnet_exe = lookup_widget("filechooser_starnet");
	GtkWidget *starnet_weights_reset = GTK_WIDGET(lookup_widget("starnet_weights_clear"));
	GtkWidget *starnet_weights = GTK_WIDGET(lookup_widget("filechooser_starnet_weights"));
	if (path && path[0] != '\0') {
		siril_file_chooser_set_filename (starnet_exe, path);
		if (starnet_executablecheck(path) & TORCH) {
			gtk_widget_set_sensitive(starnet_weights, TRUE);
			gtk_widget_set_sensitive(starnet_weights_reset, TRUE);
		} else {
			gtk_widget_set_sensitive(starnet_weights, FALSE);
			gtk_widget_set_sensitive(starnet_weights_reset, FALSE);
		}
	} else {
		gtk_widget_set_sensitive(starnet_weights, FALSE);
		gtk_widget_set_sensitive(starnet_weights_reset, FALSE);
	}
#endif
}

void initialize_starnet_weights(gchar *path) {
	GtkWidget *starnet_weights = lookup_widget("filechooser_starnet_weights");
	if (path && path[0] != '\0') {
		siril_file_chooser_set_filename (starnet_weights, path);
	}
}

void initialize_asnet_directory(const gchar *path) {
	GtkWidget *asnet_dir = lookup_widget("filechooser_asnet");
	reset_asnet_version();
	if (path && path[0] != '\0') {
		siril_file_chooser_set_filename (asnet_dir, path);
	}
}

void on_asnet_clear_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *asnet_dir = lookup_widget("filechooser_asnet");
	siril_file_chooser_set_filename(asnet_dir, "\0");
	reset_asnet_version();
}

void on_button_reset_swap_clicked(GtkButton *button, gpointer user_data) {
	reset_swapdir();
}

void on_combobox_comp_fits_method_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *box = GTK_DROP_DOWN(obj);
	(void)pspec;
	GtkWidget *hcompress_scale_spin = lookup_widget("spinbutton_comp_fits_hcompress_scale");
	GtkSpinButton *button = (GtkSpinButton *)user_data;
	gint method = gtk_drop_down_get_selected(GTK_DROP_DOWN(box));
	if (gtk_spin_button_get_value(button) == 0.0) {
		gtk_spin_button_set_value(button, 16.0);
	}
	gtk_widget_set_sensitive(hcompress_scale_spin, (method == HCOMPRESS_COMP) ? TRUE : FALSE);
}

void on_mem_radio_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	GtkCheckButton *ratio = GTK_CHECK_BUTTON(lookup_widget("memfreeratio_radio")),
			*amount = GTK_CHECK_BUTTON(lookup_widget("memfixed_radio"));
	GtkWidget *ratio_spin = lookup_widget("spinbutton_mem_ratio"),
		  *amount_spin = lookup_widget("spinbutton_mem_amount");
	if (!siril_toggle_get_active(GTK_WIDGET(togglebutton))) return;

	gtk_widget_set_sensitive(ratio_spin, togglebutton == ratio);
	gtk_widget_set_sensitive(amount_spin, togglebutton == amount);
}

void on_comp_fits_radio_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	GtkCheckButton *disabled = GTK_CHECK_BUTTON(lookup_widget("comp_fits_disabled_radio"));
	GtkWidget *method_box = lookup_widget("combobox_comp_fits_method"),
		*quantization_spin = lookup_widget("spinbutton_comp_fits_quantization"),
		*tilex_spin = lookup_widget("spinbutton_comp_fits_tileX"),
		*tiley_spin = lookup_widget("spinbutton_comp_fits_tileY"),
		*hcompress_scale_spin = lookup_widget("spinbutton_comp_fits_hcompress_scale");
	if (!siril_toggle_get_active(GTK_WIDGET(togglebutton))) return;

	gint method = gtk_drop_down_get_selected(GTK_DROP_DOWN(method_box));
	gtk_widget_set_sensitive(method_box, togglebutton != disabled);
	gtk_widget_set_sensitive(quantization_spin, togglebutton != disabled);
	gtk_widget_set_sensitive(tilex_spin, FALSE);
	gtk_widget_set_sensitive(tiley_spin, FALSE);
	gtk_widget_set_sensitive(hcompress_scale_spin, (method == HCOMPRESS_COMP) ? togglebutton != disabled : FALSE);
}

void on_filechooser_swap_file_set(GtkWidget *fileChooser, gpointer user_data) {
	GtkWidget *swap_dir = fileChooser;
	gchar *dir;

	dir = siril_file_chooser_get_filename (swap_dir);

	if (g_access(dir, W_OK)) {
		gchar *msg = siril_log_error(_("You don't have permission to write in this directory: %s\n"), dir);
		siril_message_dialog( GTK_MESSAGE_ERROR, _("Error"), msg);
		g_rw_lock_reader_lock(&com.pref_rwlock);
		siril_file_chooser_set_filename(swap_dir, com.pref.swap_dir);
		g_rw_lock_reader_unlock(&com.pref_rwlock);
		return;
	}
	if (sw_dir) {
		g_free(sw_dir);
		sw_dir = siril_file_chooser_get_filename(swap_dir);
	}
}

void on_filechooser_starnet_file_set(GtkWidget *fileChooser, gpointer user_data) {
#ifdef HAVE_LIBTIFF
	GtkWidget *starnet_exe = fileChooser;
	gchar *path;

	path = siril_file_chooser_get_filename (starnet_exe);
	printf("%s\n", path);
	if (g_access(path, X_OK)) {
		gchar *msg = siril_log_error(_("You don't have permission to execute this file: %s\n"), path);
		siril_message_dialog( GTK_MESSAGE_ERROR, _("Error"), msg);
		g_rw_lock_reader_lock(&com.pref_rwlock);
		siril_file_chooser_set_filename(starnet_exe, com.pref.starnet_exe);
		g_rw_lock_reader_unlock(&com.pref_rwlock);
		return;
	}
	st_version = starnet_executablecheck(path);
	GtkWidget *starnet_weights_reset = GTK_WIDGET(lookup_widget("starnet_weights_clear"));
	GtkWidget *starnet_weights = GTK_WIDGET(lookup_widget("filechooser_starnet_weights"));
	if (st_version & TORCH) {
		gtk_widget_set_sensitive(starnet_weights, TRUE);
		gtk_widget_set_sensitive(starnet_weights_reset, TRUE);
	} else {
		gtk_widget_set_sensitive(starnet_weights, FALSE);
		gtk_widget_set_sensitive(starnet_weights_reset, FALSE);
	}
#endif
}

void on_starnet_weights_clear_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *starnet_weights = lookup_widget("filechooser_starnet_weights");
	if (st_weights) {
		g_free(st_weights);
	}
	st_weights = g_strdup("\0");
	siril_file_chooser_set_filename(starnet_weights, st_weights);
}

void on_filechooser_starnet_weights_file_set(GtkWidget *fileChooser, gpointer user_data) {
	GtkWidget *starnet_weights = fileChooser;
	gchar *path;

	path = siril_file_chooser_get_filename (starnet_weights);

	if (g_access(path, R_OK)) {
		gchar *msg = siril_log_error(_("You don't have permission to read this file: %s\n"), path);
		siril_message_dialog( GTK_MESSAGE_ERROR, _("Error"), msg);
		g_rw_lock_reader_lock(&com.pref_rwlock);
		siril_file_chooser_set_filename(starnet_weights, com.pref.starnet_weights);
		g_rw_lock_reader_unlock(&com.pref_rwlock);
		return;
	}
	if (st_weights) {
		g_free(st_weights);
	}
	st_weights = siril_file_chooser_get_filename(starnet_weights);
}

void on_show_preview_button_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	GtkWidget *label = lookup_widget("thumbnails_label_size");
	GtkWidget *box = lookup_widget("thumbnails_box_size");

	gtk_widget_set_sensitive(label, siril_toggle_get_active(GTK_WIDGET(togglebutton)));
	gtk_widget_set_sensitive(box, siril_toggle_get_active(GTK_WIDGET(togglebutton)));
}

void on_play_introduction_clicked(GtkButton *button, gpointer user_data) {
	siril_close_dialog("settings_window");
	start_intro_script();
}

void on_reload_script_button_clicked(GtkButton *button, gpointer user_data) {
	g_thread_unref(g_thread_new("refresh_scripts", refresh_scripts_in_thread, NULL));
}

//static gboolean from_prefs_init = FALSE;	// NOT USED, SHOULD BE DELETED

void update_preferences_from_model() {
	siril_log_debug("updating preferences GUI from settings data\n");
	g_rw_lock_reader_lock(&com.pref_rwlock);
	preferences *pref = &com.pref;
	/* tab FITS/SER Debayer */
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_use_header"))), pref->debayer.use_bayer_header);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("comboBayer_pattern")), pref->debayer.bayer_pattern);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("comboBayer_inter")), pref->debayer.bayer_inter);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("xbayeroff_spin")), pref->debayer.xbayeroff);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("ybayeroff_spin")), pref->debayer.ybayeroff);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combo_roworder")),  pref->debayer.orientation);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("xtranspass_spin")), pref->debayer.xtrans_passes);

	/* tab FITS Options */
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_aladin"))), pref->rgb_aladin);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_fits_chksum"))), pref->use_checksum);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_binned_update"))), pref->binning_update);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("comp_fits_disabled_radio"))), !pref->comp.fits_enabled);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("comp_fits_enabled_radio"))), pref->comp.fits_enabled);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combobox_comp_fits_method")), pref->comp.fits_method);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_comp_fits_quantization")), pref->comp.fits_quantization);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_comp_fits_hcompress_scale")), pref->comp.fits_hcompress_scale);
	{
		/* IDs aligned with settings_window.ui combobox_ext items */
		static const char *const fits_ext_ids[] = {".fit", ".fits", ".fts"};
		const gchar *want = (pref->ext == NULL) ? ".fit" : pref->ext;
		guint sel = 0;
		for (guint i = 0; i < G_N_ELEMENTS(fits_ext_ids); i++) {
			if (g_strcmp0(want, fits_ext_ids[i]) == 0) { sel = i; break; }
		}
		gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combobox_ext")), sel);
	}
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combobox_type")), pref->force_16bit ? 0 : 1);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_fits_save_icc"))), pref->fits_save_icc);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbox_heterogeneous_fitseq"))), pref->allow_heterogeneous_fitseq);

	/* tab Astrometry */
	fill_astrometry_catalogue(pref->gui.catalog);

	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("compass_combobox")), pref->gui.position_compass);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("wcs_formalism_combobox")), pref->wcs_formalism);
	if (pref->catalogue_paths[0] && (g_file_test(pref->catalogue_paths[0], G_FILE_TEST_EXISTS))) {
		GtkWidget *button = lookup_widget("localcatalogue_path1");
		siril_file_chooser_set_filename(button, pref->catalogue_paths[0]);
	}
	if (pref->catalogue_paths[1] && (g_file_test(pref->catalogue_paths[1], G_FILE_TEST_EXISTS))) {
		GtkWidget *button = lookup_widget("localcatalogue_path2");
		siril_file_chooser_set_filename(button, pref->catalogue_paths[1]);
	}
	if (pref->catalogue_paths[2] && (g_file_test(pref->catalogue_paths[2], G_FILE_TEST_EXISTS))) {
		GtkWidget *button = lookup_widget("localcatalogue_path3");
		siril_file_chooser_set_filename(button, pref->catalogue_paths[2]);
	}
	if (pref->catalogue_paths[3] && (g_file_test(pref->catalogue_paths[3], G_FILE_TEST_EXISTS))) {
		GtkWidget *button = lookup_widget("localcatalogue_path4");
		siril_file_chooser_set_filename(button, pref->catalogue_paths[3]);
	}
	if (pref->catalogue_paths[4] && (g_file_test(pref->catalogue_paths[4], G_FILE_TEST_EXISTS))) {
		GtkWidget *button = lookup_widget("localcatalogue_path5");
		siril_file_chooser_set_filename(button, pref->catalogue_paths[4]);
	}
	if (pref->catalogue_paths[5] && (g_file_test(pref->catalogue_paths[5], G_FILE_TEST_EXISTS))) {
		GtkWidget *button = lookup_widget("localcatalogue_path6");
		siril_file_chooser_set_filename(button, pref->catalogue_paths[5]);
	}
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("gaia_cache_duration")), pref->astrometry.gaia_cache_duration);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_asnet_sip_order")), pref->astrometry.sip_correction_order);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_sampling_tolerance")), pref->astrometry.percent_scale_range);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_asnet_radius")), pref->astrometry.radius_degrees);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_asnet_xyls"))), pref->astrometry.keep_xyls_files);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_asnet_wcs"))), pref->astrometry.keep_wcs_files);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_asnet_max_sec")), pref->astrometry.max_seconds_run);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("astrometry_update_fields"))), pref->astrometry.update_default_scale);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_asnet_show_output"))), pref->astrometry.show_asnet_output);
	if (pref->astrometry.default_obscode)
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("obscode_entry")), pref->astrometry.default_obscode);

	/* tab Pre-processing */
	if (pref->prepro.bias_lib) {
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("biaslib_entry")), pref->prepro.bias_lib);
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_bias"))), pref->prepro.use_bias_lib);
	}

	if (pref->prepro.dark_lib) {
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("darklib_entry")), pref->prepro.dark_lib);
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_dark"))), pref->prepro.use_dark_lib);
	}

	if (pref->prepro.flat_lib) {
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("flatlib_entry")), pref->prepro.flat_lib);
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_flat"))), pref->prepro.use_flat_lib);
	}

	if (pref->prepro.disto_lib) {
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("distolib_entry")), pref->prepro.disto_lib);
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_disto"))), pref->prepro.use_disto_lib);
	}

	if (pref->prepro.stack_default) {
		gtk_editable_set_text(GTK_EDITABLE(lookup_widget("stack_default_entry")), pref->prepro.stack_default);
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_stack"))), pref->prepro.use_stack_default);
	} else {
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("check_button_pref_stack"))), FALSE);
	}

	/* X-Trans AF / sample geometry entries — populated by the dedicated
	 * helper in fix_xtrans_af.c so the cached widget statics there get
	 * used, and so unconfigured (w==0, h==0) prefs leave the entries
	 * blank rather than displaying "0". */
	init_xtrans_ui_pixels();

	/* tab Photometry */
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinOuter")), pref->phot_set.outer);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinInner")), pref->phot_set.inner);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("photometry_force_radius_button"))), !pref->phot_set.force_radius);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinRadRatio")), pref->phot_set.auto_aperture_factor);
	gtk_widget_set_sensitive(lookup_widget("spinRadRatio"), !pref->phot_set.force_radius);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinAperture")), pref->phot_set.aperture);
	gtk_widget_set_sensitive(lookup_widget("spinAperture"), pref->phot_set.force_radius);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinGain")), pref->phot_set.gain);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinMinPhot")), pref->phot_set.minval);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinMaxPhot")), pref->phot_set.maxval);

	/* tab Analysis Tools */
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinpanel")), pref->analysis.mosaic_panel);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinwindow")), pref->analysis.mosaic_window);

	/* tab User Interface */
	siril_language_fill_combo(pref->lang);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combo_theme")), pref->gui.combo_theme);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("pref_fontsize")), pref->gui.font_scale);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_iconstyle"))), pref->gui.icon_symbolic);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("rememberWindowsCheck"))), pref->gui.remember_windows);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("show_preview_button"))), pref->gui.show_thumbnails);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("thumbnails_box_size")), pref->gui.thumbnail_size == 512 ? 2 : (pref->gui.thumbnail_size == 256 ? 1 : 0));
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_default_stf")), pref->gui.default_rendering_mode);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_ui_mask_tint"))), pref->gui.mask_tints_vports);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_ui_mask_bitpix")), pref->default_mask_bitpix == 8 ? 0 : pref->default_mask_bitpix == 16 ? 1 : pref->default_mask_bitpix == 32 ? 2 : 3);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_default_histo_mode")), pref->gui.display_histogram_mode);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_ui_roimode")), pref->gui.roi_mode);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_ui_mmb_zoom")), pref->gui.mmb_action);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_mouse_speed_limit")), pref->gui.mouse_speed_limit);

	GdkRGBA color;
	gdk_rgba_parse(&color, pref->gui.config_colors.color_bkg_samples);
	gtk_color_dialog_button_set_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_bkg")), &color);
	gdk_rgba_parse(&color, pref->gui.config_colors.color_std_annotations);
	gtk_color_dialog_button_set_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_std_annot")), &color);
	gdk_rgba_parse(&color, pref->gui.config_colors.color_dso_annotations);
	gtk_color_dialog_button_set_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_dso_annot")), &color);
	gdk_rgba_parse(&color, pref->gui.config_colors.color_sso_annotations);
	gtk_color_dialog_button_set_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_sso_annot")), &color);
	gdk_rgba_parse(&color, pref->gui.config_colors.color_tmp_annotations);
	gtk_color_dialog_button_set_rgba(
		GTK_COLOR_DIALOG_BUTTON(lookup_widget("color_button_tmp_annot")), &color);
	/* tab Color Management.  These widgets are Phase 18 path-picker
	 * buttons; siril_file_chooser_set_filename reads/writes their
	 * stashed siril-path data. */
	GtkWidget *button = lookup_widget("pref_custom_monitor_profile");
	if (pref->icc.icc_path_monitor && (g_file_test(pref->icc.icc_path_monitor, G_FILE_TEST_EXISTS))) {
		siril_file_chooser_set_filename(button, pref->icc.icc_path_monitor);
	}
	button = lookup_widget("pref_soft_proofing_profile");
	if (pref->icc.icc_path_soft_proof && (g_file_test(pref->icc.icc_path_soft_proof, G_FILE_TEST_EXISTS))) {
		siril_file_chooser_set_filename(button, pref->icc.icc_path_soft_proof);
	}
	button = lookup_widget("custom_icc_standard_trc");
	if (pref->icc.custom_icc_trc && (g_file_test(pref->icc.custom_icc_trc, G_FILE_TEST_EXISTS))) {
		siril_file_chooser_set_filename(button, pref->icc.custom_icc_trc);
	}
	button = lookup_widget("custom_gray_icc_matching_trc");
	if (pref->icc.custom_icc_gray && (g_file_test(pref->icc.custom_icc_gray, G_FILE_TEST_EXISTS))) {
		siril_file_chooser_set_filename(button, pref->icc.custom_icc_gray);
	}
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combo_rendering_intent")), pref->icc.rendering_intent);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combo_export_intent")), pref->icc.export_intent);
	initialize_icc_preferences_widgets();
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("working_gamut")), pref->icc.working_gamut);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("export_profile_8bit")), pref->icc.export_8bit_method);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("export_profile_16bit")), pref->icc.export_16bit_method);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combo_cmf_pref")), pref->icc.cmf);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("custom_monitor_profile_active"))), pref->icc.custom_monitor_profile_active);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("custom_proofing_profile_active"))), pref->icc.soft_proofing_profile_active);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("default_to_srgb"))), pref->icc.default_to_srgb);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_rendering_bpc"))), pref->icc.rendering_bpc);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_pedantic_linear"))), pref->icc.pedantic_linear);
	icc_assign_type autoassign = pref->icc.autoassignment;
	gboolean val = autoassign & ICC_ASSIGN_ON_LOAD;
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_load"))), val);
	val = autoassign & ICC_ASSIGN_ON_STACK;
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_stack"))), val);
	val = autoassign & ICC_ASSIGN_ON_STRETCH;
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_stretch"))), val);
	val = autoassign & ICC_ASSIGN_ON_COMPOSITION;
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_on_composition"))), val);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_icc_assign_never"))), !pref->icc.autoassignment);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_icc_autoconversion")), pref->icc.autoconversion);

	/* tab Scripts */
	pref->gui.script_path = set_list_to_preferences_dialog(pref->gui.script_path);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskScript"))), pref->gui.warn_scripts_run);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("script_check_version"))), pref->script_check_requires);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_use_gitscripts"))), pref->use_scripts_repository);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_script_automatic_updates"))), pref->auto_script_update);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("spcc_repo_enable"))), pref->spcc.use_spcc_repository);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("spcc_repo_sync_at_startup"))), pref->spcc.auto_spcc_update);

	/* Scripts tab — Python interpreter section. Guard against re-entry so the
	 * programmatic widget set doesn't trigger our confirmation dialog. */
	python_uv_widget_reverting = TRUE;
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_python_uv_managed"))),
			pref->python.uv_managed);
	guint want_idx = uv_python_index_from_version(pref->python.uv_python_version);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("combo_python_uv_version")), want_idx);
	g_free(python_uv_version_confirmed);
	python_uv_version_confirmed = g_strdup(uv_python_version_from_index(want_idx));
	python_uv_widget_reverting = FALSE;

	// Greyed-out controls + warning label when uv isn't available. The
	// pref values still get saved as the user set them; they just won't
	// take effect until uv is installed.
	gboolean uv_present = uv_executable_available();
	gtk_widget_set_sensitive(lookup_widget("checkbutton_python_uv_managed"), uv_present);
	gtk_widget_set_sensitive(lookup_widget("box_python_version_row"), uv_present && pref->python.uv_managed);
	gtk_widget_set_visible(lookup_widget("label_python_uv_missing_warn"), !uv_present);

	/* tab Performances */
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("memfreeratio_radio"))), pref->mem_mode == RATIO);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("memfixed_radio"))), pref->mem_mode == AMOUNT);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_mem_ratio")), pref->memory_ratio);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_mem_amount")), pref->memory_amount);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_hd_bitdepth")), pref->hd_bitdepth);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("spin_lazy_tile_cache")), pref->lazy_tile_cache_mb);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("pref_fftw_plan_timelimit")), pref->fftw_conf.timelimit);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_fftw_plan_strategy")), pref->fftw_conf.strategy);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_fftw_multithreaded"))), pref->fftw_conf.multithreaded);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(lookup_widget("pref_conv_min_fft")), pref->fftw_conf.fft_cutoff);
	int n = pref->max_slice_size, max_slice_size = 0;
	// Shift n to the right until it becomes 1
	if (n > 0) {
		while (n > 1) {
			n >>= 1;
			max_slice_size++;
		}
	}
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("pref_max_slice_size")), pref->max_slice_size > 32768 || pref->max_slice_size <= 0 ? 0 : max_slice_size - 8);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("pref_match_drizzweight_bitpix"))), pref->drizz_weight_match_bitpix);

	/* tab Miscellaneous */
	initialize_path_directory(pref->swap_dir);
	initialize_graxpert_executable(pref->graxpert_path);
	initialize_starnet_executable(pref->starnet_exe);
	initialize_starnet_weights(pref->starnet_weights);
	initialize_asnet_directory(pref->asnet_dir);

	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscHideInfoROI"))), pref->gui.enable_roi_warning ? FALSE : TRUE);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskQuit"))), pref->gui.silent_quit);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskSave"))), pref->gui.silent_linear);
	gtk_editable_set_text(GTK_EDITABLE(lookup_widget("miscCopyright")), pref->copyright == NULL ? "" : pref->copyright);
	siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("miscAskUpdateStartup"))), pref->check_update);
	g_rw_lock_reader_unlock(&com.pref_rwlock);
}

static void set_icc_filechooser_directories() {
	GtkWidget *fc2 = lookup_widget("custom_icc_standard_trc");
	GtkWidget *fc3 = lookup_widget("custom_gray_icc_matching_trc");
	GtkWidget *fc4 = lookup_widget("pref_custom_monitor_profile");
	GtkWidget *fc5 = lookup_widget("pref_soft_proofing_profile");

	siril_file_chooser_set_current_folder_path(fc2, default_system_icc_path());
	siril_file_chooser_set_current_folder_path(fc3, default_system_icc_path());
	siril_file_chooser_set_current_folder_path(fc4, default_system_icc_path());
	siril_file_chooser_set_current_folder_path(fc5, default_system_icc_path());
	GtkDropDown* combo = (GtkDropDown*) lookup_widget("working_gamut");
	on_working_gamut_changed(G_OBJECT(combo), NULL, NULL);
}

static void dump_ui_to_global_var() {
	siril_log_debug("updating settings from preferences GUI\n");
	/* tab FITS/SER Debayer */
	update_debayer_preferences();
	/* tab FITS Options */
	update_FITS_options_preferences();
	/* tab Astrometry */
	update_astrometry_preferences();
	/* tab Pre-processing */
	update_prepro_preferences();
	/* tab Analysis Tools */
	update_analysis_preferences();
	/* tab Photometry */
	update_photometry_preferences();
	/* tab Scripts */
	update_scripts_preferences();
	/* tab User Interface */
	update_user_interface_preferences();
	/* tab Color Management */
	update_color_management_preferences();
	/* tab Performances */
	update_performances_preferences();
	/* tab Miscellaneous */
	update_misc_preferences();
}

void on_pref_use_gitscripts_toggled(GtkCheckButton *button, gpointer user_data);
void on_spcc_repo_enable_toggled(GtkCheckButton *button, gpointer user_data);

void on_settings_window_show(GtkWidget *widget, gpointer user_data) {
	siril_log_debug("show preferences window: updating it\n");
	/* GTK4 path-chooser buttons: each GtkButton opens a GtkFileDialog on
	 * click and stores the selected path on itself.  Wire-up is idempotent
	 * across show/hide cycles. */
	siril_path_button_init(lookup_widget("localcatalogue_path1"),
	                       _("Select namedstars.dat"), "filter_namedstars", FALSE);
	siril_path_button_init(lookup_widget("localcatalogue_path2"),
	                       _("Select unnamedstars.dat"), "filter_unnamedstars", FALSE);
	siril_path_button_init(lookup_widget("localcatalogue_path3"),
	                       _("Select deepstars.dat"), "filter_deepstars", FALSE);
	siril_path_button_init(lookup_widget("localcatalogue_path4"),
	                       _("Select USNO-NOMAD-1e8.dat"), "filter_USNO-NOMAD-1e8", FALSE);
	siril_path_button_init(lookup_widget("localcatalogue_path5"),
	                       _("Select Gaia astrometric extract"), NULL, FALSE);
	siril_path_button_init(lookup_widget("localcatalogue_path6"),
	                       _("Select Gaia photometric folder"), NULL, TRUE);
	siril_path_button_init(lookup_widget("custom_icc_standard_trc"),
	                       _("Select ICC profile"), "icc_filter", FALSE);
	siril_path_button_init(lookup_widget("custom_gray_icc_matching_trc"),
	                       _("Select Gray ICC profile"), "icc_filter", FALSE);
	siril_path_button_init(lookup_widget("pref_custom_monitor_profile"),
	                       _("Select monitor profile"), "icc_filter", FALSE);
	siril_path_button_init(lookup_widget("pref_soft_proofing_profile"),
	                       _("Select soft-proofing profile"), "icc_filter", FALSE);
	siril_path_button_init(lookup_widget("filechooser_graxpert"),
	                       _("Select GraXpert executable"), NULL, FALSE);
	siril_path_button_init(lookup_widget("filechooser_starnet"),
	                       _("Select StarNet executable"), NULL, FALSE);
	siril_path_button_init(lookup_widget("filechooser_starnet_weights"),
	                       _("Select StarNet weights"), NULL, FALSE);
	GtkLabel* spcc_path_label = GTK_LABEL(lookup_widget("label_spcc_repo_path"));
	gtk_label_set_text(spcc_path_label, siril_get_spcc_repo_path());

	set_icc_filechooser_directories();
	g_signal_handlers_block_by_func(G_OBJECT(lookup_widget("spcc_repo_enable")), on_spcc_repo_enable_toggled, NULL);
	g_signal_handlers_block_by_func(G_OBJECT(lookup_widget("pref_use_gitscripts")), on_pref_use_gitscripts_toggled, NULL);
	update_preferences_from_model();
	g_signal_handlers_unblock_by_func(G_OBJECT(lookup_widget("spcc_repo_enable")), on_spcc_repo_enable_toggled, NULL);
	g_signal_handlers_unblock_by_func(G_OBJECT(lookup_widget("pref_use_gitscripts")), on_pref_use_gitscripts_toggled, NULL);
	scripts_updated = FALSE;
#ifndef HAVE_LIBGIT2
	hide_git_widgets();
#else
	fill_script_repo_tree(FALSE);
#endif
#ifndef HAVE_SQLITE
	// Hide UI for cache if we don't have SQLite
	gtk_widget_set_visible(lookup_widget("gaia_cache_duration_label"), FALSE);
	gtk_widget_set_visible(lookup_widget("gaia_cache_duration"), FALSE);
	gtk_widget_set_visible(lookup_widget("clean_gaia_cache"), FALSE);
#endif
}

gboolean check_pref_sanity(gchar **error) {
	gchar *err = NULL;
	/* check for photometry pref */
	double in = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinInner")));
	double out = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinOuter")));

	if (in >= out) {
		err = _("Inner radius value must be less than outer. Please change the value in Photometry tab.");
		*error = err;
		return FALSE;
	}
	/* check for FITS pref */
	gdouble quantization = gtk_spin_button_get_value(GTK_SPIN_BUTTON(lookup_widget("spinbutton_comp_fits_quantization")));
	if (quantization == 0.0) {
		GtkDropDown *combo = GTK_DROP_DOWN(lookup_widget("combobox_comp_fits_method"));
		if (gtk_drop_down_get_selected(combo) != GZIP1_COMP && gtk_drop_down_get_selected(combo) != GZIP2_COMP) {
			err = _("Setting quantization to 0 has only a sense with a GZIP compression "
									"and GZIP 2 often produces better compression of floating­point images.");
			*error = err;
			return FALSE;
		}
	}
	return TRUE;
}

void on_apply_settings_button_clicked(GtkButton *button, gpointer user_data) {
	gchar *err;
	if (!check_pref_sanity(&err)) {
		siril_message_dialog(GTK_MESSAGE_ERROR, _("Wrong value"), err);
	} else {
		g_rw_lock_writer_lock(&com.pref_rwlock);
		free_preferences(&com.pref);
		dump_ui_to_global_var();
		g_rw_lock_writer_unlock(&com.pref_rwlock);
#ifdef HAVE_LIBGIT2
		if (!com.pref.use_scripts_repository)
			on_disable_gitscripts();
		else
#endif
			g_thread_unref(g_thread_new("refresh_script_menu", refresh_script_menu_in_thread, GINT_TO_POINTER((int) scripts_updated)));
			// To update the UI with scripts from the repo
			// Note this line is part of the if/else with the #ifdef and always runs
			// otherwise. This is intentional.
		scripts_updated = FALSE;
		initialize_FITS_name_entries();	// To update UI with new preferences
		refresh_star_list();		// To update star list with new preferences
		if (com.found_object)
			refresh_found_objects();
		save_main_window_state(NULL);
		writeinitfile();
		validate_custom_profiles(); // Validate and load custom ICC profiles
		if (update_custom_gamut) {
			update_profiles_after_gamut_change();
			update_custom_gamut = FALSE;
		}
		refresh_icc_transforms();
		if (single_image_is_loaded() || sequence_is_loaded()) {
			color_manage(gfit, gfit->color_managed);
			gfit_modified_update_gui();
		}
		siril_close_dialog("settings_window");
		update_reg_interface(TRUE); // To update UI with new preferences
	}
	// Update the GtkSourceView theme, if the widget has been created, in case
	// the Siril theme has been changed
	if (code_view_exists())
		set_code_view_theme();
}

void on_cancel_settings_button_clicked(GtkButton *button, gpointer user_data) {
	update_custom_gamut = FALSE;
	siril_close_dialog("settings_window");
}

void on_reset_settings_button_clicked(GtkButton *button, gpointer user_data) {
	int confirm = siril_confirm_dialog(_("Reset all preferences"),
			_("Do you really want to reset all preferences to default value?"),
			_("Reset Preferences"));
	if (confirm) {
		g_rw_lock_writer_lock(&com.pref_rwlock);
		initialize_default_settings();
		g_rw_lock_writer_unlock(&com.pref_rwlock);
		update_preferences_from_model();
		update_custom_gamut = FALSE;
	}
}

void on_settings_window_hide(GtkWidget *widget, gpointer user_data) {
	update_custom_gamut = FALSE;
}

void on_external_preferred_profile_set(GtkWidget *chooser, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(lookup_widget("working_gamut"));
	if (gtk_drop_down_get_selected(combo) != 2)
		siril_message_dialog(GTK_MESSAGE_INFO, _("External ICC Profiles"), _("In order to "
		"use ICC profiles loaded from files as your preferred color space you must set "
		"the Preferred Color Space drop-down to \"From files\"."));
}

void on_working_gamut_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)obj;
	(void)pspec;
	update_custom_gamut = TRUE;
}

void on_combo_rendering_intent_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	if (gtk_drop_down_get_selected(combo) == INTENT_ABSOLUTE_COLORIMETRIC) {
		siril_toggle_set_active(GTK_WIDGET(GTK_CHECK_BUTTON(lookup_widget("checkbutton_rendering_bpc"))), FALSE);
	}
}

gchar *get_swap_dir() {
	GtkWidget *swap_dir = lookup_widget("filechooser_swap");

	if (sw_dir == NULL) {
		sw_dir = siril_file_chooser_get_filename(swap_dir);
	}
	return sw_dir;
}

/* Scripts tab — uv-managed Python checkbox. Toggling the box rebuilds the
 * interpreter on the next venv-init, so warn the user and offer an undo.
 * On Cancel we revert the widget; the re-entry guard prevents the revert
 * from re-firing this same handler. */
void on_checkbutton_python_uv_managed_toggled(GtkCheckButton *button, gpointer user_data) {
	if (python_uv_widget_reverting)
		return;
	gboolean now_on = siril_toggle_get_active(GTK_WIDGET(button));
	gboolean was_on = com.pref.python.uv_managed;
	if (now_on == was_on)
		return;  // nothing to do (e.g. opened-then-closed without changing)

	int confirm = siril_confirm_dialog(
			_("Change Python interpreter source?"),
			_("Are you sure you want to do this? You will need to rebuild "
			  "Python environments to enact the change, which may involve "
			  "significant dependency downloads."),
			_("Change"));
	if (!confirm) {
		python_uv_widget_reverting = TRUE;
		siril_toggle_set_active(GTK_WIDGET(button), was_on);
		python_uv_widget_reverting = FALSE;
		return;
	}

	// Combo sensitivity follows the new state. (Visibility of the warning
	// label is uv-availability driven and doesn't change here.)
	gtk_widget_set_sensitive(lookup_widget("box_python_version_row"),
			now_on && uv_executable_available());
}

/* Scripts tab — Python version dropdown. Same confirmation flow as the
 * checkbox above. We track the previously-confirmed version in
 * python_uv_version_confirmed so we can revert correctly on cancel. */
void on_combo_python_uv_version_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	(void)pspec;
	if (python_uv_widget_reverting)
		return;
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	guint new_idx = gtk_drop_down_get_selected(combo);
	const gchar *new_id = uv_python_version_from_index(new_idx);
	if (python_uv_version_confirmed
			&& g_strcmp0(new_id, python_uv_version_confirmed) == 0)
		return;

	int confirm = siril_confirm_dialog(
			_("Change Python version?"),
			_("Are you sure you want to do this? You will need to rebuild "
			  "Python environments to enact the change, which may involve "
			  "significant dependency downloads."),
			_("Change"));
	if (!confirm) {
		python_uv_widget_reverting = TRUE;
		gtk_drop_down_set_selected(combo,
				uv_python_index_from_version(python_uv_version_confirmed));
		python_uv_widget_reverting = FALSE;
		return;
	}
	g_free(python_uv_version_confirmed);
	python_uv_version_confirmed = g_strdup(new_id);
}

/* Nested-GMainLoop bridge for the "Rebuild ALL" dialog, which needs a
 * checkbox in the message area — GTK4 removed gtk_dialog_run, so we run a
 * local loop and record the response, mirroring the pattern in
 * keywords_tree.c. */
struct py_reset_dlg_ctx { GMainLoop *loop; gint result; };
static void py_reset_dlg_accept(GtkButton *b, gpointer ud) { (void)b; struct py_reset_dlg_ctx *c = ud; c->result = GTK_RESPONSE_ACCEPT; if (c->loop && g_main_loop_is_running(c->loop)) g_main_loop_quit(c->loop); }
static void py_reset_dlg_cancel(GtkButton *b, gpointer ud) { (void)b; struct py_reset_dlg_ctx *c = ud; c->result = GTK_RESPONSE_CANCEL; if (c->loop && g_main_loop_is_running(c->loop)) g_main_loop_quit(c->loop); }
static gboolean py_reset_dlg_close(GtkWindow *w, gpointer ud) { (void)w; struct py_reset_dlg_ctx *c = ud; c->result = GTK_RESPONSE_CANCEL; if (c->loop && g_main_loop_is_running(c->loop)) g_main_loop_quit(c->loop); return FALSE; }

/* §4.8.3: nuclear option. The historical "Reset python venv" button keeps
 * the same UI handler name but now drives rebuild_all_python_state() —
 * wiping the base venv, every per-script venv, and the ledger. uv cache is
 * preserved by default; the confirm dialog offers an explicit checkbox to
 * wipe it too. */
void on_button_python_reset_venv_clicked(gpointer user_data) {
	GtkWidget *dialog = gtk_window_new();
	gtk_window_set_title(GTK_WINDOW(dialog), _("Rebuild ALL Python environments?"));
	gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);
	gtk_window_set_destroy_with_parent(GTK_WINDOW(dialog), TRUE);
	gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(lookup_widget("settings_window")));
	gtk_window_set_resizable(GTK_WINDOW(dialog), FALSE);

	GtkWidget *content = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
	gtk_widget_set_margin_start(content, 12);
	gtk_widget_set_margin_end(content, 12);
	gtk_widget_set_margin_top(content, 12);
	gtk_widget_set_margin_bottom(content, 12);
	gtk_window_set_child(GTK_WINDOW(dialog), content);

	GtkWidget *heading = gtk_label_new(NULL);
	gtk_label_set_markup(GTK_LABEL(heading),
			_("<b>Rebuild ALL Python environments?</b>"));
	gtk_label_set_xalign(GTK_LABEL(heading), 0.0);
	gtk_box_append(GTK_BOX(content), heading);

	GtkWidget *body = gtk_label_new(
			_("This will kill any running Python scripts and delete the base "
			"Python environment and ALL per-script environments. They will be "
			"rebuilt automatically the next time each script runs, using "
			"wheels already in the local cache (fast).\n\n"
			"All modules installed at runtime by scripts will be lost."));
	gtk_label_set_wrap(GTK_LABEL(body), TRUE);
	gtk_label_set_xalign(GTK_LABEL(body), 0.0);
	gtk_widget_set_size_request(body, 420, -1);
	gtk_box_append(GTK_BOX(content), body);

	GtkWidget *check = gtk_check_button_new_with_mnemonic(
			_("Also _clear the uv wheel cache (slow; forces re-download of "
			"every dependency on next use)"));
	gtk_box_append(GTK_BOX(content), check);

	GtkWidget *bbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
	gtk_widget_set_halign(bbox, GTK_ALIGN_END);
	gtk_widget_set_margin_top(bbox, 6);
	GtkWidget *cancel_button = gtk_button_new_with_mnemonic(_("_Cancel"));
	GtkWidget *rebuild_button = gtk_button_new_with_mnemonic(_("_Rebuild"));
	gtk_widget_add_css_class(rebuild_button, "destructive-action");
	gtk_box_append(GTK_BOX(bbox), cancel_button);
	gtk_box_append(GTK_BOX(bbox), rebuild_button);
	gtk_box_append(GTK_BOX(content), bbox);
	gtk_window_set_default_widget(GTK_WINDOW(dialog), cancel_button);

	struct py_reset_dlg_ctx ctx = { g_main_loop_new(NULL, FALSE), GTK_RESPONSE_CANCEL };
	g_signal_connect(cancel_button, "clicked", G_CALLBACK(py_reset_dlg_cancel), &ctx);
	g_signal_connect(rebuild_button, "clicked", G_CALLBACK(py_reset_dlg_accept), &ctx);
	g_signal_connect(dialog, "close-request", G_CALLBACK(py_reset_dlg_close), &ctx);

	gtk_widget_set_visible(dialog, TRUE);
	g_main_loop_run(ctx.loop);
	g_main_loop_unref(ctx.loop);

	gboolean clear_cache = siril_toggle_get_active(check);
	gint response = ctx.result;
	gtk_window_destroy(GTK_WINDOW(dialog));

	if (response == GTK_RESPONSE_ACCEPT)
		rebuild_all_python_state(clear_cache);
}

/* §4.8.1: surgical — base venv only. Per-script venvs and uv cache survive. */
void on_button_python_reset_base_venv_clicked(gpointer user_data) {
	if (siril_confirm_dialog(_("Rebuild base Python environment?"),
			_("This rebuilds the shared base Python environment. Per-script "
			"environments and the uv wheel cache are preserved."),
			_("_Rebuild"))) {
		rebuild_base_venv();
	}
}

/* §4.8.4: prune. Removes per-script venvs whose scripts no longer exist OR
 * which haven't been used in N days (default 90). */
void on_button_python_prune_unused_clicked(gpointer user_data) {
	GtkSpinButton *spin = GTK_SPIN_BUTTON(lookup_widget("spin_python_prune_age_days"));
	gint days = spin ? (gint)gtk_spin_button_get_value(spin) : 90;
	GError *err = NULL;
	guint removed = prune_unused_script_venvs(days, &err);
	if (err) {
		siril_log_error(_("Prune failed: %s\n"), err->message);
		g_clear_error(&err);
		return;
	}
	if (removed > 0)
		siril_log_message(_("Pruned %u unused per-script Python "
				"environments.\n"), removed);
	else
		siril_log_message(_("No unused per-script Python environments to "
				"prune.\n"));
}

/* §4.8.4 cont.: reclaim disk by pruning the uv wheel cache. Removes wheel
 * entries not referenced by any tracked venv. Pairs naturally with the
 * "Prune unused script environments" button: prune venvs first to release
 * references, then click this to collect the orphans. */
void on_button_python_prune_uv_cache_clicked(gpointer user_data) {
	GError *err = NULL;
	if (!prune_uv_cache(&err)) {
		if (err) {
			siril_log_error(_("Prune uv cache failed: %s\n"), err->message);
			g_clear_error(&err);
		}
		return;
	}
	/* prune_uv_cache() already logs uv's stdout/stderr summary. */
}

/* these one are not on the preference dialog */

void on_cosmCFACheck_toggled(GtkCheckButton *button, gpointer user_data) {
	g_rw_lock_writer_lock(&com.pref_rwlock);
	com.pref.prepro.cfa = siril_toggle_get_active(GTK_WIDGET(button));
	g_rw_lock_writer_unlock(&com.pref_rwlock);
}

void on_checkbutton_equalize_cfa_toggled(GtkCheckButton *button, gpointer user_data) {
	g_rw_lock_writer_lock(&com.pref_rwlock);
	com.pref.prepro.equalize_cfa = siril_toggle_get_active(GTK_WIDGET(button));
	g_rw_lock_writer_unlock(&com.pref_rwlock);
}

void on_fix_xtrans_af_toggled(GtkCheckButton *button, gpointer user_data) {
	g_rw_lock_writer_lock(&com.pref_rwlock);
	com.pref.prepro.fix_xtrans = siril_toggle_get_active(GTK_WIDGET(button));
	g_rw_lock_writer_unlock(&com.pref_rwlock);
}

/* ********************************** */
