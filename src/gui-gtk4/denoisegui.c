...
static void denoise_dialog_init_statics() {
	if (denoise_modulation_adjustment == NULL) {
		...
		// GtkRadioButton
		radio_denoise_nosecondary = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "radio_denoise_nosecondary"));
		radio_denoise_vst = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "radio_denoise_vst"));
		radio_denoise_da3d = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "radio_denoise_da3d"));
		radio_denoise_sos = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "radio_denoise_sos"));
		// GtkScale
...
static void get_denoise_values(struct denoise_args *params) {
	...
	params->da3d = siril_toggle_get_active(GTK_WIDGET(radio_denoise_da3d));
	params->sos = siril_toggle_get_active(GTK_WIDGET(radio_denoise_sos)) ?
	              gtk_spin_button_get_value(spin_sos_iters) : 1;
	params->rho = gtk_spin_button_get_value(spin_rho);
	params->do_anscombe = siril_toggle_get_active(GTK_WIDGET(radio_denoise_vst));
	params->do_cosme = siril_toggle_get_active(GTK_WIDGET(check_denoise_cosmetic));
	params->suppress_artefacts = siril_toggle_get_active(GTK_WIDGET(check_denoise_suppress_artefacts));
}
...
void on_denoise_dialog_show(GtkWidget *widget, gpointer user_data) {
	...
	siril_toggle_set_active(GTK_WIDGET(check_denoise_cosmetic), TRUE);
	siril_toggle_set_active(GTK_WIDGET(check_denoise_suppress_artefacts), FALSE);
...
void update_sos(GtkCheckButton *button, gpointer user_data) {
	if (siril_toggle_get_active(GTK_WIDGET(radio_denoise_sos)))
