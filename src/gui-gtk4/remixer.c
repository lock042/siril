...
void reset_left() {
	...
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("remix_type_left")), STRETCH_PAYNE_NORMAL);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("remix_colour_left")), COL_HUMANLUM);
...
void reset_right() {
	...
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("remix_type_right")), STRETCH_ASINH);
	gtk_drop_down_set_selected(GTK_DROP_DOWN(lookup_widget("remix_colour_right")), COL_HUMANLUM);
	set_notify_block(FALSE);
}
...
int toggle_remixer_window_visibility(int _invocation, fits* _fit_left, fits* _fit_right) {
	...
	if (gtk_widget_get_visible(lookup_widget("dialog_star_remix"))) {
		...
		gtk_button_set_child(GTK_BUTTON(lookup_widget("eyedropper_SP_left")), v);
		gtk_button_set_child(GTK_BUTTON(lookup_widget("eyedropper_SP_right")), w);
...
void on_remix_colour_left_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	left_changed = TRUE;
	colour_left = gtk_drop_down_get_selected(combo);
	update_remix_histo_left();
	update_image *param = malloc(sizeof(update_image));
	param->update_preview_fn = remixer_update_preview;
	param->show_preview = TRUE;
...
void on_remix_colour_right_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	right_changed = TRUE;
	colour_right = gtk_drop_down_get_selected(combo);
	update_remix_histo_right();
	update_image *param = malloc(sizeof(update_image));
	param->update_preview_fn = remixer_update_preview;
	param->show_preview = TRUE;
...
void on_remix_type_left_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	left_changed = TRUE;
	type_left = gtk_drop_down_get_selected(combo);
...
void on_remix_type_right_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	right_changed = TRUE;
	type_right = gtk_drop_down_get_selected(combo);
...
static void remix_apply_right_file(const gchar *filename) {
	...
	if (left_loaded) {
		if(!check_images_match(&fit_left, &fit_right)) {
			...
			right_loaded = TRUE;
			/* See companion comment in remix_apply_left_file: profile
			 * matching disabled along with the fits-struct ICC fields. */
			merge_fits_headers_to_result(gfit, FALSE, &fit_left, &fit_right, NULL);
			if (fit_left.keywords.filter[0] != '\0' && fit_right.keywords.filter[0] != '\0' && strlen(fit_left.keywords.filter) >= 8 && strlen(fit_right.keywords.filter) >= 8) {
				gchar *temp_l = g_malloc(strlen(fit_left.keywords.filter) - 7);
				g_strlcpy(temp_l, fit_left.keywords.filter, strlen(fit_left.keywords.filter) - 8);
...
void on_toggle_remixer_log_histograms_toggled(GtkCheckButton *button, gpointer user_data) {
	remix_log_scale = siril_toggle_get_active(GTK_WIDGET(button));
	update_remix_histo_left();
	update_remix_histo_right();
}
...
void on_remixer_clipmode_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	left_changed = TRUE;
	right_changed = TRUE;
	clip_mode = (clip_mode_t) gtk_drop_down_get_selected(combo);
	update_remix_histo_left();
	update_remix_histo_right();
	update_image *param = malloc(sizeof(update_image));
	param->update_preview_fn = remixer_update_preview;
