...
static GtkCheckButton *sng_toggle_stretch = NULL, *sng_toggle_followon = NULL;
static GtkCheckButton *sng_toggle_upsample = NULL, *sng_toggle_starmask = NULL;
static GtkCheckButton *sng_toggle_customstride = NULL, *sng_toggle_sequence = NULL;
...
static GtkDropDown *sng_combo_next_seq = NULL;
...
static void starnetgui_init_statics(void) {
	...
	sng_toggle_stretch = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_starnet_stretch"));
	sng_toggle_followon = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_starnet_postremix"));
	sng_toggle_upsample = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_starnet_upsample"));
	sng_toggle_starmask = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_starnet_starmask"));
	sng_toggle_customstride = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "toggle_starnet_customstride"));
	sng_toggle_sequence = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "starnet_sequence_toggle"));
	...
	sng_combo_next_seq = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "sng_combo_next_seq"));
}
...
void on_starnet_dialog_show(GtkWidget *widget, gpointer user_data) {
	...
	siril_toggle_set_active(GTK_WIDGET(sng_toggle_followon), sgui_follow_on);
	siril_toggle_set_active(GTK_WIDGET(sng_toggle_sequence), FALSE);
	siril_toggle_set_active(GTK_WIDGET(sng_toggle_stretch), sgui_linear);
	siril_toggle_set_active(GTK_WIDGET(sng_toggle_upsample), sgui_upscale);
	siril_toggle_set_active(GTK_WIDGET(sng_toggle_starmask), sgui_starmask);
	siril_toggle_set_active(GTK_WIDGET(sng_toggle_customstride), sgui_customstride);
	gtk_spin_button_set_value(sng_spin_stride, sgui_starnet_stride);
	gtk_widget_set_sensitive(sng_next_seq_controls, sgui_starmask && siril_toggle_get_active(GTK_WIDGET(sng_toggle_sequence)));
...
void on_starnet_execute_clicked(GtkButton *button, gpointer user_data) {
	...
	starnetgui_init_statics();
	sgui_customstride = siril_toggle_get_active(GTK_WIDGET(sng_toggle_customstride));
	sgui_starmask = siril_toggle_get_active(GTK_WIDGET(sng_toggle_starmask));
	sgui_upscale = siril_toggle_get_active(GTK_WIDGET(sng_toggle_upsample));
	sgui_linear = siril_toggle_get_active(GTK_WIDGET(sng_toggle_stretch));
	...
	if (siril_toggle_get_active(GTK_WIDGET(sng_toggle_sequence)) == FALSE) {
		...
		if (sequence_is_loaded()) {
			...
			multi_args->new_seq_index = gtk_drop_down_get_selected(sng_combo_next_seq);
...
void on_toggle_starnet_customstride_toggled(GtkCheckButton *button, gpointer user_data) {
	sgui_customstride = siril_toggle_get_active(GTK_WIDGET(button));
...
void on_toggle_starnet_stretch_toggled(GtkCheckButton *button, gpointer user_data) {
	sgui_linear = siril_toggle_get_active(GTK_WIDGET(button));
}
void on_toggle_starnet_postremix_toggled(GtkCheckButton *button, gpointer user_data) {
	sgui_follow_on = siril_toggle_get_active(GTK_WIDGET(button));
...
void on_toggle_starnet_upsample_toggled(GtkCheckButton *button, gpointer user_data) {
	sgui_upscale = siril_toggle_get_active(GTK_WIDGET(button));
}

void on_toggle_starnet_starmask_toggled(GtkCheckButton *button, gpointer user_data) {
	starnetgui_init_statics();
	sgui_starmask = siril_toggle_get_active(GTK_WIDGET(button));
		if (sgui_follow_on && !sgui_starmask) {
		sgui_follow_on = FALSE;
		siril_toggle_set_active(GTK_WIDGET(sng_toggle_followon), sgui_follow_on);
	}
...
void on_starnet_sequence_toggle_toggled(GtkCheckButton *button, gpointer user_data) {
	gtk_widget_set_sensitive(sng_next_seq_controls, sgui_starmask && siril_toggle_get_active(GTK_WIDGET(button)));
}

void on_sng_combo_next_seq_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *combo = GTK_DROP_DOWN(obj);
	(void)pspec;
	sgui_next_seq_index = gtk_drop_down_get_selected(combo);
}
