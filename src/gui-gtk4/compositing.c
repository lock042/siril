...
static GtkCheckButton *comp_cumulate_rgb = NULL;
static GtkWidget *comp_button_align = NULL;
static GtkDropDown *comp_align_method_combo = NULL;
...
static void compositing_dialog_init_statics(void) {
	...
	comp_cumulate_rgb = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "cumulate_rgb_button"));
...
static transformation_type the_type = HOMOGRAPHY_TRANSFORMATION;
// always HOMOGRAPHY_TRANSFORMATION unless using the shift-only method
...
/* Phase 14G.3: GtkColorChooserDialog removed.  Replacement is a plain
 * GtkWindow built from colorchooserdialog.ui that hosts a
 * GtkColorDialogButton (4.10+) plus the existing wavelength/filter
 * widgets and explicit OK/Cancel buttons. */
static GtkWindow             *color_dialog        = NULL;
static GtkColorDialogButton  *color_dialog_button = NULL;
...
/* Phase 17.D: GTK4 removed gtk_file_filter_add_custom + GtkFileFilterInfo.
 * The replacement is gtk_file_filter_add_suffix (GTK 4.4+) which already
...
static void increment_pixel_components_from_layer_saturated_value(int fits_index, GdkRGBA *rgbpixel,
                                                                  float layer_pixel_value);
...
/* GTK4 event-controller callbacks for the per-layer color tile.
 * Replaces three GTK3 signals (button-press-event, button-release-event,
 * motion-notify-event); the bodies are unchanged, only the parameter
 * marshalling. */
static void on_color_tile_pressed(GtkGestureClick *gesture, int n_press,
...
/* Phase 14G.6: GtkFileChooserButton removed; the prototype now takes a
 * generic GtkWidget* (the body, if any, was already retyped earlier or
 * lives only inside #if 0 blocks for git history). */
...
static void setup_drag_and_drop(GtkButton *button, layer *l);

static gboolean on_chooser_drop(GtkDropTarget *target, const GValue *value,
                                double x, double y, gpointer user_data);
...
static void compute_compositor_mem_limits(fits *fit) {
	...
	if (limit > 0) {
		...
		if (limit < 3) {
			...
			siril_log_message(
				_(
					"Based on the available memory and initial image dimensions, up to %d images may be composited (the maximum can never exceed 8).\n"),
				limit);
		}
	}
	...
	if (count == maximum_layers)
		gtk_widget_unparent(GTK_WIDGET(add_button));
}

/* the compositing menu callback */

/* Phase 20 / 8B: DnD migrated from gtk_drag_dest_set + GdkDragContext +
 * GtkSelectionData to GtkDropTarget + GdkFileList.  Motion handling is
 * automatic with GtkDropTarget; the explicit drag-motion callback is gone. */
static gboolean on_chooser_drop(GtkDropTarget *target, const GValue *value,
                                double x, double y, gpointer user_data) {
	(void) target;
	(void) x;
	...
	gboolean valid = (g_str_has_suffix(filename_lower, ".fit") ||
	...
	l->selected_filename = g_strdup(filename);
...
static void setup_drag_and_drop(GtkButton *button, layer *l) {
	...
	g_signal_connect(target, "drop", G_CALLBACK(on_chooser_drop), l);
...
layer *create_layer(int index) {
	...
	ret->remove_button = GTK_BUTTON(gtk_button_new_from_icon_name("list-remove"));
	...
	gtk_drawing_area_set_draw_func(ret->color_w, draw_layer_color, NULL, NULL);
	...
	g_signal_connect(click, "released", G_CALLBACK(on_color_tile_released), NULL);
	 ...
	 * the replacement is to set the button's child to a GtkBox holding
	 * a label and a symbolic icon.  GtkIconInfo is gone; use
	 * gtk_image_new_from_icon_name for symbolic icons (the theme
	 * handles colour automatically). */
	ret->chooser_button = GTK_BUTTON(gtk_button_new());

	GtkWidget *button_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
	GtkWidget *button_label = gtk_label_new(_("(None)"));
	...
	gtk_button_set_child(ret->chooser_button, button_box);

	/* Stash the label widget on the button so on_filechooser_file_set
	 * (and reset_compositing_module) can update it later via
	 * gtk_label_set_text. */
	g_object_set_data(G_OBJECT(ret->chooser_button), "siril-chooser-label", button_label);

	gtk_widget_add_css_class(GTK_WIDGET(ret->chooser_button), "file");
	gtk_widget_set_size_request(GTK_WIDGET(ret->chooser_button), 200, -1);

	// Get the label widget created by GTK
	GtkWidget *child = gtk_button_get_child(ret->chooser_button);
	if (GTK_IS_LABEL(child)) {
		ret->chooser_button_label = GTK_LABEL(child);
	} else if (GTK_IS_BOX(child)) {
		// GTK created a box with label and image
		GList *children = widget_list_children(GTK_WIDGET(child));
		for (GList *l = children; l != NULL; l = l->next) {
			if (GTK_IS_LABEL(l->data)) {
				ret->chooser_button_label = GTK_LABEL(l->data);
				break;
			}
		}
	...
	/* Enable drag and drop on the button */
	setup_drag_and_drop(ret->chooser_button, ret);
...
static void add_the_layer_add_button() {
	...
	if (!add_button) {
		first_time = 1;
		add_button = GTK_BUTTON(gtk_button_new_from_icon_name("list-add"));
...
static void grid_remove_row(int layer, int free_the_row) {
	if (!layers[layer]) return;
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->remove_button));
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->color_w));
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->chooser_button)); // Changed
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->label));
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->spinbutton_x));
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->spinbutton_y));
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->spinbutton_r));
	gtk_widget_unparent(GTK_WIDGET(layers[layer]->centerbutton));
	if (free_the_row) {
...
static void grid_add_row(int layer, int index, int first_time) {
	...
	if (first_time) {
		...
		gtk_widget_set_visible(GTK_WIDGET(layers[layer]->chooser_button), TRUE); // Changed
...
void open_compositing_window() {
	...
	if (siril_toggle_get_active(GTK_WIDGET(comp_demosaicing_btn))) {
		siril_log_warning(
			_(
				"Disabling debayer-on-open setting: this must be unset in order to open monochrome images in the compositing tool.\n"));
		siril_toggle_set_active(GTK_WIDGET(comp_demosaicing_btn), FALSE);
	}

	if (!compositing_loaded) {
		register_selection_update_callback(update_compositing_registration_interface);

		/* GTK4: gtk_builder_connect_signals removed (auto-resolved by
		 * GtkBuilderCScope when the .ui was loaded). */
		...
		color_dialog = GTK_WINDOW(gtk_builder_get_object(gui.builder, "colorchooserdialog"));
		color_dialog_button = GTK_COLOR_DIALOG_BUTTON(gtk_builder_get_object(gui.builder, "colordialog_button"));
		...
		/* Phase 17: configure the luminance chooser button.  Same
		 * pattern as add_layer_add_button — GtkBox child holding a
		 * label + a symbolic icon, no GtkIconInfo / set_image. */
		GtkWidget *lum_button_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
		GtkWidget *lum_button_label = gtk_label_new(_("(None)"));
		gtk_widget_set_hexpand(lum_button_label, TRUE);
		gtk_label_set_xalign(GTK_LABEL(lum_button_label), 0.0);
		gtk_box_append(GTK_BOX(lum_button_box), lum_button_label);
		GtkWidget *lum_folder_icon = gtk_image_new_from_icon_name("folder-open-symbolic");
		gtk_box_append(GTK_BOX(lum_button_box), lum_folder_icon);
		gtk_button_set_child(layers[0]->chooser_button, lum_button_box);

		gtk_widget_add_css_class(GTK_WIDGET(layers[0]->chooser_button), "file");
		gtk_widget_set_size_request(GTK_WIDGET(layers[0]->chooser_button), 200, -1);

		layers[0]->chooser_button_label = GTK_LABEL(lum_button_label);
		g_object_set_data(G_OBJECT(layers[0]->chooser_button), "siril-chooser-label", lum_button_label);
		...
		/* Enable drag and drop on the luminance button */
		setup_drag_and_drop(layers[0]->chooser_button, layers[0]);
		...
		reg_methods[0] = new_reg_method(
			_("Deep Sky (two-step global star registration)"), &register_multi_step_global, REQUIRES_NO_SELECTION,
			REGTYPE_DEEPSKY);
		reg_methods[1] = new_reg_method(
			_("Planetary (DFT image pattern alignment)"), &register_shift_dft, REQUIRES_SQUARED_SELECTION,
			REGTYPE_PLANETARY);
		reg_methods[2] = new_reg_method(
			_("Planetary (KOMBAT image pattern alignment)"), &register_kombat, REQUIRES_ANY_SELECTION,
			REGTYPE_PLANETARY);
		reg_methods[3] =
				new_reg_method(_("Manual alignment"), &register_manual, REQUIRES_NO_SELECTION, REGTYPE_DEEPSKY);

		reg_methods[4] = NULL;
		update_compositing_registration_interface();
		/* fill compositing_align_method_combo */
		GtkDropDown *aligncombo = GTK_DROP_DOWN(
			gtk_builder_get_object(gui.builder, "compositing_align_method_combo"));
		siril_drop_down_clear_strings(aligncombo);
		i = 0;
		while (reg_methods[i] != NULL) {
			siril_drop_down_append_text(aligncombo, reg_methods[i]->name);
			i++;
		}
		if (i > 0) {
			gtk_drop_down_set_selected(GTK_DROP_DOWN(aligncombo), 0);
		}
		update_compositing_registration_interface();
		compositing_loaded = 1;
	} else {
		/* not the first load, update the CWD just in case it changed in the meantime */
		/* No need to update siril_file_chooser_set_current_folder_path since we're using
...
static int no_color_available() {
	// don't test luminance
	int i;
	for (i = 1; layers[i]; i++) {
		if (has_fit(i))
			return 0;
...
void on_composition_use_lum_toggled(GtkCheckButton *togglebutton, gpointer user_data) {
	luminance_mode = siril_toggle_get_active(GTK_WIDGET(togglebutton));
...
/* Internal function to load an image into a layer from a filename */
static void load_layer_image(layer *target_layer, const char *filename) {
	int layer, retval;
	char buf[48];
	...
	if (layers[layer]->the_fit.rx == 0 && number_of_images_loaded() == maximum_layers) {
		siril_message_dialog(GTK_MESSAGE_ERROR, _("Error: image could not be loaded"),
		                     _(
			                     "The maximum number of images of this size has been reached based on available memory limits."));
		return;
	}
	...
	if ((retval = read_single_image(filename, &layers[layer]->the_fit,
		...
		/* First we test that we load a single-channel image */
	...
	if (layer == 0) {
		GtkCheckButton *lum_button = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "composition_use_lum"));
		g_signal_handlers_block_by_func(lum_button, on_composition_use_lum_toggled, NULL);
		siril_toggle_set_active(GTK_WIDGET(lum_button), !retval);
		...
		for (int layer = 0; layer < maximum_layers; layer++) {
			if (layers[layer]) {
				if (layers[layer]->centerbutton != button) {
					...
					g_signal_handlers_block_by_func(GTK_TOGGLE_BUTTON(layers[layer]->centerbutton),
					                                on_centerbutton_toggled, NULL);
					siril_toggle_set_active(GTK_WIDGET(layers[layer]->centerbutton), FALSE);
					g_signal_handlers_unblock_by_func(GTK_TOGGLE_BUTTON(layers[layer]->centerbutton),
					                                  on_centerbutton_toggled, NULL);
				} else {
...
void on_button_align_clicked(GtkButton *button, gpointer user_data) {
	...
	GtkDropDown *regcombo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "compositing_align_method_combo"));
	method = reg_methods[gtk_drop_down_get_selected(regcombo)];
	GtkDropDown *framingcombo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "compositing_align_framing_combo"));
	int ft = gtk_drop_down_get_selected(framingcombo);
	framing = ft == 0 ? FRAMING_CURRENT : ft == 1 ? FRAMING_MIN : FRAMING_COG;
	gboolean do_sum = siril_toggle_get_active(GTK_WIDGET(comp_cumulate_rgb));
...
static float get_composition_pixel_value(int fits_index, int reg_layer, int x, int y) {
	...
	if (layers[fits_index]->the_fit.type == DATA_FLOAT)
		...
		pixel_value = (float) layers[fits_index]->the_fit.pdata[0][x + y * layers[fits_index]->the_fit.rx] /
		              USHRT_MAX_SINGLE;
	else
		pixel_value = 0.f;
	if (coeff) {
...
static void update_compositing_registration_interface() {
	...
	GtkDropDown *combo = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "compositing_align_method_combo"));
	int sel_method = gtk_drop_down_get_selected(combo);
...
void on_compositing_align_method_combo_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *widget = GTK_DROP_DOWN(obj);
	(void)pspec;
	update_compositing_registration_interface();
	int sel_method = gtk_drop_down_get_selected(widget);
	if (sel_method == 3) {
		// Prepare for manual alignment
		for (int layer = 0; layer < maximum_layers; layer++) {
			if (layers[layer]) {
...
void on_composition_combo_coloringtype_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	GtkDropDown *widget = GTK_DROP_DOWN(obj);
	(void)pspec;
	coloring_type = gtk_drop_down_get_selected(widget);
	update_result(1);
}
 ...
 * "clicked" signals on the explicit OK/Cancel buttons of the GtkWindow
 * surrogate.  The OK path applies the picked colour through the same
 * monitor-to-image ICC transform as before; Cancel just hides the
 * window. */

void on_colordialog_cancel_clicked(GtkButton *button, gpointer user_data) {
...
void on_colordialog_ok_clicked(GtkButton *button, gpointer user_data) {
	(void)button; (void)user_data;
	if (!color_dialog_button)
		return;

	if (current_layer_color_choosing > 0 && layers[current_layer_color_choosing]) {
		const GdkRGBA *picked = gtk_color_dialog_button_get_rgba(color_dialog_button);
		if (picked)
			layers[current_layer_color_choosing]->display_color = *picked;
		...
		float disp[3] = {
			(float) layers[current_layer_color_choosing]->display_color.red,
...
/* Optional: kept as a no-op signal sink so colour changes inside the
 * GtkColorDialogButton don't fire any extra handlers — the user must
 * click OK explicitly to commit. */
void on_colordialog_rgba_notify(GObject *obj, GParamSpec *pspec, gpointer user_data) {
...
/* Find which layer owns the GtkDrawingArea that fired a gesture. */
static int color_tile_to_layer(GtkWidget *widget) {
	for (int layer = 0; layers[layer]; layer++)
		if (GTK_WIDGET(layers[layer]->color_w) == widget)
			return layer;
	return -1;
}

/* GTK4 press: right-click arms the quick-edit drag (captures the
 * reference colour); left-click does nothing on press — the dialog is
 * opened on release so a press+release on the same tile counts as a
 * click. */
static void on_color_tile_pressed(GtkGestureClick *gesture, int n_press,
		double x, double y, gpointer user_data) {
	(void)n_press; (void)x; (void)y; (void)user_data;
	GtkWidget *widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(gesture));
	int layer = color_tile_to_layer(widget);
	if (layer < 0) return;
...
/* GTK4 release: left-click opens the colour chooser dialog; right-click
 * commits the quick-edit by triggering an image update. */
static void on_color_tile_released(GtkGestureClick *gesture, int n_press,
		double x, double y, gpointer user_data) {
	(void)n_press; (void)x; (void)y; (void)user_data;
	GtkWidget *widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(gesture));
...
static void populate_filter_lists() {
	int i, nb_filters;
	GtkDropDown *cbox = GTK_DROP_DOWN(gtk_builder_get_object(gui.builder, "comboboxtext_filters"));
	nb_filters = get_nb_narrow_filters();
	siril_drop_down_clear_strings(cbox);
	for (i = 0; i < nb_filters; i++)
		siril_drop_down_append_text(cbox, narrow_band_filters[i].name);
	//gtk_drop_down_set_selected(GTK_DROP_DOWN(box), -1);
}
...
void on_filter_changed(GObject *obj, GParamSpec *pspec, gpointer user_data) {
	...
	gtk_editable_set_text(GTK_EDITABLE(wl_entry), wl_text);
}

void on_wavelength_changed(GtkEditable *editable, gpointer user_data) {
	GdkRGBA color;
	double wavelength = g_ascii_strtod(gtk_editable_get_text(GTK_EDITABLE(editable)), NULL);
	if (wavelength < 380.0 || wavelength > 780.0) return;
	double r, g, b, a;
	wavelength_to_display_RGB(wavelength, &r, &g, &b, &a);
	color.red = r; color.green = g; color.blue = b; color.alpha = a;
	if (color_dialog_button)
		gtk_color_dialog_button_set_rgba(color_dialog_button, &color);
}
...
void reset_compositing_module() {
	...
	luminance_mode = 0;
	GtkCheckButton *lum_button = GTK_CHECK_BUTTON(gtk_builder_get_object(gui.builder, "composition_use_lum"));
	siril_toggle_set_active(GTK_WIDGET(lum_button), 0);
	gtk_label_set_text(layers[0]->label, _("not loaded"));
	gtk_widget_set_tooltip_text(GTK_WIDGET(layers[0]->label), _("not loaded"));

	update_compositing_registration_interface();
	on_compositing_align_method_combo_changed(G_OBJECT(comp_align_method_combo), NULL, NULL);
}
...
static void reload_all() {
	...
	// Reload files by directly calling load_layer_image
	// Start with the biggest layer
	if (layers[biggest_layer]->selected_filename) {
		load_layer_image(layers[biggest_layer], layers[biggest_layer]->selected_filename);
	}

	// Then reload all other layers
	for (int layer = 0; layer < maximum_layers; layer++) {
		if (layers[layer] && layers[layer]->selected_filename && layer != biggest_layer) {
			load_layer_image(layers[layer], layers[layer]->selected_filename);
		}
	}
...
void on_compositing_linear_match_clicked(GtkButton *button, gpointer *user_data) {
	...
	if (number_of_images_loaded() < 2) {
		siril_message_dialog(GTK_MESSAGE_ERROR, _("Error"),
		                     _("At least 2 images must be loaded to perform linear matching."));
		return;
	}
	int ref_layer = -1;
	for (int layer = 0; layer < maximum_layers; layer++) {
...
Homography H_from_translation_and_rotation(double dx, double dy, double dr, point center) {
	...
	double beta = sin(dr);
	Homography H = {
		alpha, beta, dx + (1 - alpha) * center.x - beta * center.y,
...
int manual_align_image_hook(struct generic_seq_args *args, int out_index, int in_index, fits *fit, rectangle *_,
                            int threads) {
	struct star_align_data *sadata = args->user;
	struct registration_args *regargs = sadata->regargs;
