...
void set_progress_bar_data(const char *text, double percent) {
	...
	if (com.headless) {
		...
		g_idle_add(progress_bar_idle_callback, data);
	}
}
...
void gui_log_message(const char* msg, const char* color) {
	...
	new_msg->color = color;
	g_idle_add(idle_messaging, new_msg);
}
...
static gboolean idle_set_cursor(gpointer garg) {
	...
	if (arg->change) {
		if (!current_name ||
			...
			cursor = gdk_cursor_new_from_name(arg->cursor_name, NULL);
	...
	if (gapp && GTK_IS_APPLICATION(gapp)) {
		GList *list = gtk_application_get_windows(GTK_APPLICATION(gapp));
		for (GList *l = list; l; l = l->next) {
			GtkWidget *window = GTK_WIDGET(l->data);
			/* Passing NULL for the cursor argument means the window uses
			 * the cursor of its parent surface. */
			gtk_widget_set_cursor(window, cursor);
		}
		/* gtk_application_get_windows() returns an internal list — do not free. */
	}
