...
#define TMP_FILE "/tmp/test_tmp.ser"
...
int test_ser_image_number() {
	...
	ser_init_struct(ser);
	CHECK(ser_create_file(TMP_FILE, ser, TRUE, NULL), "create file\n");
	...
	CHECK(ser_open_file(TMP_FILE, ser), "reopen\n");
	CHECK(ser->frame_count != 1, "wrong number of frames\n");

	free(ser);
	CHECK(unlink(TMP_FILE), "error unlinking file " TMP_FILE);
...
int test_ser_image_overwrite() {
	...
	CHECK(ser_write_and_close(ser) == 0, "close file succeeded with lost image\n");

	CHECK(!ser_open_file(TMP_FILE, ser), "reopen found a file\n");
...
int test_ser_image_sizes() {
	...
	CHECK(ser_write_and_close(ser) == 0, "close file succeeded for error write\n");
...
int test_ser_dates() {
	...
	CHECK(ser->ts == NULL, "no date information in SER\n");
...
int test_ser_create_from_copy() {
	...
	CHECK(ser_create_file(TMP_FILE, ser, TRUE, orig), "create file\n");
	...
	CHECK(ser_open_file(TMP_FILE, ser), "reopen\n");
	CHECK(ser->color_id != SER_RGB, "wrong image width\n");
