...
CXX=g++
LD=g++
CFLAGS="-Wall -g -I.. -fopenmp `pkg-config --cflags gtk4` `pkg-config --cflags cfitsio` `pkg-config --cflags gsl`"
LDFLAGS="-fopenmp `pkg-config --libs gtk4` `pkg-config --libs cfitsio` `pkg-config --libs gsl` -lm"
