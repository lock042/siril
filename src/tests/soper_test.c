...
/* The ICC profile cm_worker() asserts it runs on the GLib main thread.
 * In Criterion tests there is no GTK main loop, so we acquire/release
 * the default main context around the whole suite. */
static void arithmetics_setup(void) {
	g_main_context_acquire(g_main_context_default());
}

static void arithmetics_teardown(void) {
	g_main_context_release(g_main_context_default());
}

TestSuite(arithmetics, .init = arithmetics_setup, .fini = arithmetics_teardown);
