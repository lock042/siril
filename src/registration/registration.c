...
// This is a slimline version of global star alignment for use with RGB composition
...
//////
		fits *fit = regargs->seq->internal_fits[frame];
		int nbpoints, nb_stars = 0;
		float FWHMx, FWHMy, B;
		char *units;
		Homography H = { 0 };
		int filenum = regargs->seq->imgparam[frame].filenum;	// for display purposes
//		siril_debug_print("registration of image %d using %d threads\n", in_index, threads);
		...
		if (frame != regargs->reference_image) {
			psf_star **stars;
			if (regargs->seq->type == SEQ_SER || regargs->seq->type == SEQ_FITSEQ) {
			...
			image im = { .fit = fit, .from_seq = regargs->seq, .index_in_seq = frame };
			if (regargs->matchSelection && regargs->selection.w > 0 && regargs->selection.h > 0) {
				stars = peaker(&im, layer, &com.pref.starfinder_conf, &nb_stars, &regargs->selection, FALSE, TRUE, regargs->max_stars_candidates, com.pref.starfinder_conf.profile, /*threads*/1);
			}
			else {
				stars = peaker(&im, layer, &com.pref.starfinder_conf, &nb_stars, NULL, FALSE, TRUE, regargs->max_stars_candidates, com.pref.starfinder_conf.profile, /*threads*/1);
			}
...
// Don't care about these in RGB commposition
//			if (!not_matched)
...
/*		if (!regargs->no_output) {
			regargs->imgparam[out_index].filenum = args->seq->imgparam[frame].filenum;
			...
			regargs->regparam[out_index].fwhm = sadata->current_regdata[frame].fwhm;
			regargs->regparam[out_index].weighted_fwhm = sadata->current_regdata[frame].weighted_fwhm;
			regargs->regparam[out_index].roundness = sadata->current_regdata[frame].roundness;
			regargs->regparam[out_index].background_lvl = sadata->current_regdata[frame].background_lvl;
			regargs->regparam[out_index].number_of_stars = sadata->current_regdata[frame].number_of_stars;
			cvGetEye(&regargs->regparam[frame].H);
