...
/* Quality measure: average |∂x|, average |∂y|, return the smaller.
 * Input may be CV_32S (typical: mean_frame from Phase 2) or CV_32F. */
...
/* Re-blur the mean frame from Phase 2 before any AP placement:
 *   mean_frame = GaussianBlur(align.mean_frame.astype(uint16),
 *                             (frames_gauss_width,)*2, 0).astype(int32)
 * Both ap_create_grid and per-AP shift computation consume the same
 * blurred output. */
cv::Mat blur_mean_frame_for_ap(const cv::Mat &mean_frame_raw, const mpp_config_t &cfg);

/* Build the staggered AP grid on a *blurred* mean_frame and apply the
 * brightness / contrast / structure filtering chain. Pass the output of
 * blur_mean_frame_for_ap. */
