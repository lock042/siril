 ...
 *   uint8   : has_bfi    (0 / 1; 0 for a Stage-A-only run — see v9)
 ...
 * v4: global_shifts widened from int32 to double — preserves the
 * sub-pixel registration residual (see mpp.h:global_shifts comment).
 * Reader promotes v3 in-place: read int32 array, widen to
 ...
 * v8: added `bool output_32bit` to mpp_config_t. Stack-time only, so the
 * persisted value is irrelevant (always overridden at stack time), but
 * the raw cfg layout changed, so older sidecars must be regenerated by
 * re-registering.
 * v9: added a `has_bfi` byte after `has_shifts` so a Stage-A-only run can
 * be persisted (Analyze defers per-AP ranking, leaving best_frame_indices
 * NULL — see mpp_analyze). When has_bfi == 0 the best_frame_indices block
 * is omitted and the field reads back NULL; Register/recompute fills it in
 * later. This lets closing+reopening a sequence after Analyze restore the
 * mean reference frame and AP grid without re-Analyzing. The cfg layout is
 * unchanged, but the inserted byte shifts the stream, so v8 files must be
 * regenerated by re-running Analyze.
 * v10: mpp_improve appended `bool alignment_points_zero_mean,
 * alignment_points_refine_reference` and `int
 * alignment_points_reference_frames` to mpp_config_t (register-time
 * Stage B measurement options). The raw cfg layout changed, so older
 * sidecars must be regenerated by re-registering.
 * v11: mpp_improve appended `int alignment_points_step` to mpp_config_t
 * (AP grid pitch decoupled from the correlation-box size; 0 = legacy
 * geometry). Raw cfg layout changed again — v10 sidecars (never in a
 * release) must be regenerated by re-registering.
 * v12: mpp_improve added `double alignment_points_smooth_radius` to
 * mpp_config_t (per-frame robust shift-field smoothing; 0 = off). Raw
 * cfg layout changed — v11 sidecars (never in a release) must be
 * regenerated by re-registering.
 * v13: mpp_improve_experimental added `int debayer_method` to
 * mpp_config_t (Stage C CFA demosaicing algorithm; default BAYER_RCD,
 * later LMMSE). Raw cfg layout changed — v12 sidecars (never in a
 * release) must be regenerated by re-registering.
 * v14: mpp_improve added `bool rank_float_precision` to mpp_config_t
 * (float vs PSS-quantised uint8 Laplace-σ frame quality). Raw cfg
 * layout changed — v13 sidecars (never in a release) must be
 * regenerated by re-registering.
 * v15: mpp_improve_experimental added `int stack_method` to mpp_config_t
 * (Stage C engine: patch mosaic vs warp field; stack-time only). Raw
 * cfg layout changed — v14 sidecars (never in a release) must be
 * regenerated by re-registering.
 * v16: removed the dead drizzle backend fields from mpp_config_t
 * (`int drizzle_mode`, `double drizzle_pixfrac`, `int drizzle_kernel`) —
 * the STScI / Bayer dobox backends were deleted and these were never
 * read; `drizzle_scale` was also renamed to `output_scale` (same double,
 * no layout effect). Dropping three fields shrinks the raw cfg blob, so
 * v15 sidecars (never in a release) must be regenerated by re-registering. */
#define MPP_SIDECAR_VERSION 16u
...
mpp_status_t mpp_sidecar_write(const char *path, const mpp_run_t *run) {
	...
	/* A Stage-A-only run (Analyze) defers per-AP ranking, so
	 * best_frame_indices is NULL; the block is then omitted entirely. */
	const uint8_t has_bfi = run->best_frame_indices ? 1 : 0;
	if (write_all(f, &has_bfi, 1) != 0) goto err;
...
mpp_status_t mpp_sidecar_read(const char *path, mpp_run_t **run_out) {
	...
	*run_out = NULL;
	...
	/* v9+: best_frame_indices may be absent (Stage-A-only run). v8 and
	 * earlier are rejected on the version check above, so this read is
	 * unconditional here. */
	uint8_t has_bfi;
	if (read_all(f, &has_bfi, 1) != 0) { mpp_run_free(run); goto err; }
