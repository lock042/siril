 ...
 * v10: added `int stack_method` to mpp_config_t (Stage C engine: patch vs
 * warp-field). Stack-time only, so the persisted value is irrelevant (set at
 * stack time), but the raw cfg layout changed, so older sidecars must be
 * regenerated by re-registering.
 * v11: appended a `double derot_epoch_jd` after the has_bfi byte.
 * v12: replaced that field with a `uint64 derot_fingerprint` (same 8 bytes) —
 * a hash over the whole .derot plan (epoch, disk fit, body/system, per-frame
 * geometry), so Stage C rejects a .derot that changed in any way since
 * registration, not just a different epoch (0 = registered without
 * derotation). The version check rejects older files, so they must be
 * regenerated by re-registering. */
#define MPP_SIDECAR_VERSION 12u
...
mpp_status_t mpp_sidecar_write(const char *path, const mpp_run_t *run) {
	...
	/* v12: fingerprint of the derotation plan the AP geometry was registered in
	 * (0 = none). */
	if (write_all(f, &run->derot_fingerprint, sizeof(uint64_t)) != 0) goto err;
...
mpp_status_t mpp_sidecar_read(const char *path, mpp_run_t **run_out) {
	...
	*run_out = NULL;
	...
	/* v12: derotation fingerprint (older versions are rejected by the version check). */
	if (read_all(f, &run->derot_fingerprint, sizeof(uint64_t)) != 0) { mpp_run_free(run); goto err; }
