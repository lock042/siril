...
mpp_status_t mpp_sidecar_read(const char *path, mpp_run_t **run_out) {
	...
	*run_out = NULL;
	...
	/* mpp_config_t is stored as raw bytes and its layout changed in v5,
	 * so nothing older than v5 can be read (the v3 int32-shift promotion
	 * below is kept for the day the cfg snapshot becomes self-describing,
	 * but is unreachable until then). v5 differs from v6 only by the two
	 * trailing run-header ints and the bfi stride — promoted in place. */
	if (ver != 5u && ver != MPP_SIDECAR_VERSION) goto err;
