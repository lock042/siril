 ...
 * == true`), regular FITS bottom-up. The cv::Mat header doesn't carry
 * that flag — Phase 5a oracle equivalence was verified against PNG
 * inputs (top-down). The caller is responsible for orientation
 * correctness; mpp processing is rotation-invariant for
 * registration/stacking but the output preserves whichever orientation
 * the input had.
 */
...
/* Bit-depth convention from fit->orig_bitpix (Siril stores both 8-bit
 ...
 * placement, and per-AP shift is the green channel by default. For
 * mono, just layer 0. */
