...
class Segment {
	...
	public:
		...
		Segment(int startx, int starty, int endx, int endy) {
			...
			double dx = x1 - x2;
			double dy = y1 - y2;
			// theta = atan2(dy, dx);	// if accurate angle is needed
			theta = atan(dy / dx);		// we only need [-pi/2, pi/2]
...
static bool is_projection_close(Segment &s1, pointi s2a, pointi s2b) {
	...
	//siril_log_debug("s1x: %d, s1y: %d, s2x: %d, s2y: %d\n", s1x, s1y, s2x, s2y);
	if (!denom)
...
static bool segments_are_closely_colinear(Segment &s1, Segment &s2) {
	...
	//siril_log_debug("grown s1 (%d,%d)->(%d,%d)\n", gs1.x1, gs1.y1, gs1.x2, gs1.y2);
	//siril_log_debug("grown s2 (%d,%d)->(%d,%d)\n", gs2.x1, gs2.y1, gs2.x2, gs2.y2);
	...
	if (gs1.point_is_inside(s2.start()) || gs1.point_is_inside(s2.end()) ||
		...
		/*if (test)
			siril_log_debug("  one point of a segment is inside another and segments project closely\n");
		else siril_log_debug("  one point of a segment is inside another but segments are not close\n");*/
		return test;
	}
	return false;
}
...
static void merge_two_segments(std::vector<Segment> &segments, size_t i, size_t j) {
	...
	if (x1 < x2) {
		newx1 = std::min(x1, std::min(x3, x4));
		newx2 = std::max(x2, std::max(x3, x4));
	} else {
		newx1 = std::max(x1, std::max(x3, x4));
		newx2 = std::min(x2, std::min(x3, x4));
	}
	if (y1 < y2) {
		newy1 = std::min(y1, std::min(y3, y4));
		newy2 = std::max(y2, std::max(y3, y4));
	} else {
		newy1 = std::max(y1, std::max(y3, y4));
		newy2 = std::min(y2, std::min(y3, y4));
	}
	Segment s = Segment(newx1, newy1, newx2, newy2);
	segments[i] = s;
	//siril_log_debug("   updated segment %zd to (%d,%d) -> (%d,%d)\n", i, newx1, newy1, newx2, newy2);
}
...
static size_t remove_duplicate_segments(std::vector<Segment> &segments) {
	...
	do {
		...
		//siril_log_debug("end of iteration: %zd lines kept\n", new_size);
	} while (nb_lines != new_size);
	siril_log_debug("kept %zd unique lines\n", new_size);
	return new_size;
}

#define BORDER_SZ 8	// ignore lines detected less than this many pixels from the sides
...
static size_t remove_segments_on_the_sides(std::vector<Vec4i> &lines, int rx, int ry) {
	...
	for (size_t i = 0; i < nb_lines; i++) {
		...
		if (x1 > BORDER_SZ && x2 > BORDER_SZ && x1 < rx-BORDER_SZ-1 && x2 < rx-BORDER_SZ-1 &&
				y1 > BORDER_SZ && y2 > BORDER_SZ && y1 < ry-BORDER_SZ-1 && y2 < ry-BORDER_SZ-1) {
			if (i != j) {
				lines[j] = lines[i];
			}
			j++;
	...
	for (; j < nb_lines; j++)
		lines.pop_back();
	siril_log_debug("kept %zd lines not on the sides\n", new_size);
	return new_size;
}
...
int cvHoughLines(fits *image, int layer, float threshvalue, int minlen, struct track **tracks) {
	...
	size_t above = 0;
	BYTE *buffer = (BYTE *)calloc(nbpixels, 1);
	if (image->type == DATA_USHORT) {
		WORD thresh = (WORD)threshvalue;
		for (size_t i = 0; i < nbpixels; i++) {
			if (image->data[i] > thresh) {
				above++;
				buffer[i] = 255;
			}
		}
		...
		for (size_t i = 0; i < nbpixels; i++) {
			if (image->fdata[i] > threshvalue) {
				above++;
				buffer[i] = 255;
			}
		}
	}
	double percent = 100.0 * above / nbpixels;
	siril_log_debug("%.2f%% of pixels above the threshold (%zd)\n", percent, above);
	//if (percent > 1.41f) {
	if (percent > 16.0f) {	// for wide field satellite rate tracking, 16 is ok
		free(buffer);
		siril_log_debug("not running line detection, threshold level is too low\n");
		return -1;
	}
	Mat binary = Mat(image->ry, image->rx, CV_8UC1, buffer);

	std::vector<Vec4i> lines; // will hold the results of the detection
	HoughLinesP(binary, lines, 1.0, CV_PI / 540.0, minlen, (double)minlen, (double)POS_EPSILON);
	size_t nb_lines = lines.size();
	if (nb_lines) {
		nb_lines = remove_segments_on_the_sides(lines, image->rx, image->ry);
		siril_log_debug("we have %zd candidate segments\n", nb_lines);
		...
		std::transform(lines.begin(), lines.end(), segments.begin(), [=](Vec4i v) {
				// flip Y to get display coordinates
		...
		for (size_t i = 0; i < nb_lines; i++) {
			...
			int x1 = segments[i].x1, x2 = segments[i].x2, y1 = segments[i].y1, y2 = segments[i].y2;
			siril_log_debug("Line detected (%d,%d)->(%d,%d), angle %f, length %f\n",
		...
		for (size_t i = 0; i < gsl_histogram_bins(hist); i++) {
			...
			if (inbin > 0) {
				...
				siril_log_debug(_("%d objects with an angle in [%f, %f]\n"), inbin, start, start + bin_size);
			}
		}
			...
			*tracks = (struct track *)malloc(nb_lines * sizeof(struct track));
			if (!*tracks) {
				...
				for (size_t i = 0; i < nb_lines; i++) {
					...
					(*tracks)[i].mag = 99.9f;
					(*tracks)[i].mag_err = 0.0f;
					(*tracks)[i].mag_is_absolute = FALSE;
					(*tracks)[i].mag_is_accurate = FALSE;
					(*tracks)[i].snr = 0.0f;
				}
			}
		}
...
/* for an image size rx x ry, make a mask for the line segment going from (sx, sy) to (ex,
 * ey) with the requested width. Another mask can be made for a shape around the streak
 * that masks the background pixels. Width of the main mask is 3*fwhm, the background is
 * between 6 and 9 times fwhm.
 */
int create_streak_masks(int rx, int ry, int sx, int sy, int ex, int ey, float fwhm, BYTE **aperture_mask, BYTE **background_mask) {
	int size = rx * ry;
	BYTE *data1 = (BYTE *)calloc(size, sizeof(BYTE));
	BYTE *data2 = (BYTE *)calloc(size, sizeof(BYTE));
	if (!data1 || !data2) {
		PRINT_ALLOC_ERR;
		free(data1); free(data2);
		return 1;
	}
	Mat im = Mat(ry, rx, CV_8UC1, data1);
	Point start = Point(sx, ry - sy);	// display to FITS y coordinates
	Point end = Point(ex, ry - ey);
	int _3fwhm = roundf_to_int(3.0f * fwhm);
	//siril_log_debug("line from (%d, %d) to (%d, %d)\n", start.x, start.y, end.x, end.y);
	cv::line(im, start, end,
			Scalar(0xff),	// color: white
			_3fwhm,		// width
			LINE_4, 0);	// line type: LINE_AA for antialiased, LINE_4 or LINE_8 for
					// 4-connected or 8-connected Bresenham algorithm

	/* first mask is 2 fwhm thick. We grow it to 3 and to 5, then subtract the 5 by the 3 */
	memcpy(data2, data1, size);
	Mat im2 = Mat(ry, rx, CV_8UC1, data2);
	Mat tmp = im.clone();
	int grow_size1 = std::max(3, _3fwhm);
	int grow_size2 = std::max(6, roundf_to_int(6.0f * fwhm));
	Mat kernel3 = getStructuringElement(MORPH_RECT, Size(grow_size1, grow_size1));
	Mat kernel4 = getStructuringElement(MORPH_RECT, Size(grow_size2, grow_size2));
	cv::dilate(tmp, tmp, kernel3);
	cv::dilate(im2, im2, kernel4);
	// subtract
	for (int i = 0; i < size; i++)
		data2[i] -= tmp.data[i];
	// count (debug)
	int ap_count = 0, bg_count = 0;
	for (int i = 0; i < size; i++) {
		if (data1[i])
			ap_count++;
		if (data2[i])
			bg_count++;
		if (data1[i] && data2[i])
			siril_log_debug("a pixel is in both masks\n");
	}
	siril_log_debug("pixel count in masks: aperture %d, background %d\n", ap_count, bg_count);

	*aperture_mask = data1;
	*background_mask = data2;
	return 0;
}
