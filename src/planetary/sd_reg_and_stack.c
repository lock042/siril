...
/* TODO:
 * check thread safety of caching
 */
...
static int mppsd_image_hook(struct generic_seq_args *args, int out_index, int in_index, fits *fit, rectangle *_) {
	struct mppsd_data *mppdata = args->user;
	int zone_idx;
	...
	for (zone_idx = 0; zone_idx < mppdata->nb_zones; zone_idx++) {
		...
		error = search_local_match_gradient(mppdata->reference_image, gaussian_data,
				fit->rx, fit->ry, &ref_area, &im_area, max_radius, 1, &shiftx, &shifty);
		if (error) {
			// TODO: meaningful message
		...
		/* AP stacking */
		add_image_zone_to_stacking_sum(fit, zone, in_index, mppdata->sum, mppdata->count);
...
static int mppsd_finalize_hook(struct generic_seq_args *args) {
	...
	/* compute averages for the zones and merge with the reference image for areas
