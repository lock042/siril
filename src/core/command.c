...
static gboolean sequence_cfa_warning_check(sequence *seq) {
	...
	if (mono && cfa) {
		...
		siril_log_color_message(
				_(
						"Warning: sequence contains undebayered CFA images. Applying a sequence function not intended for this kind of image. This is likely to give poor results: check your intended workflow.\n"),
				"salmon");
		retval = FALSE;
	} else {
		retval = TRUE;
...
gboolean image_cfa_warning_check() {
	...
	if (gfit.naxes[2] == 1 && gfit.keywords.bayer_pattern[0] != '\0') {
		...
		siril_log_color_message(
				_(
						"Warning: an undebayered CFA image is loaded. Applying an image function not intended for this kind of image. This is likely to give poor results: check your intended workflow.\n"),
				"salmon");
		retval = FALSE;
	} else {
		retval = TRUE;
...
int process_satu(int nb) {
	struct enhance_saturation_data *args = malloc(
			sizeof(struct enhance_saturation_data));
	...
	siril_log_message(
			_(
					"Applying saturation enhancement of %d%%, from level %g times (median + sigma).\n"),
...
int process_save(int nb) {
	...
	gchar *savename = update_header_and_parse(&gfit, filename,
			PATHPARSE_MODE_WRITE_NOFAIL, TRUE, &status);
	if (status > 0) {
		retval = 1;
	} else {
		set_cursor_waiting(TRUE);
...
gpointer run_nlbayes_on_fit(gpointer p) {
	...
	msg1 = malloc(n + 1);
	n = snprintf(msg1, n + 1, _("NL-Bayes denoise (mod=%.3f"),
			args->modulation);
	if (args->da3d) {
		...
		m = snprintf(NULL, 0, _(", SOS enabled (iters=%d, rho=%.3f)"),
				args->sos, args->rho);
		msg2 = malloc(m + 1);
		m = snprintf(msg2, m + 1, _(", SOS enabled (iters=%d, rho=%.3f)"),
				args->sos, args->rho);
	...
	if (args->fit == &gfit && args->fit->naxes[2] == 3
			&& args->suppress_artefacts) {
		fits *loop = NULL;
		if (new_fit_image(&loop, args->fit->rx, args->fit->ry, 1,
				args->fit->type)) {
			retval = 1;
		}
		loop->naxis = 2;
		...
		if (retval == 0) {
			if (args->fit->type == DATA_FLOAT) {
				for (size_t i = 0; i < 3; i++) {
					...
					retval = do_nlbayes(loop, args->modulation, args->sos,
							args->da3d, args->rho, args->do_anscombe);
					for (size_t j = 0; j < npixels; j++) {
		...
		retval = do_nlbayes(args->fit, args->modulation, args->sos, args->da3d,
				args->rho, args->do_anscombe);
	}
...
int process_denoise(int nb) {
	...
	for (int i = 1; i < nb; i++) {
		...
		if (g_str_has_prefix(arg, "-vst")) {
			...
			if (mod <= 0.f || mod > 1.f) {
				siril_log_message(
						_("Error: modulation must be > 0.0 and <= 1.0.\n"));
				free(args);
				return CMD_OK;
			} else
				args->modulation = mod;
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error in rho parameter: must be strictly > 0 and < 1, aborting.\n"));
				free(args);
				return CMD_ARG_ERROR;
			}
			if (!error) {
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error: SOS iterations must be >= 1. Defaulting to 1.\n"));
				sos = 1;
			} else if (sos > 10)
				siril_log_message(
						_(
								"Note: high number of SOS iterations. Processing time may be lengthy...\n"));
			if (!error) {
				args->sos = sos;
			}
		}
	...
	if (args->do_anscombe && (args->sos != 1 || args->da3d)) {
		siril_log_color_message(
				_(
						"Error: will not carry out DA3D or SOS iterations with Anscombe transform VST selected. aborting.\n"),
				"red");
		free(args);
		return CMD_ARG_ERROR;
	}
	if (args->do_anscombe)
		siril_log_message(
				_(
						"Will apply generalised Anscombe variance stabilising transform.\n"));
	if (args->da3d) {
		siril_log_message(_("Will carry out final stage DA3D denoising.\n"));
		if (args->sos != 1) {
			siril_log_message(
					_(
							"Will not carry out both DA3D and SOS. SOS iterations set to 1.\n"));
			args->sos = 1;
		}
	}
	image_cfa_warning_check();
...
int process_starnet(int nb) {
	...
	if (starnet_executablecheck(com.pref.starnet_exe) == NIL) {
		siril_log_color_message(
				_("Error: StarNet executable (%s) is not valid.\n"), "red",
	...
	for (int i = 1; i < nb; i++) {
		...
		if (g_str_has_prefix(arg, "-stretch")) {
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error in stride parameter: must be a positive even integer, max 512, aborting.\n"));
				free(starnet_args);
				return CMD_ARG_ERROR;
			}
			if (!error) {
...
int process_seq_starnet(int nb) {
	...
	struct multi_output_data *multi_args = calloc(1,
			sizeof(struct multi_output_data));
	if (!multi_args)
...
int process_savetif(int nb) {
	...
	if (status > 0) {
		...
		set_cursor_waiting(TRUE);
		retval = savetif(savename, &gfit, bitspersample, astro_tiff,
				com.pref.copyright, tiff_compression, TRUE, TRUE);
		set_cursor_waiting(FALSE);
	}
	free(astro_tiff);
	g_free(filename);
...
static char* normalize_rebayerfilename(char *filename_buffer, const char *input,
		long int maxpath) {
...
int process_rebayer(int nb) {
	...
	if (nb < 5) {
		siril_log_color_message(
				_(
						"Error, requires at least 4 arguments to specify the 4 files!\n"),
				"red");
	...
	set_cursor_waiting(TRUE);
	int retval = readfits(normalize_rebayerfilename(filename, word[1], maxpath),
			&cfa0, NULL, FALSE);
	retval += readfits(normalize_rebayerfilename(filename, word[2], maxpath),
			&cfa1, NULL, FALSE);
	retval += readfits(normalize_rebayerfilename(filename, word[3], maxpath),
			&cfa2, NULL, FALSE);
	retval += readfits(normalize_rebayerfilename(filename, word[4], maxpath),
			&cfa3, NULL, FALSE);
	if (retval) {
...
int process_imoper(int nb) {
	...
	default:
		siril_log_color_message(
				_("Could not understand the requested operator\n"), "red");
		return CMD_ARG_ERROR;
	}

	fits fit = { 0 };
	if (readfits(word[1], &fit, NULL, !com.pref.force_16bit))
...
int process_fmul(int nb) {
	...
	if (end == word[1] || coeff <= 0.f) {
		siril_log_message(
				_(
						"Multiplying by a coefficient less than or equal to 0 is not possible.\n"));
		return CMD_ARG_ERROR;
	}
...
int process_unpurple(int nb) {
	...
	*args = (struct unpurpleargs ) { .fit = fit, .starmask = &starmask,
					.withstarmask = withstarmask, .thresh = thresh,
					.mod_b = mod, .verbose = FALSE };
...
int process_epf(int nb) {
	gchar *end;
	double d = 0.0, mod = 1.0, sigma_col = 11.0, sigma_space = 11.0;
	...
	for (int i = 1; i < nb; i++) {
		...
		if (!g_strcmp0(arg, "-guided")) {
			...
			if (d > 25)
				siril_log_color_message(
						_(
								"Warning: d > approx. 25 may result in lengthy execution times.\n"),
						"salmon");
		} else if (g_str_has_prefix(arg, "-mod=")) {
			gchar *val = arg + 5;
			mod = g_ascii_strtod(val, &end);
	...
	if (sigma_space > 20.0 && d == 0)
		siril_log_color_message(
				_(
						"Warning: spatial sigma > approx. 20 with auto diameter may result in lengthy execution times.\n"),
				"salmon");
	...
	if (filter == EP_GUIDED) {
		if (filename) {
			...
			if (readfits(filename, guidefit, NULL, FALSE)) {
				siril_log_color_message(
						_("Error: guide image could not be loaded\n"), "red");
				clearfits(guidefit);
				free(guidefit);
				g_free(filename);
				return CMD_ARG_ERROR;
		...
		if (guidefit->rx != gfit.rx || guidefit->ry != gfit.ry) {
			siril_log_color_message(
					_("Error: guide image dimensions do not match\n"), "red");
	...
	if (filter == EP_GUIDED && d == 0.0) {
		siril_log_color_message(
				_(
						"Warning: d = 0.0 cannot be used to specify automatic diameter when using a guided filter. Setting d to default value of 5.\n"),
				"salmon");
		d = 5.0;
	}
	struct epfargs *args = calloc(1, sizeof(struct epfargs));
	*args = (struct epfargs ) { .fit = fit, .guidefit = guidefit, .d = d,
...
int process_getref(int nb) {
	...
	if (seq->reference_image < 0) {
		siril_log_message(
				_(
						"Reference image is undefined, the following would be used:\n"));
	...
	if (seq->type == SEQ_REGULAR) {
		...
		siril_log_message(_("Image %d\n"), ref_image);

	if (!seq->imgparam[ref_image].incl)
		siril_log_message(
				_(
						"Warning: this image is excluded from the sequence main processing list\n"));
	adjust_cutoff_from_updated_gfit();
	return CMD_OK;
}
...
int process_makepsf(int nb) {
	...
	if (!g_strcmp0(arg_1, "clear")) {
		if (get_thread_run()) {
			siril_log_message(
					_(
							"Error: will not clear the PSF while a sequence is running.\n"));
		...
		if (!g_strcmp0(arg_1, "save")) {
			...
			if (!word[2] || word[2][0] == '\0') {
				...
				if (!(g_str_has_suffix(word[2], ".fit")
						|| g_str_has_suffix(word[2], ".fits")
						|| g_str_has_suffix(word[2], ".fts")
						|| g_str_has_suffix(word[2], ".tif"))) {
					siril_log_color_message(
							_(
									"Error: filename must have the extension \".fit\", \".fits\", \".fts\" or \".tif\"\n"),
							"red");
		...
		if (!g_strcmp0(arg_1, "blind")) {
			if (!(single_image_is_loaded() || sequence_is_loaded())) {
				siril_log_message(
						_(
								"Error: image or sequence must be loaded to carry out %s PSF estimation, aborting...\n"),
						"blind");
				status = CMD_GENERIC_ERROR;
				goto terminate_makepsf;
			}
			...
			for (int i = 2; i < nb; i++) {
				...
				if (!g_strcmp0(arg, "-l0")) {
					...
					if (error || lambda < 0.f || lambda > 100000.f) {
						siril_log_message(
								_(
										"Error in %s parameter: must be in %s, aborting.\n"),
								"lambda", "]0,10000[");
						goto terminate_makepsf;
					}
					data->lambda = lambda;
					...
					if (ks < 3 || !(ks % 2) || ks > min(gfit.rx, gfit.ry)) {
						siril_log_message(
								_(
										"Error in ks parameter: must be odd and between 3 and minimum of (image height, image width), aborting.\n"));
						goto terminate_makepsf;
					}
					data->ks = ks;
					...
					if (arg[0] == '\0') {
						siril_log_message(
								_("Error: no filename specified, aborting.\n"));
						goto terminate_makepsf;
					} else {
						if (!(g_str_has_suffix(arg, ".fit")
								|| g_str_has_suffix(arg, ".fits")
								|| g_str_has_suffix(arg, ".fts")
								|| g_str_has_suffix(arg, ".tif"))) {
							siril_log_color_message(
									_(
											"Error: filename must have the extension \".fit\", \".fits\", \".fts\" or \".tif\"\n"),
									"red");
					...
					siril_log_color_message(
							_("Unknown parameter %s, aborting.\n"), "red", arg);
					goto terminate_makepsf;
				}
			}
			image_cfa_warning_check();
	...
	terminate_makepsf: g_free(data->savepsf_filename);
	free(data);
	return status;
}
	...
	for (int i = first_arg; i < nb; i++) {
		...
		if (g_str_has_prefix(arg, "-loadpsf=")) {
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error in alpha parameter: must be between 0 and 1e5, aborting.\n"));
				return CMD_ARG_ERROR;
			}
			if (!error) {
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error in iterations parameter: must be between 1 and 1e5, aborting.\n"));
				return CMD_ARG_ERROR;
			}
			if (!error) {
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error in stop parameter: must be between 0 and 1, aborting.\n"));
				return CMD_ARG_ERROR;
			}
			if (!error) {
			...
			if (arg == end)
				...
				siril_log_message(
						_(
								"Error in step size parameter: must be between 0 and 1, aborting.\n"));
				return CMD_ARG_ERROR;
			}
			if (!error) {
...
int process_unsharp(int nb) {
	...
	char log[90];
	sprintf(log, "Unsharp filtering, sigma: %.2f, coefficient: %.2f", sigma,
			multi);
	gfit.history = g_slist_append(gfit.history, strdup(log));
	return CMD_OK | CMD_NOTIFY_GFIT_MODIFIED;
}
...
int process_ccm(int nb) {
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			...
			if (end == word[word_index]) {
				siril_log_message(
						_("Invalid matrix element (%d, %d) %s, aborting.\n"), i,
						j, word[word_index]);
				free(prefix);
				free(args);
				return CMD_ARG_ERROR;
	...
	if (word[10 + offset]) {
		...
		if (end == word[10 + offset] || args->power < 0.f
				|| args->power > 10.f) {
			siril_log_message(
					_(
							"Invalid power %s, must be between 0.0 and 10.0: aborting.\n"),
					word[10 + offset]);
			free(prefix);
			free(args);
			return CMD_ARG_ERROR;
	...
	siril_log_message(
			_(
					"Applying CCM with coefficients %f, %f, %f, %f, %f, %f, %f, %f, %f and power %f\n"),
	...
	if (is_sequence) {
		...
		if (!isrgb(&gfit)) {
			siril_log_color_message(
					_(
							"Color Conversion Matrices can only be applied to 3-channel images.\n"),
					"red");
...
int process_crop(int nb) {
	if (is_preview_active()) {
		siril_log_message(
				_(
						"It is impossible to crop the image when a filter with preview session is active. "
	...
	if (com.selection.h && com.selection.w) {
		...
		if (nb == 5) {
			...
			if (end1 == word[1] || area.x < 0 || end2 == word[2]
					|| area.y < 0) {
				siril_log_message(
						_("Crop: x and y must be positive values.\n"));
				return CMD_ARG_ERROR;
			}
			area.w = g_ascii_strtoull(word[3], &end1, 10);
			area.h = g_ascii_strtoull(word[4], &end2, 10);
			if (end1 == word[3] || area.w < 0 || end2 == word[4]
					|| area.h < 0) {
				siril_log_message(
						_("Crop: width and height must be greater than 0.\n"));
				return CMD_ARG_ERROR;
			}
			if (area.x + area.w > gfit.rx || area.y + area.h > gfit.ry) {
				siril_log_message(
						_(
								"Crop: width and height, respectively, must be less than %d and %d.\n"),
						gfit.rx, gfit.ry);
				return CMD_ARG_ERROR;
			}
		} else {
			siril_log_message(
					_(
							"Crop: select a region or provide x, y, width, height\n"));
			return CMD_ARG_ERROR;
		}
	}
	...
	char log[90];
	sprintf(log, _("Crop (x=%d, y=%d, w=%d, h=%d)"), area.x, area.y, area.w,
			area.h);
	gfit.history = g_slist_append(gfit.history, strdup(log));
...
int process_ght_args(int nb, gboolean ght_seq, int stretchtype,
		ght_params *params, struct ght_data *seqdata) {
	...
	for (int i = (ght_seq ? 2 : 1); i < nb; i++) {
		...
		if (!strcmp(arg, "R")) {
			...
			if (!g_ascii_strncasecmp(argument, "clip", 4))
				...
				siril_log_color_message(
						_("Error, unknown clip mode %s specified\n"), "red",
						argument);
				return CMD_ARG_ERROR;
			}
	...
	if (stretchtype == STRETCH_LINEAR) {
		if (BP != BP) {
			siril_log_message(
					_(
							"Error: BP must be specified between 0.0 and 1.0 using -BP=\n"));
			return CMD_WRONG_N_ARG;
		}
		if (BP < 0. || BP > 1.) {
		...
		if (D != D) {
			siril_log_message(
					_(
							"Error: D must be specified between 0.0 and 1.0 using -D=\n"));
			return CMD_WRONG_N_ARG;
		}
		...
		BP = 0.0;
		if (stretch_colourmodel == COL_SAT
				&& (!(do_red && do_green && do_blue))) {
			siril_log_message(
					_(
							"Error: saturation stretch requires that all channels must be selected.\n"));
			return CMD_ARG_ERROR;
		}
	}
...
int process_seq_ghs(int nb, int stretchtype) {
	...
	if (params->payne_colourstretchmodel == COL_SAT && seq->nb_layers != 3) {
		siril_log_message(
				_("Error: cannot apply saturation stretch to mono images.\n"));
		free(params);
		free(seqdata->seqEntry);
		free(seqdata);
		return CMD_ARG_ERROR;
...
int process_ghs(int nb, int stretchtype) {
	...
	if (params->payne_colourstretchmodel == COL_SAT && gfit.naxes[2] != 3) {
		siril_log_message(
				_("Error: cannot apply saturation stretch to a mono image.\n"));
		free(params);
		return CMD_ARG_ERROR;
	}
	image_cfa_warning_check();
	...
	case STRETCH_PAYNE_NORMAL:
		sprintf(log,
				"GHS (pivot: %.3f, amount: %.2f, local: %.1f [%.2f, %.2f])",
				params->SP, params->D, params->B, params->LP, params->HP);
		break;
	case STRETCH_PAYNE_INVERSE:
		sprintf(log,
				"Inverse GHS (pivot: %.3f, amount: %.2f, local: %.1f [%.2f, %.2f])",
				params->SP, params->D, params->B, params->LP, params->HP);
		break;
	case STRETCH_ASINH:
	...
	case STRETCH_INVASINH:
		sprintf(log,
				"GHS inverse asinh (pivot: %.3f, amount: %.2f [%.2f, %.2f])",
				params->SP, params->D, params->LP, params->HP);
		break;
	case STRETCH_LINEAR:
...
int process_wavelet(int nb) {
	...
	if (end2 == word[2]
			|| (Type_Transform != TO_PAVE_LINEAR
					&& Type_Transform != TO_PAVE_BSPLINE)) {
	...
	if (gfit.type == DATA_USHORT) {
		...
		for (chan = 0; chan < nb_chan; chan++) {
			dir[chan] = g_build_filename(tmpdir, File_Name_Transform[chan],
					NULL);
		...
		for (chan = 0; chan < nb_chan; chan++) {
			dir[chan] = g_build_filename(tmpdir, File_Name_Transform[chan],
					NULL);
			wavelet_transform_file_float(gfit.fpdata[chan], gfit.ry, gfit.rx,
					dir[chan], Type_Transform, Nbr_Plan);
			g_free(dir[chan]);
		}
	} else
		return CMD_INVALID_IMAGE;
...
int process_asinh(int nb) {
	...
	if (end == word[arg_offset] || beta < 1.0) {
		siril_log_color_message(
				_("Stretch must be greater than or equal to 1\n"), "red");
		return CMD_ARG_ERROR;
	}
	arg_offset++;
	...
	char log[90];
	sprintf(log, "Asinh stretch (amount: %.1f, offset: %.1f, human: %s)", beta,
			offset, human_luminance ? "yes" : "no");
	gfit.history = g_slist_append(gfit.history, strdup(log));
...
int process_ls(int nb) {
	...
	/* No paths are given in argument */
		...
		if (!com.wd) {
			siril_log_message(
					_("Cannot list files, set working directory first.\n"));
			return CMD_GENERIC_ERROR;
		}
...
int process_merge(int nb) {
	...
	if (file_name_has_invalid_chars(word[nb - 1])) {
		siril_log_color_message(
				_(
						"Specified output name %s contains forbidden characters, aborting\n"),
				"red", word[nb - 1]);
		return CMD_ARG_ERROR;
	}
	...
	for (int i = 0; i < nb_seq; i++) {
		...
		if (i != 0
				&& (seqs[i]->rx != seqs[0]->rx || seqs[i]->ry != seqs[0]->ry
						|| seqs[i]->nb_layers != seqs[0]->nb_layers
						|| seqs[i]->bitpix != seqs[0]->bitpix
						|| seqs[i]->type != seqs[0]->type)) {
			siril_log_message(
					_(
							"All sequences must be the same format for merging. Sequence `%s' is different\n"),
					word[i + 1]);
			retval = 1;
			free(seqpath1);
			free(seqpath2);
	...
	case SEQ_SER:
		if (g_str_has_suffix(word[nb - 1], ".ser"))
			...
			outseq_name = g_strdup_printf("%s.ser", word[nb - 1]);
		...
		for (int i = 0; i < nb_seq; i++) {
			for (unsigned int frame = 0; frame < seqs[i]->number; frame++) {
				...
				if (ser_read_frame(seqs[i]->ser_file, frame, fit, FALSE,
					...
					siril_log_message(
							_(
									"Failed to read frame %d from input sequence `%s'\n"),
							frame, word[i + 1]);
					retval = 1;
				...
				if (ser_write_frame_from_fit(&out_ser, fit, written_frames)) {
					siril_log_message(
							_("Failed to write frame %d in merged sequence\n"),
							written_frames);
					retval = 1;
					seqwriter_release_memory();
					ser_close_and_delete_file(&out_ser);
		...
		if (ser_write_and_close(&out_ser)) {
			siril_log_message(
					_("Error while finalizing the merged sequence\n"));
			retval = 1;
		}
		break;
	...
	case SEQ_FITSEQ:
		if (g_str_has_suffix(word[nb - 1], com.pref.ext))
			...
			outseq_name = g_strdup_printf("%s%s", word[nb - 1], com.pref.ext);
		...
		for (int i = 0; i < nb_seq; i++) {
			for (unsigned int frame = 0; frame < seqs[i]->number; frame++) {
				seqwriter_wait_for_memory();
				fits *fit = calloc(1, sizeof(fits));
				if (fitseq_read_frame(seqs[i]->fitseq_file, frame, fit, FALSE,
						-1)) {
					siril_log_message(
							_(
									"Failed to read frame %d from input sequence `%s'\n"),
	...
	default:
		siril_log_message(
				_(
						"This type of sequence cannot be created by Siril, aborting the merge\n"));
		retval = 1;
	}

	merge_clean_up: for (int i = 0; i < nb_seq; i++) {
		if (seqs[i])
...
int process_mirrorx_single(int nb) {
	...
	if (stat_file(word[1], &imagetype, &realname)) {
		siril_log_color_message(
				_("Error opening image %s: file not found or not supported.\n"),
				"red", word[1]);
		free(realname);
		return CMD_FILE_NOT_FOUND;
	}
	if (imagetype != TYPEFITS && imagetype != TYPETIFF) {
		siril_log_color_message(
				_(
						"This command is only supported with FITS and TIFF, able to contain orientation information\n"),
				"red");
		free(realname);
		return CMD_INVALID_IMAGE;
	}
	if (imagetype == TYPEFITS && fitseq_is_fitseq(realname, NULL)) {
		siril_log_color_message(
				_(
						"This command is only supported with single FITS images, for the first HDU, not a FITS cube.\n"),
				"red");
		free(realname);
		return CMD_INVALID_IMAGE;
	}
...
int process_mtf(int nb) {
	...
	if (end1 == word[1] || end2 == word[2] || end3 == word[3]
			|| params.shadows < 0.0 || params.midtones <= 0.0
			|| params.highlights <= 0.0 || params.shadows >= 1.0
			|| params.midtones >= 1.0 || params.highlights > 1.0) {
	...
	sprintf(log, "%s transfer (%.3f, %.4f, %.3f)",
			inverse ? "Inverse midtones" : "Midtones", params.shadows,
			params.midtones, params.highlights);
	gfit.history = g_slist_append(gfit.history, strdup(log));
...
int process_autoghs(int nb) {
	...
	int ret = compute_all_channels_statistics_single_image(&gfit, STATS_BASIC,
	...
	if (linked) {
		...
		if (gfit.type == DATA_USHORT)
			SP *= (gfit.orig_bitpix == BYTE_IMG) ?
					INV_UCHAR_MAX_SINGLE : INV_USHRT_MAX_SINGLE;
...
#pragma omp parallel for num_threads(com.max_thread) schedule(static) if(nb_channels > 1)
		...
		for (int i = 0; i < nb_channels; ++i) {
			if (stats[i]) {
				float SP = stats[i]->median
						+ shadows_clipping * stats[i]->sigma;
				if (gfit.type == DATA_USHORT)
					SP *= (gfit.orig_bitpix == BYTE_IMG) ?
							INV_UCHAR_MAX_SINGLE : INV_USHRT_MAX_SINGLE;
				...
				ght_params params = { .B = b, .D = amount, .LP = lp, .SP = SP,
						.HP = hp, .BP = 0.0, STRETCH_PAYNE_NORMAL, COL_INDEP,
						TRUE, TRUE, TRUE };
	...
	char log[100];
	sprintf(log,
			"AutoGHS (%sk.sigma: %.2f, amount: %.2f, local: %.1f [%.2f, %.2f])",
...
int process_autostretch(int nb) {
	...
	if (nb > arg_index) {
		...
		if (end == word[arg_index] || target_bg < 0.0f || target_bg > 1.0f) {
			siril_log_message(
					_(
							"The target background value must be in the [0, 1] range\n"));
			return CMD_ARG_ERROR;
		}
	}
	...
	if (linked) {
		struct mtf_params params;
		find_linked_midtones_balance(&gfit, shadows_clipping, target_bg,
				&params);
		params.do_red = TRUE;
		params.do_green = TRUE;
		params.do_blue = TRUE;
		...
		find_unlinked_midtones_balance(&gfit, shadows_clipping, target_bg,
				params);
...
int process_resample(int nb) {
	...
	if (word[1][0] == '-') {
		...
		if (end == word[1] || factor <= 0.2 || factor > 5.0) {
			siril_log_message(
					_(
							"Scale %lf not allowed. Should be between 0.2 and 5.0.\n"),
					factor);
			return CMD_ARG_ERROR;
		}
		if (factor == 1.0) {
	...
	for (int i = 2; i < nb; i++) {
		if (g_str_has_prefix(word[i], "-interp=")) {
			...
			if (!g_ascii_strncasecmp(value, "nearest", 7)
					|| !g_ascii_strncasecmp(value, "ne", 2)) {
				interpolation = OPENCV_NEAREST;
				continue;
			}
			if (!g_ascii_strncasecmp(value, "cubic", 5)
					|| !g_ascii_strncasecmp(value, "cu", 2)) {
				interpolation = OPENCV_CUBIC;
				continue;
			}
			if (!g_ascii_strncasecmp(value, "lanczos4", 8)
					|| !g_ascii_strncasecmp(value, "la", 2)) {
				interpolation = OPENCV_LANCZOS4;
				continue;
			}
			if (!g_ascii_strncasecmp(value, "linear", 6)
					|| !g_ascii_strncasecmp(value, "li", 2)) {
				interpolation = OPENCV_LINEAR;
				continue;
			}
			if (!g_ascii_strncasecmp(value, "area", 4)
					|| !g_ascii_strncasecmp(value, "ar", 2)) {
	...
	char log[90];
	sprintf(log, "Resampled from %d x %d, %s interp%s", fromX, fromY,
			interp_to_str(interpolation),
			((interpolation == OPENCV_LANCZOS4 || interpolation == OPENCV_CUBIC)
					&& clamp) ? ", clamped" : "");
	gfit.history = g_slist_append(gfit.history, strdup(log));
...
int process_rgradient(int nb) {
	if (gfit.orig_bitpix == BYTE_IMG) {
		siril_log_color_message(
				_("This process cannot be applied to 8b images\n"), "red");
		return CMD_INVALID_IMAGE;
	}

	gchar *endxc, *endyc, *enddR, *endda;

	struct rgradient_filter_data *args = malloc(
			sizeof(struct rgradient_filter_data));
	...
	if (endxc == word[1] || endyc == word[2] || enddR == word[3]
		...
		siril_log_message(
				_(
						"The coordinates cannot be greater than the size of the image. "
...
int process_rotate(int nb) {
	...
	if (com.selection.w > 0 && com.selection.h > 0) {
		siril_log_color_message(
				_(
						"Rotation will apply only to current selection, the resulting image will be cropped.\n"),
				"salmon");
		area = com.selection;
		has_selection = TRUE;
	}
	...
	if (end == word[1] || angle < 0.0 || angle > 360.0) {
		siril_log_message(
				_("Angle %lf not allowed. Should be between 0.0 and 360.0.\n"),
				angle);
		return CMD_ARG_ERROR;
	}
	...
	for (int i = 2; i < nb; i++) {
		if (g_str_has_prefix(word[i], "-interp=")) {
			...
			if (has_selection) {
				siril_log_color_message(
						_(
								"-nocrop option is not valid if a selection is active. Ignoring\n"),
						"red");
			} else
				crop = 0;
		} else {
	...
	if (angle == 0.0 || angle == 360.0) {
		siril_log_message(
				_("Angle is 0.0 or 360.0 degrees. Doing nothing...\n"));
		return CMD_ARG_ERROR;
	}
...
int process_set_mag(int nb) {
	...
	if (gui.qphot) {
		...
		if (com.selection.w > 300 || com.selection.h > 300) {
			siril_log_message(
					_(
							"Current selection is too large. To determine the PSF, please make a selection around a single star.\n"));
		...
		psf_star *result = psf_get_minimisation(&gfit, select_vport(gui.cvport),
				&com.selection, TRUE, ps, TRUE,
				com.pref.starfinder_conf.profile, &error);
		free(ps);
		if (result) {
			found = TRUE;
			mag = result->mag;
...
int process_set_photometry(int nb) {
	if (nb > 0) {
		double inner = -1.0, outer = -1.0, aperture = -1.0, gain = -1.0, force =
				-1.0;
		...
		if (inner > 0) {
			if (outer > 0) {
				if (outer > inner) {
					...
					siril_log_message(
							_(
									"Inner radius value must be less than outer. Please change the value."));
					error = TRUE;
				}
			} else {
		...
		if (min >= -65536) {
			if (max > 0) {
				if (min < max) {
					...
					siril_log_message(
							_(
									"minimum value must be smaller than the maximum\n"));
					error = TRUE;
				}
			} else {
			...
			if (com.pref.phot_set.minval < max) {
				...
				siril_log_message(
						_("minimum value must be smaller than the maximum\n"));
				error = TRUE;
			}
		}
	...
	siril_log_message(
			_(
					"Local background annulus radius: %.1f to %.1f, %s: %.1f (%s), camera conversion gain: %f e-/ADU, using pixels with values ]%f, %f[\n"),
			com.pref.phot_set.inner, com.pref.phot_set.outer,
			com.pref.phot_set.force_radius ?
					_("aperture") : _("dynamic aperture"),
			com.pref.phot_set.force_radius ?
					com.pref.phot_set.aperture :
					com.pref.phot_set.auto_aperture_factor,
			com.pref.phot_set.force_radius ?
					_("forced") : _("times the half-FWHM"),
			com.pref.phot_set.gain, com.pref.phot_set.minval,
...
int process_set_ref(int nb) {
	...
	if (end == word[2] || n < 0 || n >= seq->number) {
		siril_log_message(
				_("The reference image must be set between 1 and %d\n"),
				seq->number);
		return CMD_ARG_ERROR;
	}
...
int process_set_mag_seq(int nb) {
	...
	siril_log_message(
			_(
					"Reference magnitude has been set for star %d to %f and will be computed for each image\n"),
			i - 1, mag);
	drawPlot();
	return CMD_OK;
}
			...
			if (current) {
				if (g_str_has_prefix(current, "-radius=")) {
					...
					if (end == value || radius < 3 || radius > 50) {
						siril_log_message(
								_(
										"Wrong parameter values. Radius must be between 3 and 50, aborting.\n"));
						return CMD_ARG_ERROR;
					}
					...
					if (end == value || sigma < 0.05) {
						siril_log_message(
								_(
										"Wrong parameter values. Sigma must be greater than 0.05, aborting\n"));
						return CMD_ARG_ERROR;
					}
					...
					if (end
							== value|| minbeta < 0.0 || minbeta >= MOFFAT_BETA_UBOUND) {
						siril_log_message(
								_(
										"Wrong parameter values. Minimum beta must be greater than or equal to 0.0 and less than %.0f, aborting\n"),
								MOFFAT_BETA_UBOUND);
						return CMD_ARG_ERROR;
					}
					...
					if (end == value || roundness < 0.0 || roundness > 0.95) {
						siril_log_message(
								_(
										"Wrong parameter values. Roundness must be between 0 and 0.95, aborting.\n"));
						return CMD_ARG_ERROR;
					}
					...
					if (end == value || focal_length < 0) {
						siril_log_message(
								_(
										"Wrong parameter values. Focal length must be greater than 0, aborting.\n"));
						return CMD_ARG_ERROR;
					}
					...
					if (end == value || pixel_size_x < 0) {
						siril_log_message(
								_(
										"Wrong parameter values. Pixel size must be greater than 0, aborting.\n"));
						return CMD_ARG_ERROR;
					}
					...
					if (!(g_ascii_strcasecmp(value, "on")))
						...
						siril_log_message(
								_(
										"Wrong parameter values. Relax must be set to on or off, aborting.\n"));
						return CMD_ARG_ERROR;
					}
					...
					if (end == value || convergence < 1 || convergence > 3) {
						siril_log_message(
								_(
										"Wrong parameter values. Convergence must be between 1 and 3, aborting.\n"));
						return CMD_ARG_ERROR;
					}
					...
					siril_log_message(
							_(
									"Resetting findstar parameters to default values.\n"));
					sigma = 1.;
					roundness = 0.5;
...
int process_psf(int nb) {
	...
	if (gfit.naxes[2] > 1 && nb == 1 && com.headless) {
		siril_log_color_message(
				_(
						"Please display the channel on which you want to compute the PSF or use -channel argument\n"),
				"red");
		return CMD_GENERIC_ERROR;
	}
	...
	if (nb == 2) {
		...
		if (word[1] == next || channel > (int) gfit.naxes[2]) {
			siril_log_message(
					_(
							"Please provide the channel number starting from 0 for red\n"));
			return CMD_ARG_ERROR;
		}
	}
	...
	psf_error error;
	struct phot_config *ps = phot_set_adjusted_for_image(&gfit);
	psf_star *result = psf_get_minimisation(&gfit, channel, &com.selection,
			TRUE, ps, TRUE, profile, &error);
	free(ps);
	if (result) {
...
static int parse_star_position_arg(char *arg, sequence *seq, fits *first,
		rectangle *seqpsf_area, gchar **target_descr) {
	rectangle area;
	if (g_str_has_prefix(arg, "-at=") || g_str_has_prefix(arg, "-refat=")) {
		...
		area.w = size;
		area.h = size;
		if (area.x < 0 || area.y < 0 || area.h <= 0 || area.w <= 0
				|| area.x + area.w >= first->rx
				|| area.y + area.h >= first->ry) {
			siril_log_message(
					_(
							"The given coordinates are not in the image, aborting\n"));
			return CMD_ARG_ERROR;
		}
		...
		if (enforce_area_in_image(&area, seq, 0)) {
			siril_log_message(
					_(
							"Selection was not completely inside the first image of the sequence, aborting.\n"));
			return CMD_SELECTION_ERROR;
		}
		if (target_descr && arg[1] != 'a')
			*target_descr = g_strdup_printf("at %d, %d", x, y);
	} else if (g_str_has_prefix(arg,
			"-wcs=") || g_str_has_prefix(arg, "-refwcs=")) {
		char *value;
		if (arg[1] == 'w')
			value = arg + 5;
		else
		...
		*sep++ = '\0';
		...
		if (!has_wcs(first)) {
			siril_log_message(
					_(
							"The reference image of the sequence does not have the WCS information required for star selection by equatorial coordinates\n"));
...
int process_seq_psf(int nb) {
	if (com.script && nb < 4) {
		siril_log_message(
				_("Arguments are not optional when called from a script\n"));
		return CMD_ARG_ERROR;
	}
	if (!com.headless && !sequence_is_loaded() && nb < 4) {
		siril_log_message(
				_(
						"Arguments are optional only if a sequence is already loaded and selection around a star made\n"));
		return CMD_ARG_ERROR;
	}
	...
	if (nb < 4) {
		...
		if (end == word[2] || layer >= seq->nb_layers) {
			siril_log_message(
					_(
							"PSF cannot be computed on channel %d for this sequence of %d channels\n"),
					layer, seq->nb_layers);
			free_sequence(seq, TRUE);
			clearfits(&first);
			return CMD_ARG_ERROR;
	...
	if (framing == ORIGINAL_FRAME) {
		if (com.headless)
			...
			GtkToggleButton *follow = GTK_TOGGLE_BUTTON(
					lookup_widget("followStarCheckButton"));
...
int process_light_curve(int nb) {
	...
	if (autoring) { // we detect the stars in the reference frame
		...
		if (fwhm <= 0.0f) {
			siril_log_color_message(
					_(
							"Could not find stars in the reference image, aborting.\n"),
					"red");
			if (seq != &com.seq)
				free_sequence(seq, TRUE);
			clearfits(&reffit);
		...
		siril_log_message(
				_(
						"Setting inner and outer photometry ring radii to %.1f and %.1f (FWHM is %f)\n"),
		...
		siril_log_message(
				_(
						"Using preconfigured inner and outer photometry ring radii of %.1f and %.1f\n"),
				com.pref.phot_set.inner, com.pref.phot_set.outer);
	}
	...
	if (g_str_has_prefix(word[argidx], "-ninastars=")) {
		...
		if (!seq_has_wcs) {
			siril_log_message(
					_(
							"No image in the sequence was found with the WCS information required for star selection by equatorial coordinates, plate solve the reference or the current\n"));
			free(args);
			if (seq != &com.seq)
				free_sequence(seq, TRUE);
			return CMD_FOR_PLATE_SOLVED;
		}
		if (parse_nina_stars_file_using_WCS(args, file, TRUE, TRUE,
				has_GUI ? &gfit : &reffit)) {
			if (seq != &com.seq)
				free_sequence(seq, TRUE);
		...
		for (int arg_index = argidx; arg_index < nb; arg_index++) {
			if (parse_star_position_arg(word[arg_index], seq,
					has_GUI ? &gfit : &reffit, &args->areas[arg_index - argidx],
					&args->target_descr)) {
				free_light_curve_args(args);
				return CMD_ARG_ERROR;;
			}
...
int process_seq_crop(int nb) {
	...
	if (nb >= startoptargs) {
		...
		siril_log_message(
				_("Crop: select a region or provide x, y, width, height\n"));
		if (!check_seq_is_comseq(seq))
			free_sequence(seq, TRUE);
		return CMD_ARG_ERROR;
	}
	...
	struct crop_sequence_data *args = calloc(1,
			sizeof(struct crop_sequence_data));
...
int process_seq_resample(int nb) {
	...
	struct scale_sequence_data *args = calloc(1,
			sizeof(struct scale_sequence_data));
		...
		if (word[i]) {
			if (g_str_has_prefix(word[i], "-prefix=")) {
				...
				if (toY < 10) {
					siril_log_message(
							_(
									"Error: height cannot be less than 10, aborting.\n"));
					if (!check_seq_is_comseq(seq))
						free_sequence(seq, TRUE);
					free(args->prefix);
					return CMD_ARG_ERROR;
				...
				if (toX < 10) {
					siril_log_message(
							_(
									"Error: width cannot be less than 10, aborting.\n"));
					if (!check_seq_is_comseq(seq))
						free_sequence(seq, TRUE);
					free(args->prefix);
					return CMD_ARG_ERROR;
				...
				if (maxdim < 10) {
					siril_log_message(
							_(
									"Error: max dimension cannot be less than 10, aborting.\n"));
					if (!check_seq_is_comseq(seq))
						free_sequence(seq, TRUE);
					free(args->prefix);
					return CMD_ARG_ERROR;
				...
				if (args->scale < 0.01) {
					siril_log_message(
							_(
									"Error: scale cannot be less than 0.01, aborting.\n"));
					if (!check_seq_is_comseq(seq))
						free_sequence(seq, TRUE);
					free(args->prefix);
					return CMD_ARG_ERROR;
				...
				if (current == end) {
					siril_log_message(
							_(
									"Error parsing argument to -scale= (%s), aborting.\n"),
							current);
					if (!check_seq_is_comseq(seq))
						free_sequence(seq, TRUE);
					free(args->prefix);
				...
				siril_log_message(
						_("Unknown transformation type %s, aborting.\n"),
						value);
				return CMD_ARG_ERROR;
			}
		}
	...
	if (args->scale < 0.1) {
		siril_log_message(
				_("Error: scale cannot be less than 0.01, aborting.\n"));
		free(args->prefix);
		free(args);
		if (!check_seq_is_comseq(seq))
			free_sequence(seq, TRUE);
...
int process_histo(int nb) {
	...
	GOutputStream *output_stream = (GOutputStream*) g_file_replace(file, NULL,
			FALSE, G_FILE_CREATE_NONE, NULL, &error);
...
int process_ffill(int nb) {
	...
	if ((!com.selection.h) || (!com.selection.w)) {
		if (nb == 6) {
			...
			siril_log_message(
					_(
							"Fill2: select a region or provide x, y, width, height\n"));
			return CMD_ARG_ERROR;
		}
	} else {
		area = com.selection;
...
cmd_errors parse_findstar(struct starfinder_data *args, int start, int nb) {
	for (int i = start; i < nb; i++) {
		...
		if (g_str_has_prefix(current, "-out=")) {
			...
			int nb_layers =
					(start == 2) ?
							args->im.from_seq->nb_layers :
							args->im.fit->naxes[2];
			if (nb_layers == 1) {  // handling mono case
				siril_log_message(
						_("This sequence is mono, ignoring layer number.\n"));
				continue;
			}
			value = current + 7;
			gchar *end;
			...
			if (end == value || layer < 0 || layer > 2) {
				siril_log_message(
						_(
								"Unknown layer number %s, must be between 0 and 2, will use green layer.\n"),
						value);
				layer = GLAYER;
				continue;
			}
			...
			if (end
					== value|| max_stars > MAX_STARS_FITTED || max_stars < MIN_STARS_FITTED) {
				// limiting values to avoid too long computation or too low number of candidates
				siril_log_message(
						_(
								"Max number of stars %s not allowed. Should be between %d and %d.\n"),
...
int process_findstar(int nb) {
	...
	if (gfit.naxes[2] == 1 && gfit.keywords.bayer_pattern[0] != '\0') {
		...
		siril_log_color_message(
				_(
						"Warning: an undebayered CFA image is loaded. Star detection may produce results for this image but will not perform optimally and star parameters may be inaccurate.\n"),
				"salmon");
	}
...
int process_seq_findstar(int nb) {
	...
	if (args->starfile) {
		siril_log_message(
				_("Option -out= is not available for sequences, ignoring\n"));
		g_free(args->starfile);
		args->starfile = NULL;
	}
	...
	if (mono && cfa) {
		...
		siril_log_color_message(
				_(
						"Warning: sequence contains undebayered CFA images. Star detection may produce results for this sequence but will not perform optimally and star parameters may be inaccurate.\n"),
				"salmon");
	}
...
int process_findhot(int nb) {
	if (gfit.naxes[2] != 1) {
		siril_log_message(
				_(
						"find_hot must be applied on an one-channel master-dark frame"));
		return CMD_NEED_INIT_FIRST;
	}
	GError *error = NULL;
	long icold, ihot;
...
int process_cosme(int nb) {
	...
	if (retval)
		siril_log_color_message(
				_("There were some errors, please check your input file.\n"),
				"salmon");
...
int process_fmedian(int nb) {
	...
	if (end1 == word[1] || !(args->ksize & 1) || args->ksize < 2
			|| args->ksize > 15) {
		siril_log_message(
				_(
						"The size of the kernel MUST be odd and in the range [3, 15].\n"));
		free(args);
		return CMD_ARG_ERROR;
	}
...
int process_cdg(int nb) {
	...
	if (!FindCentre(&gfit, &x_avg, &y_avg)) {
		siril_log_message(
				_("Center of gravity coordinates are (%.3lf, %.3lf)\n"), x_avg,
				y_avg);
		return CMD_OK;
	}
	return CMD_GENERIC_ERROR;
...
int process_scnr(int nb) {
	...
	if (argidx < nb) {
		...
		if (end == word[argidx] || args->type > 3) {
			siril_log_message(
					_(
							"Type can either be 0 (average neutral) or 1 (maximum neutral), 2 (maximum mask) or 3 (additive mask)\n"));
			free(args);
			return CMD_ARG_ERROR;
		}
		argidx++;
...
int process_subsky(int nb) {
	...
	if (!strcmp(word[arg_index], "-rbf"))
		...
		if (end == word[arg_index] || degree < 1 || degree > 4) {
			siril_log_message(
					_(
							"Polynomial degree order must be within the [1, 4] range.\n"));
			return CMD_ARG_ERROR;
		}
	}
	...
	while (arg_index < nb && word[arg_index]) {
		...
		if (is_sequence && g_str_has_prefix(arg, "-prefix=")) {
			...
			if (interp != BACKGROUND_INTER_RBF)
				siril_log_color_message(
						_(
								"smooth parameter is unused with the polynomial model, ignoring.\n"),
						"salmon");
...
int select_unselect(gboolean select) {
	...
	if (end1 == word[2] || from < 1 || from > seq->number) {
		siril_log_message(
				_(
						"The second argument must be between 1 and the number of images.\n"));
		if (!check_seq_is_comseq(seq))
			free_sequence(seq, TRUE);
		return CMD_ARG_ERROR;
	}
	if (end2 == word[3] || to < from) {
		siril_log_message(
				_(
						"The third argument must be larger or equal than the \"from\" argument.\n"));
		if (!check_seq_is_comseq(seq))
			free_sequence(seq, TRUE);
		return CMD_ARG_ERROR;
	}
	if (to > seq->number) {
		siril_log_color_message(
				_("The third argument is larger than the number of images.\n"),
				"salmon");
	...
	siril_log_message(
			_(
					"Selection update finished, %d images are selected in the sequence\n"),
			seq->selnum);
...
int process_split_cfa(int nb) {
	...
	if (gfit.type == DATA_USHORT) {
		if (!(ret = split_cfa_ushort(&gfit, &f_cfa0, &f_cfa1, &f_cfa2, &f_cfa3))) {
			ret = save1fits16(cfa0, &f_cfa0, 0) || save1fits16(cfa1, &f_cfa1, 0)
					|| save1fits16(cfa2, &f_cfa2, 0)
					|| save1fits16(cfa3, &f_cfa3, 0);
		}
	} else if (gfit.type == DATA_FLOAT) {
		if (!(ret = split_cfa_float(&gfit, &f_cfa0, &f_cfa1, &f_cfa2, &f_cfa3))) {
			ret = save1fits32(cfa0, &f_cfa0, 0) || save1fits32(cfa1, &f_cfa1, 0)
...
int process_seq_mtf(int nb) {
	...
	if (end1 == word[2] || end2 == word[3] || end3 == word[4]
			|| args->params.shadows < 0.0 || args->params.midtones <= 0.0
			|| args->params.highlights <= 0.0 || args->params.shadows >= 1.0
			|| args->params.midtones >= 1.0 || args->params.highlights > 1.0) {
		siril_log_message(_("Invalid argument to %s, aborting.\n"), word[0]);
		free(args->seqEntry);
		free(args);
		if (!check_seq_is_comseq(seq))
...
int process_seq_split_cfa(int nb) {
	...
	struct multi_output_data *args = calloc(1,
			sizeof(struct multi_output_data));
...
int process_seq_extractHa(int nb) {
	...
	struct simple_extract_data *args = calloc(1,
			sizeof(struct simple_extract_data));
...
int process_stat(int nb) {
	...
	gboolean cfa = FALSE;
	int argidx = 1;
	if (nb == 2 && !g_strcmp0(word[1], "-cfa") && nplane == 1
			&& gfit.keywords.bayer_pattern[0] != '\0') {
		siril_debug_print("Running stats on CFA\n");
		nplane = 3;
		cfa = TRUE;
		if ((com.selection.w && com.selection.w < 2)
				|| (com.selection.h && com.selection.h < 2)) {
			siril_log_color_message(
					_(
							"Statistics cannot be made on CFA images with a selection smaller than a 2x2 square, aborting\n"),
					"red");
			return CMD_GENERIC_ERROR;
		}
		argidx++;
	...
	for (layer = 0; layer < nplane; layer++) {
		...
		if (cfa)
			super_layer = -layer - 1;
		imstats *stat = statistics(NULL, -1, &gfit, super_layer, &com.selection,
				STATS_MAIN, MULTI_THREADED);
		if (!stat) {
			siril_log_message(
					_(
							"Statistics computation failed for channel %d (all nil?).\n"),
					layer);
			continue;
		}
		...
		if (option == STATS_BASIC) {
			...
			if (gfit.type == DATA_USHORT) {
				siril_log_message(
						_(
								"%s layer: Mean: %0.1f, Median: %0.1f, Sigma: %0.1f, "
										"Min: %0.1f, Max: %0.1f, bgnoise: %0.1f, "
...
int process_jsonmetadata(int nb) {
	...
	for (int i = 2; i < nb; i++) {
		if (g_str_has_prefix(word[i], "-out=") && word[i][5] != '\0') {
			...
			if (!gfit.rx || !gfit.ry) {
				siril_log_color_message(
						_(
								"No image appears to be loaded, reloading from '%s'\n"),
	...
	int status = 0;
	fitsfile *fptr;
	if (siril_fits_open_diskfile_img(&fptr, input_filename, READONLY,
			&status)) {
	...
	if (compute_stats) {
		if (use_gfit) {
			compute_all_channels_statistics_single_image(&gfit,
					STATS_BASIC | STATS_FOR_CFA, MULTI_THREADED, stats);
			gboolean cfa = gfit.keywords.bayer_pattern[0] != '\0';
			nb_channels = cfa ? 3 : (int) gfit.naxes[2];
		} else {
			fits fit = { 0 };
			fit.fptr = fptr;
			if (read_fits_metadata(&fit)
					|| read_fits_with_convert(&fit, input_filename, FALSE)) {
				fits_close_file(fptr, &status);
				return CMD_GENERIC_ERROR;
			}
			compute_all_channels_statistics_single_image(&fit,
					STATS_BASIC | STATS_FOR_CFA, MULTI_THREADED, stats);
...
int header_hook(struct generic_seq_metadata_args *args, fitsfile *fptr,
		int index) {
	...
	if (args->output_stream) {
		GError *error = NULL;
		if (!g_output_stream_printf(args->output_stream, NULL, NULL, &error,
				"%d,%s\n", index + 1, string->str)) {
			g_warning("%s\n", error->message);
			g_clear_error(&error);
		...
		gchar *output = NULL;
		for (int i = 0;
				i < g_strv_length(token_keys) && token_keys[i]
						&& token_values[i]; i++) {
			gchar *tmp = g_strdup_printf("%s = %s,", token_keys[i],
					token_values[i]);
			if (output == NULL) {
				output = g_strdup(tmp);
			} else {
...
int process_seq_header(int nb) {
	...
	for (key = 2; key < nb; key++) {
		...
		if (key > 2)
			list_of_keys = g_string_append(list_of_keys, ", ");
	...
	for (int i = key; i < nb; i++) {
		if (g_str_has_prefix(word[i], "-out=")) {
			...
			GError *error = NULL;
			output_stream = (GOutputStream*) g_file_replace(file, NULL, FALSE,
					G_FILE_CREATE_NONE, NULL, &error);
			g_object_unref(file);
			if (!output_stream) {
				if (error) {
					siril_log_color_message(
							_("Failed to create output file: %s\n"), "red",
							error->message);
					g_clear_error(&error);
				}
				return CMD_ARG_ERROR;
			}
			if (!g_output_stream_printf(output_stream, NULL, NULL, NULL,
					"# image number,%s\n", header)) {
	...
	struct generic_seq_metadata_args *args = calloc(1,
			sizeof(struct generic_seq_metadata_args));
	args->seq = seq;
	args->keys = list;
...
int process_link(int nb) {
	if (word[1][0] == '-') {
		siril_log_message(
				_(
						"First argument is the converted sequence name and shall not start with a -\n"));
		return CMD_ARG_ERROR;
	}
		...
		if (g_str_has_prefix(current, "-start=")) {
			...
			if (!g_file_test(value, G_FILE_TEST_EXISTS)) {
				if (g_mkdir_with_parents(value, 0755) < 0) {
					siril_log_color_message(
							_("Cannot create output folder: %s\n"), "red",
							value);
					free(destroot);
					return CMD_GENERIC_ERROR;
				}
	...
	if (sort_dateobs) {
		...
		while (cur) {
			...
			if (!date) {
				siril_log_color_message(
						_(
								"Sorting by date could not be done, file %s doesn't have DATE-OBS, not sorting\n"),
						"red", filename);
				failed = TRUE;
				break;
			}
	...
	if (!allow_to_open_files(count, &nb_allowed)) {
		siril_log_message(
				_(
						"You should pass an extra argument -fitseq to convert your sequence to fitseq format.\n"));
...
int process_convert(int nb) {
	...
	if ((dir = g_dir_open(com.wd, 0, &error)) == NULL) {
		siril_log_message(
				_("Conversion: error opening working directory %s.\n"), com.wd);
	...
	while ((file = g_dir_read_name(dir)) != NULL) {
		...
		if (!ext || file[0] == '.')
			continue;
		image_type type = get_type_for_extension(ext);
		if (type == TYPERAW && output == SEQ_SER
				&& !g_ascii_strcasecmp(ext, "raf") && !debayer) {
			siril_log_message(
					_(
							"FujiFilm XTRANS sensors are not supported by SER v2 (CFA-style) standard. You may use FITS sequences instead."));
			g_list_free_full(list, g_free);
			free(destroot);
			return CMD_GENERIC_ERROR;
		}
		if ((raw_only && type == TYPERAW)
				|| (!raw_only && type != TYPEUNDEF && type != TYPEAVI
						&& type != TYPESER)) {
			list = g_list_append(list, g_build_filename(com.wd, file, NULL));
			count++;
		}
	...
	if (!count) {
		if (raw_only)
			...
			siril_log_message(_("No files were found for conversion\n"));
		free(destroot);
		return CMD_GENERIC_ERROR;
	}
	/* sort list */
...
int process_register(int nb) {
		...
		if (!strcmp(word[i], "-2pass")) {
			...
			if (!check_seq_is_comseq(seq)) { // processing an image from the current sequence
				...
				if (seq_read_frame_metadata(seq, image_to_load, preffit)) {
					siril_log_message(
							_(
									"Could not load the reference image of the sequence, aborting.\n"));
					goto terminate_register_on_error;
				}
			} else
				preffit = &gfit;
			gchar *msgdisto = NULL;
			gboolean disto_valid = validate_disto_params(preffit, filename,
					regargs->undistort, &msgdisto, NULL);
			if (preffit != &gfit)
				clearfits(preffit);
			if (!disto_valid) {
			...
			if (regargs->seq->nb_layers != 1) {  // handling mono case
				siril_log_message(
						_(
								"This sequence is not mono / CFA, cannot drizzle.\n"));
				goto terminate_register_on_error;
			}
			drizzle = TRUE;
			// Drizzle options
			...
			if (end == arg || value < 0.1 || value > 3.) {
				siril_log_color_message(
						_("Invalid argument to %s, aborting.\n"), "red",
						word[i]);
				goto terminate_register_on_error;
			}
			...
			if (driz->flat) {
				siril_log_color_message(
						_("Error: flat image already set. Aborting.\n"), "red");
				goto terminate_register_on_error;
			}
			if (seq->is_variable) {
				siril_log_color_message(
						_(
								"Error: flat image cannot work with variable sized sequence.\n"),
						"red");
				goto terminate_register_on_error;
			}
			...
			gchar *error = NULL;
			int status;
			if (seq_read_frame_metadata(seq, regargs->reference_image,
					&reffit)) {
				siril_log_color_message(
						_("NOT USING FLAT: Could not load reference image\n"),
						"red");
				clearfits(&reffit);
				goto terminate_register_on_error;
			}
			gchar *expression = path_parse(&reffit, flat_filename,
					PATHPARSE_MODE_READ, &status);
			clearfits(&reffit);
			if (status) {
				...
				if (expression[0] == '\0') {
					siril_log_message(
							_(
									"Error: no master flat specified in the preprocessing tab.\n"));
					goto terminate_register_on_error;
				} else {
					...
					if (!readfits(expression, driz->flat, NULL, TRUE)) {
						if (driz->flat->naxes[2] != seq->nb_layers) {
							...
							siril_log_message(
									_(
											"Master flat read for use as initial pixel weight\n"));
						}

					} else
						error = _("NOT USING FLAT: cannot open the file");
					if (error) {
						goto terminate_register_on_error;
					}
				}
	...
	if (drizzle) {
		...
		if (!check_seq_is_comseq(seq)) { // processing an image from the current sequence
			...
			preffit = &gfit;
		if (preffit->naxes[2] == 1
				&& preffit->keywords.bayer_pattern[0] != '\0') {
	...
	if (!regargs->no_output) {
		int nb_frames =
				regargs->filters.filter_included ?
						regargs->seq->selnum : regargs->seq->number;
		int64_t size = seq_compute_size(regargs->seq, nb_frames,
				get_data_type(seq->bitpix));
		if (regargs->output_scale != 1.f)
			size = (int64_t) (regargs->output_scale * regargs->output_scale
					* (float) size);
		if (test_available_space(size)) {
			siril_log_color_message(
					_("Not enough space to save the output images, aborting\n"),
					"red");
			goto terminate_register_on_error;
		}
	} else if (regargs->output_scale != 1.f) {
		siril_log_color_message(
				_("Scaling a sequence with -2pass has no effect, ignoring\n"),
				"salmon");
	}

	if (regargs->interpolation == OPENCV_NONE
			&& !(regargs->type == SHIFT_TRANSFORMATION)) {
#ifdef HAVE_CV44
		regargs->type = SHIFT_TRANSFORMATION;
		siril_log_color_message(
				_(
						"Forcing the registration transformation to shift, which is the only transformation compatible with no interpolation\n"),
				"salmon");
	...
	if (regargs->interpolation == OPENCV_NONE
			&& (regargs->output_scale != 1.f || regargs->seq->is_variable)) {
		siril_log_color_message(
				_(
						"When interpolation is set to None, the images must be of same size and no scaling can be applied. Aborting\n"),
				"red");
		goto terminate_register_on_error;
	}
	...
	if (regargs->interpolation == OPENCV_AREA
			|| regargs->interpolation == OPENCV_LINEAR
			|| regargs->interpolation == OPENCV_NEAREST
			|| regargs->interpolation == OPENCV_NONE || regargs->no_output
			|| drizzle)
		regargs->clamp = FALSE;
	if (regargs->clamp)
	...
	terminate_register_on_error: free(regargs);
	if (!check_seq_is_comseq(seq)) {
		free_sequence(seq, TRUE);
	}
	free(driz);
	...
	if (g_str_has_prefix(current, "-filter-fwhm=")) {
		...
		if (value[0] != '\0') {
			...
			if (end == value) {
				siril_log_message(
						_(
								"Could not parse argument `%s' to the filter `%s', aborting.\n"),
						value, current);
				return CMD_ARG_ERROR;
			}
			if (*end == '%' || *end == 'k') {
...
int process_seq_applyreg(int nb) {
	...
	if (layer == -1) {
		siril_log_color_message(
				_("No registration data exists for this sequence, aborting\n"),
				"red");
		goto terminate_register_on_error;
	}
	...
	for (int i = 2; i < nb; i++) {
		if (!strcmp(word[i], "-drizzle")) {
			...
			if (!(seq->regparam[layer2])) {
				siril_log_color_message(
						_(
								"Registration data does not exist for layer #%d, will use layer #%d instead.\n"),
						"red", layer2, layer);
				continue;
			}
	...
	set_progress_bar_data(_("Registration: Applying existing data"),
			PROGRESS_RESET);

	start_in_new_thread(register_thread_func, regargs);
	return CMD_OK;

	terminate_register_on_error: free(driz);
	if (!check_seq_is_comseq(seq))
		free_sequence(seq, TRUE);
	free(regargs->prefix);
...
static int parse_stack_command_line(struct stacking_configuration *arg,
		int first, gboolean med_options_allowed, gboolean rej_options_allowed,
	...
	while (word[first]) {
		...
		if (!strcmp(current, "-nonorm") || !strcmp(current, "-no_norm"))
			...
			if (!med_options_allowed) {
				siril_log_message(
						_(
								"Output normalization is allowed only with median or mean stacking, ignoring.\n"));
			} else {
			...
			if (!rej_options_allowed) {
				siril_log_message(
						_(
								"Weighting is allowed only with average stacking, ignoring.\n"));
			} else if (arg->norm == NO_NORM) {
				siril_log_message(
						_(
								"Weighting is allowed only if normalization has been activated, ignoring.\n"));
			} else if (arg->apply_nbstack_weights || arg->apply_wfwhm_weights
					|| arg->apply_nbstars_weights) {
			...
			if (!rej_options_allowed) {
				siril_log_message(
						_(
								"Weighting is allowed only with average stacking, ignoring.\n"));
			} else if (arg->apply_nbstack_weights || arg->apply_noise_weights
					|| arg->apply_nbstars_weights) {
				siril_log_message(_("Only one weighting method can be used\n"));
			} else {
			...
			if (!rej_options_allowed) {
				...
				siril_log_message(
						_(
								"Weighting is allowed only if normalization has been activated, ignoring.\n"));
			} else if (arg->apply_noise_weights || arg->apply_wfwhm_weights
					|| arg->apply_nbstars_weights) {
				siril_log_message(_("Only one weighting method can be used\n"));
			} else {
			...
			if (!med_options_allowed) {
				siril_log_message(
						_(
								"Fast normalization is allowed only with average stacking, ignoring.\n"));
			} else if (arg->norm == NO_NORM) {
				siril_log_message(
						_(
								"Fast normalization is allowed only if normalization has been activated, ignoring.\n"));
			} else {
				arg->lite_norm = TRUE;
			}
			...
			if (!med_options_allowed) {
				siril_log_message(
						_(
								"Normalization options are not allowed in this context, ignoring.\n"));
			} else {
				value = current + 6;
				if (!strcmp(value, "add"))
					arg->norm = ADDITIVE;
			...
			if (!med_options_allowed) {
				siril_log_message(
						_(
								"RGB equalization is allowed only with average stacking, ignoring.\n"));
			} else if (arg->norm == NO_NORM) {
				siril_log_message(
						_(
								"RGB equalization is allowed only if normalization has been activated, ignoring.\n"));
			} else {
			...
			if (out_allowed) {
				...
				siril_log_message(
						_(
								"Output filename option is not allowed in this context, ignoring.\n"));
			}
		} else if (g_str_has_prefix(current, "-rejmap")) {
			if (!rej_options_allowed) {
				siril_log_message(
						_(
								"Rejection maps can only be created with rejection stacking, ignoring.\n"));
			} else if (arg->type_of_rejection == NO_REJEC) {
				siril_log_message(
						_(
								"Rejection maps can only be created if rejection has been activated, ignoring.\n"));
			} else {
			...
			siril_log_message(
					_("Unexpected argument to stacking `%s', aborting.\n"),
					current);
			return CMD_ARG_ERROR;
		}
		first++;
...
static int stack_one_seq(struct stacking_configuration *arg) {
	...
	if (!test_regdata_is_valid_and_shift(args.seq, args.reglayer)) {
		siril_log_color_message(
				_(
						"Stacking has detected registration data on layer %d with more than simple shifts. You should apply existing registration before stacking\n"),
				"red", args.reglayer);
		free_sequence(seq, TRUE);
		return CMD_GENERIC_ERROR;
	}
	...
	if (args.maximize_framing && !can_reframe) {
		siril_log_color_message(
				_(
						"No registration data in the sequence. Maximize framing will be ignored\n"),
				"red");
	...
	if (!args.maximize_framing && must_reframe) {
		siril_log_color_message(
				_(
						"The sequence has different image sizes and registration data. Forcing to maximize framing\n"),
				"red");
	...
	if (!can_reframe && seq->is_variable) {
		siril_log_color_message(
				_(
						"The sequence has different image sizes but no registration data, cannot stack. Aborting\n"),
				"red");
		free_sequence(seq, TRUE);
		return CMD_GENERIC_ERROR;
	}
	if (args.upscale_at_stacking && !can_upscale) {
		siril_log_color_message(
				_(
						"No registration data in the sequence or images with different sizes. Upscale at stacking will be ignored\n"),
				"red");
		args.upscale_at_stacking = FALSE;
	}
	if ((args.upscale_at_stacking || args.maximize_framing)
			&& arg->method == stack_median) {
		siril_log_color_message(
				_(
						"Cannot upscale or maximize framong with median stacking. Disabling\n"),
				"red");
		args.maximize_framing = FALSE;
		args.upscale_at_stacking = FALSE;
	}
	...
	args.description = describe_filter(seq, args.filtering_criterion,
	...
	if (!retval) {
		...
		if (!arg->result_file) {
			...
			char *suffix =
					...
					g_str_has_suffix(seq->seqname, "-") ? "" : "_";
			snprintf(filename, maxpath - 1, "%s%sstacked%s", seq->seqname,
					suffix, com.pref.ext);
			...
			gchar *parsedname = update_header_and_parse(&args.result,
					expression, PATHPARSE_MODE_WRITE_NOFAIL, TRUE, &status);
			char filename[maxpath];
		...
		if (savefits(arg->result_file, &args.result)) {
			siril_log_color_message(
					_("Could not save the stacking result %s\n"), "red",
					arg->result_file);
			retval = CMD_GENERIC_ERROR;
		} else
		...
		if (args.create_rejmaps) {
			...
			if (args.merge_lowhigh_rejmaps) {
				...
				soper_unscaled_div_ushort_to_float(args.rejmap_low,
						args.nb_images_to_stack);
				describe_stack_for_history(&args, &args.rejmap_low->history,
						TRUE, FALSE);
				...
				soper_unscaled_div_ushort_to_float(args.rejmap_high,
						args.nb_images_to_stack);
				describe_stack_for_history(&args, &args.rejmap_low->history,
						TRUE, FALSE);
		...
		siril_log_color_message(
				_("Stacking failed, please check the log to fix your issue.\n"),
				"red");
	}
...
static gpointer stackall_worker(gpointer garg) {
	...
	siril_log_message(
			_("Looking for sequences in current working directory...\n"));
	if (check_seq() || (dir = g_dir_open(com.wd, 0, &error)) == NULL) {
		siril_log_message(
				_(
						"Error while searching sequences or opening the directory.\n"));
		if (error) {
...
int process_stackall(int nb) {
	...
	if (!word[1]) {
		...
		if (!strcmp(word[1], "sum")) {
			...
			} else if (!strcmp(word[3], "w")
					|| !strcmp(word[3], "winsorized")) {
				arg->type_of_rejection = WINSORIZED;
			} else if (!strcmp(word[3], "g")
					|| !strcmp(word[3], "generalized")) {
				arg->type_of_rejection = GESDT;
			} else {
				arg->type_of_rejection = WINSORIZED;
				shift = 0;
			}
			if (!word[2 + shift] || !word[3 + shift] || (arg->sig[0] =
					g_ascii_strtod(word[2 + shift], &end1)) < 0.0
					|| (arg->sig[1] = g_ascii_strtod(word[3 + shift], &end2))
							< 0.0 || end1 == word[2 + shift]
					|| end2 == word[3 + shift]) {
				siril_log_color_message(
						_(
								"The average stacking with rejection requires two extra arguments: sigma low and high.\n"),
						"red");
				goto failure;
			}
			if (((arg->type_of_rejection == GESDT))
				...
				siril_log_color_message(
						_(
								"Extra parameters of GESDT rejection algorithm must be between 0 and 1, default is 0.3 and 0.05.\n"),
						"red");
				goto failure;
			}
			if (((arg->type_of_rejection == PERCENTILE))
				...
				siril_log_color_message(
						_(
								"Extra parameters of percentile rejection algorithm must be between 0 and 1, default is 0.2 and 0.1.\n"),
						"red");
				goto failure;
			}
			arg->method = stack_mean_with_rejection;
		...
		if (parse_stack_command_line(arg, start_arg_opt, allow_med_options,
				allow_rej_options, FALSE))
			goto failure;
	}
	...
	failure: g_free(arg->result_file);
	g_free(arg->seqfile);
	free(arg);
	return CMD_ARG_ERROR;
}
...
static gpointer stackone_worker(gpointer garg) {
	...
	if (retval) {
		if (retval == ST_ALLOC_ERROR) {
			siril_log_message(
					_(
							"It looks like there is a memory allocation error, change memory settings and try to fix it.\n"));
		}
	} else {
...
int process_stackone(int nb) {
	struct stacking_configuration *arg = calloc(1,
			sizeof(struct stacking_configuration));
	arg->norm = NO_NORM;
	...
	if (!word[2]) {
		...
		if (!strcmp(word[2], "sum")) {
			...
			if (nb < 4) {
				siril_log_color_message(
						_("Missing arguments for rejection stacking.\n"),
						"red");
				goto failure;
			}
			if (!strcmp(word[3], "p") || !strcmp(word[3], "percentile")) {
			...
			if ((nb < 4 + shift + 1) || !word[3 + shift] || !word[4 + shift]
					...
					|| !string_is_a_number(word[4 + shift]) || (arg->sig[0] =
							g_ascii_strtod(word[3 + shift], &end1)) < 0.0
					|| (arg->sig[1] = g_ascii_strtod(word[4 + shift], &end2))
							< 0.0 || end1 == word[3 + shift]
					|| end2 == word[4 + shift]) {
...
struct preprocessing_data* parse_preprocess_args(int nb, sequence *seq) {
	int retvalue = 0;
	struct preprocessing_data *args = calloc(1,
			sizeof(struct preprocessing_data));
	fits reffit = { 0 };
	int bitpix;
	char *realname = NULL;
	image_type imagetype;
	if (seq) {
		...
		if (stat_file(word[1], &imagetype, &realname)) {
			siril_log_color_message(
					_(
							"Error opening image %s: file not found or not supported.\n"),
					"red", word[1]);
		...
		if (g_str_has_prefix(word[i], "-bias=")) {
			...
			if (expression && expression[0] == '=') {
				...
				if (!offsetlevel) {
					siril_log_message(
							_(
									"The offset value could not be parsed from expression: %s, aborting.\n"),
							expression + 1);
					retvalue = 1;
					g_free(expression);
					break;
					...
					if ((offsetlevel > maxlevel) || (offsetlevel < -maxlevel)) { // not excluding all neg values here to allow defining a pedestal
						siril_log_message(
								_(
										"The offset value is out of allowable bounds [-%d,%d], aborting.\n"),
								maxlevel, maxlevel);
						...
						args->bias_level *=
								(bitpix == BYTE_IMG) ?
										INV_UCHAR_MAX_SINGLE :
										INV_USHRT_MAX_SINGLE; //converting to [0 1] to use with soper
						args->use_bias = TRUE;
					}
				}
			} else {
				...
				expression = NULL;
				int status;
				expression = path_parse(&reffit, word[i] + 6,
						PATHPARSE_MODE_READ, &status);
				if (status) {
					g_free(expression);
				...
				if (!readfits(expression, args->bias, NULL,
						!com.pref.force_16bit)) {
					args->use_bias = TRUE;
					// if input is 8b, we assume 32b master needs to be rescaled
					if ((args->bias->type == DATA_FLOAT)
							&& (bitpix == BYTE_IMG)) {
						soper(args->bias, USHRT_MAX_SINGLE / UCHAR_MAX_SINGLE,
								OPER_MUL, TRUE);
					}
				} else {
					retvalue = CMD_INVALID_IMAGE;
			...
			int status;
			gchar *expression = path_parse(&reffit, word[i] + 6,
					PATHPARSE_MODE_READ, &status);
			...
			if (!readfits(expression, args->dark, NULL,
					!com.pref.force_16bit)) {
				args->use_dark = TRUE;
				// if input is 8b, we assume 32b master needs to be rescaled
				if ((args->dark->type == DATA_FLOAT) && (bitpix == BYTE_IMG)) {
					soper(args->dark, USHRT_MAX_SINGLE / UCHAR_MAX_SINGLE,
							OPER_MUL, TRUE);
				}
			} else {
				retvalue = CMD_INVALID_IMAGE;
			...
			if (!readfits(expression, args->flat, NULL,
					!com.pref.force_16bit)) {
				args->use_flat = TRUE;
			...
			args->ppprefix = strdup(value);
				...
				"-opt") || g_str_has_prefix(word[i], "-opt=")) {
			if (bitpix == BYTE_IMG) {
				siril_log_color_message(
						_(
								"Dark optimization: This process cannot be applied to 8b images\n"),
						"red");
				retvalue = CMD_INVALID_IMAGE;
				break;
			}
			...
			if (word[i][4] == '=') {
				...
				if (value[0] == '\0' || strcmp(value, "exp")) {
					siril_log_message(
							_("Missing or wrong argument to %s, aborting.\n"),
							current);
					retvalue = CMD_ARG_ERROR;
					break;
				}
			...
			if (!strcmp(value, "dark")) {
				if (!args->use_dark) {
					siril_log_message(
							_(
									"You must specify a masterdark with -dark= before activating this option, aborting.\n"));
					retvalue = CMD_ARG_ERROR;
					break;
				}
				...
				if (args->sigma[0] > 0)
					siril_log_message(
							_(
									"Cosmetic correction from masterdark: using sigma %.2lf for cold pixels.\n"),
							args->sigma[0]);
				else
					siril_log_message(
							_(
									"Cosmetic correction from masterdark: deactivated for cold pixels.\n"));
				if (args->sigma[1] > 0)
					siril_log_message(
							_(
									"Cosmetic correction from masterdark: using sigma %.2lf for hot pixels.\n"),
							args->sigma[1]);
				else
					siril_log_message(
							_(
									"Cosmetic correction from masterdark: deactivated for hot pixels.\n"));
			} else if (!strcmp(value, "bpm")) {
				if (word[i + 1] && word[i + 1][0] != '\0') {
					args->bad_pixel_map_file = g_file_new_for_path(word[i + 1]);
					if (!check_for_cosme_file_sanity(
							args->bad_pixel_map_file)) {
						//g_object_unref(args->bad_pixel_map_file); // This is unreferenced in check_for_cosme_file_sanity
						args->bad_pixel_map_file = NULL;
						siril_log_message(
								_("Could not open file %s, aborting.\n"),
								word[i + 1]);
						retvalue = 1;
						break;
					} else {
						siril_log_message(
								_(
										"Cosmetic correction from Bad Pixel Map: %s.\n"),
								word[i + 1]);
						i++;
					}
				} else {
					siril_log_message(
							_(
									"You must specify a bad pixel map file with -cc=bpm option, aborting.\n"));
					retvalue = CMD_ARG_ERROR;
					break;
				}
			} else {
	...
	prepro_parse_end: clearfits(&reffit);
	free(realname);
	if (retvalue) {
...
int process_set_32bits(int nb) {
	...
	if (com.pref.force_16bit)
		siril_log_message(
				_("16-bit per channel in processed images mode is active\n"));
	else
		siril_log_message(
				_("32-bit per channel in processed images mode is active\n"));
	writeinitfile();
	return CMD_OK;
}

int process_set_compress(int nb) {
	if (word[1][0] != '0' && word[1][0] != '1' && word[1][0] != 'y'
			&& word[1][0] != 'n' && word[1][1] != '\0') {
		siril_log_message(_("Invalid argument %s, aborting.\n"), word[1]);
		return CMD_ARG_ERROR;
	}
	...
	if (compress) {
		if (!word[2] || !word[3] || (!g_str_has_prefix(word[2], "-type="))) {
			siril_log_message(
					_(
							"Please specify the type of compression and quantization value.\n"));
			return CMD_ARG_ERROR;
		}
		gchar *comp = NULL;
...
//			siril_log_message(_("Wrong type of compression. Choices are rice, gzip1, gzip2 or hcompress\n"));
			siril_log_message(
					_(
							"Wrong type of compression. Choices are rice, gzip1, gzip2\n"));
			return CMD_ARG_ERROR;
		}
		if (!word[3]) {
		...
		if (end == word[3]
				|| (q == 0.0
						&& (method == RICE_COMP || method == HCOMPRESS_COMP))) {
			siril_log_message(
					_(
							"Quantization can only be equal to 0 for GZIP1 and GZIP2 algorithms.\n"));
			g_free(comp);
		...
		siril_log_message(
				_(
						"Compression enabled with the %s algorithm and a quantization value of %.2lf\n"),
				comp, q);
		g_free(comp);
	} else {
...
int process_set_mem(int nb) {
	...
	if (end == word[1] || ratio < 0.05 || ratio > 4.0) {
		siril_log_message(
				_(
						"The accepted range for the ratio of memory used for stacking is [0.05, 4], with values below the available memory recommended\n"));
		return CMD_ARG_ERROR;
	}
	if (ratio > 1.0) {
		siril_log_message(
				_(
						"Setting the ratio of memory used for stacking above 1 will require the use of on-disk memory, which can be very slow and is unrecommended (%g requested)\n"),
				ratio);
	}
...
int process_extract(int nb) {
	...
	struct wavelets_filter_data *args = malloc(
			sizeof(struct wavelets_filter_data));
...
int process_requires(int nb) {
	...
	if (new_enough < 0) {
		siril_log_color_message(
				_(
						"A newer version (>= %lu.%lu.%lu-%s%lu) of Siril is required in order to run this script: please update your installation.\n"),
				"red", required_version.major_version,
				required_version.minor_version, required_version.micro_version,
				required_version.beta_version ? _("beta") :
				required_version.rc_version ? _("RC") : "",
				required_version.patched_version);
		return CMD_GENERIC_ERROR;
	} else if (too_new >= 0) {
		siril_log_color_message(
				_(
						"This script has been marked as obsolete for this version of Siril, please check for an update to the script.\n"),
				"red");
		return CMD_GENERIC_ERROR;
	}
	siril_log_message(
			_("OK! This script is compatible with this version of Siril.\n"));
	return CMD_OK;
}
...
int process_boxselect(int nb) {
	...
	if (nb == 1) {
		if (com.selection.w > 0 && com.selection.h > 0)
			siril_log_message(
					_("Current selection [x, y, w, h]: %d %d %d %d\n"),
					com.selection.x, com.selection.y, com.selection.w,
					com.selection.h);
		else
			siril_log_message(_("No current selection in image\n"));
		return CMD_OK;
	}
	...
	if (nb > 1 && !strcmp(word[1], "-clear")) {
		if (nb > 2) {
			siril_log_message(
					_(
							"Too many arguments to boxselect, use either -clear or the coordinates, not both.\n"));
			return CMD_ARG_ERROR;
		}
		delete_selected_area();
	...
	if (x + w > gfit.rx || y + h > gfit.ry) {
		siril_log_message(
				_(
						"The provided coordinates are outside the dimension of the currently loaded image (%d x %d).\n"),
				gfit.rx, gfit.ry);
		return CMD_ARG_ERROR;
	}
...
int process_rgbcomp(int nb) {
	...
	if (g_str_has_prefix(word[1], "-lum=")) {
		...
		fits l = { 0 };
		if (readfits(lum_file, &l, NULL, TRUE))
			return CMD_INVALID_IMAGE;
		if (readfits(word[2], &r, NULL, TRUE)) {
			clearfits(&l);
			return CMD_INVALID_IMAGE;
		}
		...
		if (l.naxes[2] != 1 || check_loaded_fits_params(&l, &r, &g, &b, NULL)) {
			...
			siril_log_message(
					_(
							"Image must all have the same dimensions and be monochrome\n"));
			return CMD_ARG_ERROR;
		}
		...
		rgb_extract_last_options(next_arg, &result_filename, "composed_lrgb",
				&do_sum);

		if (had_an_rgb_image)
			merge_fits_headers_to_result(rgbptr, do_sum, &l, &r, NULL);
		else
			merge_fits_headers_to_result(rgbptr, do_sum, &l, &r, &g, &b, NULL);
		rgbptr->history = g_slist_append(rgbptr->history,
				strdup("LRGB composition"));
		...
		if (readfits(word[1], &r, NULL, TRUE))
			return CMD_INVALID_IMAGE;
		if (readfits(word[2], &g, NULL, TRUE)) {
			clearfits(&r);
			return CMD_INVALID_IMAGE;
		}
		if (readfits(word[3], &b, NULL, TRUE)) {
			clearfits(&r);
			clearfits(&g);
			return CMD_INVALID_IMAGE;
		}
		...
		next_arg = 4;
		rgb_extract_last_options(next_arg, &result_filename, "composed_rgb",
				&do_sum);

		merge_fits_headers_to_result(rgbptr, do_sum, &r, &g, &b, NULL);
		rgbptr->history = g_slist_append(rgbptr->history,
				strdup("RGB composition"));
	...
	siril_log_message(
			_(
					"Successful RGB composition with metadata accumulate option %s.\n"),
...
static int do_pcc(int nb, gboolean spectro) {
	if (!has_wcs(&gfit)) {
		siril_log_color_message(
				_("This command only works on plate solved images\n"), "red");
		return CMD_FOR_PLATE_SOLVED;
	}
	siril_cat_index cat = CAT_AUTO;
	double mag_offset = 0.0, target_mag = -1.0;
	gchar *monosensor = NULL, *oscsensor = NULL, *rfilter = NULL, *gfilter =
			NULL, *bfilter = NULL, *oscfilter = NULL, *osclpf = NULL,
			*whiteref = NULL;
	gchar *spcc_strings_to_free[8] = { oscsensor, oscfilter, osclpf, monosensor,
			rfilter, gfilter, bfilter, whiteref };
	...
	double obsheight =
			gfit.keywords.siteelev != DEFAULT_DOUBLE_VALUE ?
					gfit.keywords.siteelev : 10.0;
	...
	while (nb > next_arg && word[next_arg]) {
		if (g_str_has_prefix(word[next_arg], "-bgtol=")) {
			...
			if (end == arg || end == arg2 || t0 < 0.1 || t0 > 10.0 || t1 < 0.1
					|| t1 > 10.0) {
				siril_log_message(_("Invalid argument to %s, aborting.\n"),
						word[next_arg]);
				for (int z = 0; z < 8; z++) {
					g_free(spcc_strings_to_free[z]);
				}
				return CMD_ARG_ERROR;
			}
			t0 = 0.0 - t0;
	...
	if (gfit.keywords.wcslib->lin.dispre == NULL) {
		siril_log_message(
				_(
						"Found linear plate solve data, you may need to solve your image with distortions to ensure correct calibration of stars near image corners.\n"));
	}
	...
	if (spectro && nb_mode) {
		...
		if (wl[RLAYER] < 380.0 || wl[RLAYER] > 700.0 || wl[GLAYER] < 380.0
				|| wl[GLAYER] > 700.0 || wl[BLAYER] < 380.0
				|| wl[BLAYER] > 700.0) {
			siril_log_message(
					_(
							"NB wavelength out of range (must be 380 <= wl <= 700)\n"));
			for (int z = 0; z < 8; z++) {
				g_free(spcc_strings_to_free[z]);
			}
			return CMD_ARG_ERROR;
		}
		if (bw[RLAYER] < 1.0 || bw[RLAYER] > 40.0 || bw[GLAYER] < 1.0
				|| bw[GLAYER] > 40.0 || bw[BLAYER] < 1.0 || bw[BLAYER] > 40.0) {
			siril_log_message(
					_(
							"NB bandwidth out of range (must be 1.0 <= wl <= 40.0)\n"));
			for (int z = 0; z < 8; z++) {
				g_free(spcc_strings_to_free[z]);
			}
			return CMD_ARG_ERROR;
	...
	if (!spectro && local_cat && cat != CAT_LOCAL) {
		siril_log_color_message(
				_("Using remote %s instead of local NOMAD catalogue\n"),
	...
	if (spectro) {
		...
		if (oscsensor || mono_or_osc == 1) {
			pcc_args->selected_sensor_osc = get_favourite_oscsensor(
					com.spcc_data.osc_sensors,
					oscsensor ? oscsensor : com.pref.spcc.oscsensorpref);
			GList *osc = g_list_nth(com.spcc_data.osc_sensors,
					pcc_args->selected_sensor_osc);
			if (osc) {
			...
			pcc_args->selected_filter_osc = get_favourite_spccobject(
					com.spcc_data.osc_filters,
					oscfilter ? oscfilter :
					com.pref.spcc.oscfilterpref ?
							com.pref.spcc.oscfilterpref : "No filter");
			pcc_args->selected_filter_lpf = get_favourite_spccobject(
					com.spcc_data.osc_lpf,
					osclpf ? osclpf :
					com.pref.spcc.lpfpref ?
							com.pref.spcc.lpfpref : "Full spectrum");
			if (pcc_args->selected_sensor_osc == -1
					|| pcc_args->selected_filter_osc == -1
					|| (pcc_args->is_dslr && pcc_args->selected_filter_lpf == -1)) {
				siril_log_message(
						_(
								"Either the sensor or a filter / LPF was not specified as argument or guessable from previous use. Ensure all necessary data is set.\n"));
				for (int z = 0; z < 8; z++) {
					g_free(spcc_strings_to_free[z]);
				}
				free(pcc_args);
			...
			if (dslr)
				siril_log_message(
						_(
								"SPCC will use OSC sensor \"%s\", filter \"%s\" and DSLR LPF \"%s\"\n"),
						oscsensor ? oscsensor : com.pref.spcc.oscsensorpref,
						oscfilter ? oscfilter : com.pref.spcc.oscfilterpref,
						osclpf ? osclpf : com.pref.spcc.lpfpref);
			else
				siril_log_message(
						_(
								"SPCC will use OSC sensor \"%s\" and filter \"%s\"\n"),
						oscsensor ? oscsensor : com.pref.spcc.oscsensorpref,
						oscfilter ? oscfilter : com.pref.spcc.oscfilterpref);
			pcc_args->is_dslr = dslr == 1 ? TRUE :
								dslr == -1 ? FALSE : com.pref.spcc.is_dslr;
		} else {
			pcc_args->selected_sensor_m = get_favourite_spccobject(
					com.spcc_data.mono_sensors,
					monosensor ? monosensor : com.pref.spcc.monosensorpref);
			pcc_args->selected_filter_r = get_favourite_spccobject(
					com.spcc_data.mono_filters[0],
					rfilter ? rfilter : com.pref.spcc.redpref);
			pcc_args->selected_filter_g = get_favourite_spccobject(
					com.spcc_data.mono_filters[1],
					gfilter ? gfilter : com.pref.spcc.greenpref);
			pcc_args->selected_filter_b = get_favourite_spccobject(
					com.spcc_data.mono_filters[2],
					bfilter ? bfilter : com.pref.spcc.bluepref);
			if (pcc_args->selected_sensor_m == -1
					|| pcc_args->selected_filter_r == -1
					|| pcc_args->selected_filter_g == -1
					|| pcc_args->selected_filter_b == -1) {
				siril_log_message(
						_(
								"Either the sensor or a filter was not specified as argument or guessable from previous use. Ensure all necessary data is set.\n"));
				for (int z = 0; z < 8; z++) {
					g_free(spcc_strings_to_free[z]);
				}
				free(pcc_args);
			...
			siril_log_message(
					_(
							"SPCC will use mono senor \"%s\" and filters \"%s\", \"%s\" and \"%s\n"),
					monosensor ? monosensor : com.pref.spcc.monosensorpref,
					rfilter ? rfilter : com.pref.spcc.redpref,
					gfilter ? gfilter : com.pref.spcc.greenpref,
					bfilter ? bfilter : com.pref.spcc.bluepref);
		}
		pcc_args->selected_white_ref = get_favourite_spccobject(
				com.spcc_data.wb_ref,
				whiteref ? whiteref : "Average Spiral Galaxy");
...
int process_platesolve(int nb) {
	gboolean noflip = FALSE, force = FALSE, downsample = FALSE, autocrop = TRUE,
			nocache = FALSE, asnet_blind_pos = FALSE, asnet_blind_res = FALSE,
			noreg = FALSE;
	...
	double mag_offset = 0.0, target_mag = -1.0, searchradius =
			com.pref.astrometry.radius_degrees;
	...
	if (seqps) {
		...
		if (seq->type == SEQ_SER) {
			siril_log_color_message(
					_(
							"SER cannot contain WCS info, plate solving will export to FITS cube format\n"),
					"salmon");
		}
		next_arg++;
		if (check_seq_is_comseq(seq)) {
	...
	if (nb > next_arg
			&& (word[next_arg][0] != '-'
					|| (word[next_arg][1] >= '0' && word[next_arg][1] <= '9'))) {
		...
		if (!sep) {
			...
			target_coords = siril_world_cs_new_from_objct_ra_dec(word[next_arg],
					word[next_arg + 1]);
			next_arg += 2;
		} else {
			*sep++ = '\0';
	...
	while (nb > next_arg && word[next_arg]) {
		if (!strcmp(word[next_arg], "-noflip"))
			...
			if (cat != CAT_AUTO)
				siril_log_message(
						_(
								"Specifying a catalog has no effect for astrometry.net solving\n"));
	...
	if (!seqps && !force) {
		siril_log_message(
				_("Image is already plate solved. Nothing will be done.\n"));
		goto clean_and_exit_platesolve;
		// not an arror, retval is CMD_OK
	}

	if (local_cat
			&& (cat == CAT_AUTO
					|| (cat != CAT_GAIADR3 && cat != CAT_PPMXL
							&& cat != CAT_APASS))) {
		cat = CAT_LOCAL;
	...
	if (solver == SOLVER_LOCALASNET && !asnet_is_available()) {
		siril_log_color_message(
				_(
						"The local astrometry.net solver was not found, aborting. Please check the settings.\n"),
				"red");
		retval = CMD_GENERIC_ERROR;
		goto clean_and_exit_platesolve;
	}
	...
	if (solver == SOLVER_SIRIL) {
		if (asnet_blind_pos) {
			siril_log_color_message(
					_("Siril internal solver cannot be set blind in %s\n"),
					"salmon", _("position"));
	...
	if (!target_coords) {
		...
		if (solver != SOLVER_LOCALASNET && !target_coords) {
			siril_log_color_message(
					_(
							"Cannot plate solve, no target coordinates passed and image header doesn't contain any either\n"),
					"red");
			if (seqps)
				clearfits(preffit);
		...
		if (target_coords) {
			siril_log_message(
					_("Using target coordinate from image header: %f, %f\n"),
	...
	if (target_coords && asnet_blind_pos) {
		if (forced_metadata[FORCED_CENTER]) {
			siril_log_color_message(
					_(
							"%s is ignored when using local astrometry.net blind in %s\n"),
	...
	if (asnet_blind_res) {
		...
		args->pixel_size = max(preffit->keywords.pixel_size_x,
				preffit->keywords.pixel_size_y);
		if (args->pixel_size <= 0.0) {
			args->pixel_size = com.pref.starfinder_conf.pixel_size_x;
			if (args->pixel_size <= 0.0) {
				siril_log_color_message(
						_(
								"Pixel size not found in image or in settings, cannot proceed\n"),
						"red");
				if (seqps)
					clearfits(preffit);
				retval = CMD_INVALID_IMAGE;
			...
			siril_log_message(_("Using pixel size from image: %.2f\n"),
					args->pixel_size);
	}
	...
	if (asnet_blind_res) {
		...
		if (args->focal_length <= 0.0) {
			...
			if (args->focal_length <= 0.0) {
				siril_log_color_message(
						_(
								"Focal length not found in image or in settings, cannot proceed\n"),
						"red");
				if (seqps)
					clearfits(preffit);
				retval = CMD_INVALID_IMAGE;
			...
			siril_log_message(_("Using focal length from image: %.2f\n"),
					args->focal_length);
	}
	...
	if (target_mag > -1.0) {
		if (solver != SOLVER_SIRIL)
			siril_log_message(
					_(
							"Magnitude alteration arguments are useless for astrometry.net plate solving\n"));
		else {
	...
	if (!searchradius && solver == SOLVER_LOCALASNET && !asnet_blind_pos) {
		...
		siril_log_color_message(
				_(
						"Cannot force null radius for localasnet if not blind solving, using default instead\n"),
				"red");
	} else {
	...
	if (seqps || sequence_is_loaded()) { // we are platesolving an image from a sequence or a sequence, we can't allow to flip (may be registered)
		noflip = TRUE;
		siril_debug_print(
				"forced no flip for solving an image from a sequence\n");
	}
	...
	if (solver == SOLVER_LOCALASNET) { // see !432
		if (single_image_is_loaded() && com.uniq && com.uniq->filename) {
			...
			args->filename = g_strdup_printf("%s%.5d", com.seq.seqname,
					com.seq.current + 1);
		} else {
	...
	return CMD_OK;
	clean_and_exit_platesolve: if (check_seq_is_comseq(seq))
		free_sequence(seq, TRUE);
	if (target_coords)
		siril_world_cs_unref(target_coords);
	if (args)
...
static conesearch_params* parse_conesearch_args(int nb) {
		...
		if (g_str_has_prefix(word[arg_idx], "-cat=")) {
			...
			if (!g_strcmp0(arg, "tycho2"))
				...
				if (!gfit.keywords.date_obs) {
					siril_log_color_message(
							_(
									"This option only works on images that have observation date information\n"),
							"red");
					g_free(params->obscode);
					g_free(params);
					return NULL;
			...
			if (strlen(arg) != 3) {
				siril_log_color_message(
						_(
								"The observatory should be coded as a 3-letter word\n"),
						"red");
				g_free(params->obscode);
				g_free(params);
				return NULL;
			...
			if (!local_cat) {
				siril_log_color_message(
						_("No local catalogues found, ignoring -trix option\n"),
						"red");
				continue;
			}
			gchar *end;
			...
			if (trix < 0 || trix > 511) {
				siril_log_color_message(
						_("Trixel number must be between 0 and 511\n"), "red");
				g_free(params->obscode);
				g_free(params);
				return NULL;
			}
			...
			if (!(g_ascii_strcasecmp(arg, "on")))
				...
				siril_log_message(
						_(
								"Wrong parameter values. Log must be set to on or off, aborting.\n"));
				g_free(params->obscode);
				g_free(params);
				return NULL;
			}
			...
			if (!(g_ascii_strcasecmp(arg, "on")))
				...
				siril_log_message(
						_(
								"Wrong parameter values. Tag must be set to on or off, aborting.\n"));
				g_free(params->obscode);
				g_free(params);
				return NULL;
			}
	...
	if (params->compare && !is_star_catalogue(params->cat)) {
		siril_log_message(
				_(
						"Cannot use -compare argument with non-star catalogues, ignoring.\n"));
...
int process_start_ls(int nb) {
	...
	gboolean use_file_watcher = FALSE/*, remove_gradient = FALSE*/, shift_reg =
			TRUE, use_32b = FALSE;
	for (int i = 1; i < nb; i++) {
		if (!word[i])
			continue;
		if (g_str_has_prefix(word[i], "-dark="))
			...
			siril_log_message(
					"gradient removal in live stacking is not yet implemented\n");
			return CMD_GENERIC_ERROR;
		} else if (!strcmp(word[i], "-watch_files")) {
			//use_file_watcher = TRUE;
			siril_log_message(
					"file watcher in headless live stacking is not yet implemented\n");
			return CMD_GENERIC_ERROR;
	...
	return start_livestack_from_command(dark_file, flat_file,
			use_file_watcher/*, remove_gradient*/, shift_reg, use_32b);
}
...
int process_livestack(int nb) {
	...
	if (!livestacking_is_started()) {
		siril_log_message(
				_(
						"Live stacking needs to be initialized with the START_LS command first\n"));
		return CMD_NEED_INIT_FIRST;
	}
	if (livestacking_uses_filewatcher()) {
		siril_log_message(
				_(
						"Live stacking was configured to use file watching, not this command\n"));
		return CMD_ARG_ERROR;
	}
...
int process_parse(int nb) {
	...
	if (gfit.header) { // fits or astrotiff - do not update the header
		...
		expression = update_header_and_parse(&gfit, word[1], mode, FALSE,
				&status);
	}
...
static show_params* parse_show_args(int nb) {
	...
	parse_coords: if (nb > next_arg && !isalpha(word[next_arg][0])
			&& (isdigit(word[next_arg][0]) || isdigit(word[next_arg][1]))) {
		char *sep = strchr(word[next_arg], ',');
		if (!sep) {
			params->coords = siril_world_cs_new_from_objct_ra_dec(
					word[next_arg], word[next_arg + 1]);
			next_arg += 2;
		} else {
			*sep++ = '\0';
			params->coords = siril_world_cs_new_from_objct_ra_dec(
...
cut_struct* parse_cut_args(int nb, sequence *seq, cmd_errors *err) {
	...
	*err = CMD_OK;
	...
	int nb_layers =
			(cut_args->seq) ?
					cut_args->seq->nb_layers : cut_args->fit->naxes[2];
	for (int i = start; i < nb; i++) {
		char *arg = word[i], *end;
		if (!word[i])
			break;
		if (g_str_has_prefix(word[i],
				"-tri") || g_str_has_prefix(word[i], "-bgremove")) {
			cut_args->tri = TRUE;
			...
			sensor_pattern pattern = get_cfa_pattern_index_from_string(
					reffit.keywords.bayer_pattern);
			if ((reffit.naxes[2] > 1)
					|| ((!(pattern == BAYER_FILTER_RGGB
							|| pattern == BAYER_FILTER_GRBG
							|| pattern == BAYER_FILTER_BGGR
				...
				siril_log_color_message(
						_(
								"Error: CFA mode cannot be used with color images or mono images with no Bayer pattern.\n"),
						"red");
				*err = CMD_ARG_ERROR;
				break;
			}
			...
			if (g_str_has_prefix(arg, "red"))
				...
				siril_log_message(
						_("Incorrect option follows -layer=, aborting.\n"));
				*err = CMD_ARG_ERROR;
				break;
			}
			...
			if ((*err = read_cut_pair(value, &cut_args->cut_start))) {
				siril_log_color_message(
						_("Error: Could not parse -from values.\n"), "red");
				break;
			}
			...
			if ((*err = read_cut_pair(value, &cut_args->cut_end))) {
				siril_log_color_message(
						_("Error: Could not parse -to values.\n"), "red");
				break;
			}
			...
			if ((*err = read_cut_pair(value, &cut_args->cut_wn1))) {
				siril_log_color_message(
						_("Error: Could not parse -wn1at values.\n"), "red");
				break;
			}
			...
			if ((*err = read_cut_pair(value, &cut_args->cut_wn2))) {
				siril_log_color_message(
						_("Error: Could not parse -wn2at values.\n"), "red");
				break;
			}
		} else if (g_str_has_prefix(arg, "-filename=")) {
			if (seq) {
				siril_log_color_message(
						_("Error: this option cannot be used for sequences.\n"),
						"red");
				*err = CMD_ARG_ERROR;
				break;
			}
...
int process_icc_assign(int nb) {
	...
	if (!g_ascii_strncasecmp(arg, "srgblinear", 10)) {
		...
		profile =
				gfit.naxes[2] == 1 ?
						gray_linear() :
						siril_color_profile_linear_from_color_profile(
								com.icc.working_standard);
	} else if (!g_ascii_strncasecmp(arg, "working", 7)) {
		profile = copyICCProfile(
				gfit.naxes[2] == 1 ?
						com.icc.mono_standard : com.icc.working_standard);
	} else if (g_file_test(arg, G_FILE_TEST_EXISTS)
			&& g_file_test(arg, G_FILE_TEST_IS_REGULAR)) {
...
int process_icc_convert_to(int nb) {
	...
	if (!gfit.icc_profile) {
		siril_log_color_message(
				_(
						"Image has no color profile assigned to convert from. Assign a profile first.\n"),
				"red");
	...
	if (word[2]) {
		if (!g_ascii_strcasecmp(word[2], "perceptual"))
			...
			siril_log_color_message(
					_(
							"Specified intent not recognized. Specify one of \"perceptual\", \"relative\", \"saturation\" or \"absolute\"\n"),
					"red");
			com.pref.icc.processing_intent = temp_intent;
			return CMD_GENERIC_ERROR;
		}
...
int process_disto(int nb) {
	if (!has_wcs(&gfit) || !gfit.keywords.wcslib->lin.dispre) {
		siril_log_color_message(
				_(
						"This command only works on plate solved images with distortions included\n"),
				"red");
		return CMD_FOR_PLATE_SOLVED;
	}
	if (nb > 2)
...
int process_limit(int nb) {
	...
	if (gfit.type == DATA_USHORT) {
		siril_log_message(
				_(
						"16-bit images cannot have out-of-range pixels: nothing to do.\n"));
		return CMD_OK;
	}
