...
int process_start_ls(int nb) {
	...
	gboolean use_file_watcher = FALSE/*, remove_gradient = FALSE*/, shift_reg = TRUE;
	for (int i = 1; i < nb; i++) {
		if (!word[i])
			continue;
	...
	return start_livestack_from_command(dark_file, flat_file, use_file_watcher/*, remove_gradient*/, shift_reg);
}
