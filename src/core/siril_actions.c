...
void binarize_mask_activate(GSimpleAction *action, GVariant *parameter, gpointer user_data) {
	if (!gfit || !gfit->mask || !gfit->mask->data) {
		siril_message_dialog(GTK_MESSAGE_ERROR, _("No mask present"),
		                     _("There is no mask to binarize."));
		return;
	}

	// Show a dialog to get binarization parameters
	GtkWidget *dialog = gtk_dialog_new_with_buttons(_("Binarize Mask"),
	...
	// Determine default hi value based on mask bitpix
	float default_hi;
	...
	GtkWidget *label_lo = gtk_label_new(_("Low threshold:"));
	gtk_widget_set_halign(label_lo, GTK_ALIGN_END);
	GtkWidget *spin_lo = gtk_spin_button_new_with_range(0.0, default_hi, 0.01);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(spin_lo), 0.0);
	gtk_spin_button_set_digits(GTK_SPIN_BUTTON(spin_lo), gfit->mask->bitpix == 32 ? 6 : 2);
	gtk_grid_attach(GTK_GRID(grid), label_lo, 0, 0, 1, 1);
	gtk_grid_attach(GTK_GRID(grid), spin_lo, 1, 0, 1, 1);

	GtkWidget *label_hi = gtk_label_new(_("High threshold:"));
	gtk_widget_set_halign(label_hi, GTK_ALIGN_END);
	GtkWidget *spin_hi = gtk_spin_button_new_with_range(0.0, default_hi, 0.01);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(spin_hi), default_hi);
	gtk_spin_button_set_digits(GTK_SPIN_BUTTON(spin_hi), gfit->mask->bitpix == 32 ? 6 : 2);
	gtk_grid_attach(GTK_GRID(grid), label_hi, 0, 1, 1, 1);
	gtk_grid_attach(GTK_GRID(grid), spin_hi, 1, 1, 1, 1);
	...
	if (response == GTK_RESPONSE_ACCEPT) {
		float lo = gtk_spin_button_get_value(GTK_SPIN_BUTTON(spin_lo));
		float hi = gtk_spin_button_get_value(GTK_SPIN_BUTTON(spin_hi));

		gtk_widget_destroy(dialog);

		if (lo >= hi) {
			siril_message_dialog(GTK_MESSAGE_ERROR, _("Invalid parameters"),
