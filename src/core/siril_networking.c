...
// following and hardening, and timeouts/signal-safety so a dead or stalled server
// cannot hang the calling thread (NOSIGNAL is also required when called from
// worker threads). The optional CA bundle from $CURL_CA_BUNDLE is applied here
// too, so every transfer (GET, range and the liveness probe) picks it up
// consistently. Keeping this in one place stops the three transfer paths drifting
// apart as options are added.
static CURLcode apply_common_curl_opts(CURL *curl) {
	...
	if (ca_bundle) {
		...
		if (curl_easy_setopt(curl, CURLOPT_CAINFO, ca_bundle) != CURLE_OK)
			siril_log_debug("Could not set CURLOPT_CAINFO from CURL_CA_BUNDLE\n");
	}
	return retval;
}
...
// names, coordinates, search terms, etc.). Returns a newly allocated string
// (free with g_free), or NULL if value is NULL.
