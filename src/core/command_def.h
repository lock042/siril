...
#define STR_CONVERT N_("Convert all images in a known format into Siril's FITS images. For FITS images, Siril will try to make a symlink. If not possible, files are copied. \"-start=index\" sets the starting index parameter and the \"-out=\" option allows to convert files into another directory" )
