...
static int readinitfile() {
	...
	if (debayer_setting) {
		...
		config_setting_lookup_bool(debayer_setting, "guess_orientation", &com.pref.debayer.guess_orientation);
...
static void _save_debayer(config_t *config, config_setting_t *root) {
	...
	debayer_setting = config_setting_add(debayer_group, "guess_orientation", CONFIG_TYPE_BOOL);
	config_setting_set_bool(debayer_setting, com.pref.debayer.guess_orientation);
