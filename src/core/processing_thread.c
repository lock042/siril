/*
 * processing_thread.c — persistent single-worker-thread job queue
 *
 * This file replaces the "PROCESSING THREAD MANAGEMENT" section of the
 * original processing.c (approximately the block bounded by the banner
 * comment and the end of unreserve_thread / end_generic).  Everything else
 * in processing.c — generic_sequence_worker, generic_image_worker,
 * seq_prepare_hook, etc. — is unchanged; they call start_in_new_thread,
 * stop_processing_thread, get_thread_run, and waiting_for_thread exactly as
 * before.
 *
 * See processing_thread.h for the full design rationale and invariants.
 */

/* ── The only include needed beyond what processing.c already has ─────── */
...
/* (In practice merge this block into the existing processing.c and keep its
...
static void
processing_job_free(ProcessingJob *job)
{
...
static gpointer
worker_thread_main(gpointer user_data G_GNUC_UNUSED)
{
...
void
processing_system_shutdown(void)
{
    if (!worker_running)
        return;
...
ProcessingJob *
processing_submit_job(ProcessingFunc func, gpointer data)
{
...
void
processing_wait_for_job(ProcessingJob *job)
{
    if (!job)
        return;
    g_mutex_lock(&job->mutex);
...
void
processing_request_cancel(void)
{
...
gboolean
processing_is_job_active(void)
{
    return g_atomic_int_get(&job_active_flag) != 0;
}

gboolean
processing_in_worker_thread(void)
{
...
gboolean
start_in_new_thread(ProcessingFunc func, gpointer data)
{
    /*
...
gboolean
start_in_reserved_thread(ProcessingFunc func, gpointer data)
{
    /*
...
gboolean
end_generic(gpointer arg G_GNUC_UNUSED)
{
    stop_processing_thread();
    set_cursor_waiting(FALSE);   /* belt-and-suspenders; stop_processing_thread
                                    already calls this, but the old callers
                                    relied on the double-call being harmless   */
    return FALSE;
}
