...
- [x] Replace `COPY_BE64` macro definition (lines 67–79) with
...
- [x] Rename all 97 `COPY_BE64(…)` call sites in this file to
      `COPY_FIELD(…)`. Each call site's `type` argument is
      individually re-audited in §4.2bis.

#### Macros to ELIMINATE entirely

- [x] Delete `FROM_BE64_INTO` macro (lines 81–87). At each of its
...
- [x] Delete `TO_BE64_INTO` macro (lines 89–97). At each of its
...
- [x] Update the comment block above the macros and the line-125
...
- [x] **`keywords_to_py`** in `siril_pythoncommands.c:101+` ↔
...
- [x] **`imstats_to_py`** in `siril_pythoncommands.c` ↔ the
      `ImageStats` deserialiser in `models.py`. Audit pending.
- [x] **`homography_to_py`** ↔ `Homography` deserialiser. Audit
      pending.
- [x] **`distodata_to_py`** ↔ `DistoData` deserialiser. Audit
      pending.
- [x] **`imgdata_to_py`** ↔ `ImgData` deserialiser. Audit pending.
- [x] **`psfstar_to_py`** ↔ `PSFStar` deserialiser. Audit pending.
- [x] **`analysis_to_py`** ↔ wherever the analysis tuple is parsed.
- [x] **`fits_to_py`** ↔ `FFit` deserialiser. This is the
...
- [x] `send_response()` in `siril_pythonmodule.c:128` — drop the
      `GUINT32_TO_BE` wrap on `ResponseHeader.length`.
- [x] `handle_client_communication()` (or whichever function reads
      the CommandHeader at line ~3000-ish) — drop the corresponding
      `GUINT32_FROM_BE`. (Need to verify the actual site.)
- [x] Header type comments: update inline `// Convert to network
      byte order` style comments to reflect the new reality.

### 4.4 `incoming_image_info_t` parsing in `siril_pythonmodule.c`

- [x] `handle_set_pixeldata_request` (lines 613–623) — drop the
      `FROM_BE` on width/height/channels/size/data_type.
- [x] `handle_plot_request` (lines 790–800) — drop equivalents.
- [x] `handle_save_image_file_request` (lines 937–942) — drop
      equivalents.
- [x] Other `handle_*_request` functions taking
...
- [x] Header send/recv (`'!Bi'`, `'!BI'`) — lines ~329, 362, 388
- [x] Size headers (`'!Q'`) — line ~528
- [x] Progress / float payloads (`'!f'`) — line ~1096
- [x] Rectangles + region selectors (`'!IIII'`, `'!IIIII'`,
      `'!IIIIII'`) — many lines (~1212, 1237, 1295, 1363, 1425,
      1427, 1611, 1611, 1753, etc.)
- [x] Single ints (`'!I'`, `'!i'`) — many lines (~1271, 1432,
      2265, 2747, 2770, 2820, 2851, 2962, 2993, 3547, 3655, 4154,
      4221)
- [x] Two doubles (`'!2d'`) — lines ~1473, 1530
- [x] Mixed bool + int combinations (`'!??'`, `'!I???'`, `'!???'`,
      `'!I?'`) — lines ~1600, 1741, 1742, 3267, 4191, 4444
- [x] Multi-int with strings (`'!II'`) — lines ~2890, 2927
- [x] Bool / int parse on response (`'!I'`, `'!i'`, `'!d'`) —
      lines ~3780, 3782, 3784, 4121
- [x] Anything else `grep` finds after the batches above — final
      sweep to confirm `grep -E "struct\.(pack|unpack)\([^,]*['\"]\!"
      connection.py` returns nothing.
- [x] Update inline comments that say "network byte order" to say
      "native byte order".
...
- [x] **`FFit`** — currently assembled inline in
...
- [x] **`Homography`** — currently has no methods. Mirrors C's
...
- [x] **`DistoData`** — no methods today. Mirrors C's
      `distodata_to_py`. Add the same pair. Find call site(s).
- [x] **`Sequence`** — no methods today. Audit C's `seq_to_py`
      (if it exists) / wherever sequence data gets serialized.
      Add `serialize` / `deserialize` to match.
- [x] **`FPoint`** — has `serialize` but no `deserialize`. Add
      the missing half.
- [x] **`_SharedMemoryInfo`** (in `shm.py`) — a `ctypes.Structure`
      ...
      repetition.
- [x] **General sweep**: `grep -nE "struct\.unpack" connection.py`
...
- [x] `python_module/pyproject.toml`: `1.1.21` → `1.1.22`.
- [x] Document in plan §6 (rollout) that this is a wire-protocol
...
- [x] Scripts that explicitly check `check_module_version` for
...
- [x] `grep -E "GUINT[136][26]_TO_BE|GUINT[136][26]_FROM_BE|GINT[136][26]_TO_BE|GINT[136][26]_FROM_BE|GUINT64_TO_BE|GUINT64_FROM_BE" src/io/siril_python*.c` returns zero results.
- [x] `grep -E "struct\.(pack|unpack)\([^,]*['\"]\!" python_module/sirilpy/connection.py python_module/sirilpy/models.py` returns zero results.
- [x] `grep -E "COPY_BE64|FROM_BE64_INTO|TO_BE64_INTO" src/io/siril_python*.c` returns zero results (all macros renamed or deleted).
- [x] After §4.7bis: every model class with a non-trivial wire
...
- [x] **Wire-size sanity**: after each `*_to_py` audit (§4.2bis),
...
- [x] **Headers**: any pyscript that runs at all (the command/
      response dispatch goes through them on every roundtrip).
- [x] **Single-int reads/writes**: `SirilInterface.get_active_vport`,
      `SirilInterface.get_selection`, `set_selection`.
- [x] **Dimensions / image metadata**: `get_dimensions()`,
      `is_image_loaded()`, `is_sequence_loaded()`.
- [x] **Pixel data round-trip**: `get_pixeldata()` →
      `set_pixeldata()` with a small fits.
- [x] **FITS header / keywords**: `get_fits_header()`,
      `get_keywords()`, `set_image_header()`. These exercise the
      `COPY_NATIVE64` / `FROM_NATIVE64_INTO` macros.
- [x] **Float payloads**: `update_progress(0.5)`.
- [x] **Double pairs**: `pix2wcs(x, y)`, `wcs2pix(ra, dec)`.
- [x] **SHM transfer**: any operation that uses
...
- [x] **Polygons**: `add_user_polygon(...)`, `delete_user_polygon`.
...
6. **Bulk-struct: `keywords_to_py` ↔ `FKeywords`** — the
   ...
   **[done — `97ec17520`]** Net 1465 → 1409 bytes per keyword block.
7. **Bulk-struct: `imstats_to_py` ↔ `ImageStats`**.
   **[done — `438e204ae`]** One-character flip (`!2q12d` → `=2q12d`);
   the int64_t cast on the C side stays for Windows `long` portability.
8. **Bulk-struct: `homography_to_py` ↔ `Homography`** — in this
   commit ALSO add the missing `Homography.serialize` /
   `.deserialize` (§4.7bis) so the field-format string lives on
   the class. Retire any inline unpacker.
   **[done — `909ed510a`]** Homography 88 → 80 bytes; RegData
   136 → 108 bytes. Adds `Homography.{serialize,deserialize}`.
9. **Bulk-struct: `distodata_to_py` + `imgdata_to_py` ↔
   ...
   **[done — `cfb94208c`]** ImgData 48 → 32 bytes; DistoData head
   24 → 12 bytes. `DistoData.{serialize,deserialize}` added,
   `get_seq_distodata` inline parser retired.
10. **Bulk-struct: `psfstar_to_py` + `analysis_to_py` ↔
    `PSFStar`, analysis tuple**.
    **[done — `7c76c2885`]** PSFStar 296 → 276 bytes; ImageAnalysis
    151 → 131 bytes. Drive-by fix for a pre-existing NameError in
    `ImageAnalysis.serialize()` (`FLEN` → `self.FLEN`).
11. **Bulk-struct: `fits_to_py` ↔ `FFit`** — the composite that
    ...
    **[done — `980c4aa08`]** FFit core 104 → 60 bytes. End-to-end
    `iface.get_image()` returns the right `_naxes`, bitpix, and
    `keywords.program`.
...
11a. **Class consolidation cleanup** — add the remaining missing
    ...
    **[done — `4151745c3`]** Sequence head 128 → 68 bytes;
    `_SharedMemoryInfo.from_wire_buffer` retires three inline
    unpack-and-construct blocks in `get_seq_frame`; `FPoint` gets
    matching `serialize` / `deserialize`.
...
12. **Final sweep + version bump** — `grep` confirms zero
    ...
    compatibility note. **[this commit]** Also collapsed the
    remaining 50+ misleading `_BE` local-var names on the C side
    (artefacts of the byte-swap era), simplified the
    pix2wcs / wcs2pix temp-var dance, and rewrote the misleading
    "network byte order" comments throughout connection.py /
    plot.py to say "native byte order" instead.
