...
class ConnectionError(SirilError):
    """
    Raised when there are problems connecting to or communicating with Siril.
    ...
    """
    def __init__(self, message: str = "Failed to connect to Siril"):
...
class DataError(SirilError):
    ...
    - Buffer overflows
    """
    def __init__(self, message: str = "Error handling data"):
        super().__init__(message)

class NoImageError(SirilError):
    """Exception raised when trying to operate on non-existent image."""
    def __init__(self, message: str = "No Siril image loaded"):
        super().__init__(message)
